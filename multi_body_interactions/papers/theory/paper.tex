...
\begin{abstract}
  ...
  fermionic AEAs which exhibits emergent SU($N$)-symmetric multi-body
...
% Rapid advancements in the experimental capabilities with ultracold
% alkaline-earth-like atoms (AEAs) \cite{fukuhara2007degenerate,
% stellmer2009boseeinstein, deescobar2009boseeinstein,
% tey2010doubledegenerate, desalvo2010degenerate, taie2012su,
% martin2013quantum, cappellini2014direct, scazza2014observation,
% zhang2014spectroscopic, cazalilla2014ultracold} bring to a
% surprisingly near term the prospect of performing quantum
% simulations of spin models and lattice field theories exhibiting
% SU($N$) symmetry \cite{cazalilla2009ultracold, hermele2009mott,
% hermele2011topological, gorshkov2010twoorbital, banerjee2013atomic,
% cazalilla2014ultracold}.  AEAs have ground and metastable
% (electronic) orbital states\note{[I want to briefly discuss how we
% present this]} ${}^1S_0$ and ${}^3P_0$, often referred to as the
% ``clock'' states due to their use in optical lattice clocks
% \cite{derevianko2011colloquium, katori2011optical}, with
% ultra-narrow (millihertz) optical transition linewidths and
% associated $\sim10^2$ second excited-state lifetimes.  Due to the
% lack of electronic orbital angular momentum in the ${}^1S_0$ and
% ${}^3P_0$ states, AEAs exhibit decoupled orbital and nuclear spin
% degrees of freedom.  For atoms with total nuclear spin $I$ and
% $N=2I+1$ nuclear spin levels, this decoupling leads to
% nuclear-spin-conserving SU($N$)-symmetric interactions governed
% entirely by orbital-state parameters \cite{cazalilla2009ultracold,
% taie2012su, zhang2014spectroscopic}.
...
The atoms have a unique, ultra-narrow optical transition between
...
nuclear spin, and motional states, can be fully controlled in a 3-D
lattice with high fidelity \cite{daley2008quantum,
...
severe sign problems which plague classical simulations of strongly
interacting fermionic systems \cite{wieseu.-j.2013ultracold,
  wu2003exact}.

% One of the major drivers of experimental capabilities with AEAs is
% the development of optical lattice clocks
% \cite{derevianko2011colloquium, katori2011optical}.  These clocks
% traditionally operate in a one-dimensional (1-D) geometry with
% harmonic confinement of atoms in the plane orthogonal to the lattice
% axis \cite{derevianko2011colloquium, hinkley2013atomic,
% bloom2014optical}.  By operating at higher atomic densities, one can
% simultaneously interrogate a larger number of atoms and thereby
% increase the signal-to-noise ratio of the clock.  With the extreme
% sensitivities accessible to state-of-the-art optical lattice clocks,
% however, 1-D clock experiments with ${}^{87}$Sr and ${}^{171}$Yb
% detect density-dependent fractional frequency shifts at the
% $10^{-16}$ level \cite{campbell2009probing, lemke2009spin}.  For the
% purpose of timekeeping, these frequency shifts amount to an
% undesirable source of systematic error, but they introduced a new,
% exciting paradigm for the field of quantum simulation: a marriage
% between precision clock spectroscopy and quantum many-body physics
% \cite{taie2012su, scazza2014observation, martin2013quantum,
% cappellini2014direct, zhang2014spectroscopic, rey2014probing}.
% Somewhat counter-intuitively, one of the most promising strategies
% for improving the interaction-limited 1-D clock design has been to
% use higher dimensional lattices to generate tighter on-site
% confinement of atoms, thereby {\it increasing} the strength of
% inter-atomic interactions and making evolution into interacting
% many-body states energetically unfavorable
% \cite{swallows2011suppression, lemke2011wave,
% campbell2017fermidegenerate}.  A wonderful side-effect of this
% strategy for building better clocks is the development of a highly
% controllable quantum many-body system in the strongly-interacting
% regime, where inter-atomic interactions set the largest energy scale
% relevant for system dynamics.
...
for the first time with fermionic atoms with internal degrees of
freedom and multiple collisional parameters.  Nonetheless, due to the
SU($N$) symmetry of these collisions, we are able to find a simple way
to express effective multi-body Hamiltonians and fully characterize
their low-lying many-body energy eigenstates.  While some past work
has detected experimental signatures of multi-body interactions in the
...
ref.~\cite{goban2018emergence}, similarly to the measurements of
...
$s\in\set{\g,\e}$; and $E_n$ is the energy of a single atom on site
$i$ in motional state $n$.  In a harmonic trap approximation we would
have $E_n=\p{3/2+n_x+n_y+n_z}\omega$ for an on-site angular trap
...
\begin{align}
  ...
  \label{eq:couplings}
\end{align}
where $m_A$ is the mass of a single atom.  Defining the coupling
tensor
\begin{align}
...
\end{align}
For nuclear spins $\mu,\nu$, the coupling tensor $G^{qr}_{st}$ gives
the coupling constant between the two-atom states
...
($i$) on all operators, and implicitly sum over all indices in a
summand which do not have a fixed value.  We may also at times
suppress motional state indices on the overlap integral
...
$K^{k\ell}_{mn}={K^{k\ell}_{mn}}^*=K^{mn}_{k\ell}$.  This choice is
always possible due to the analytic properties of Wannier orbitals
$\phi_{in}$ \cite{kohn1959analytic}.
...
observed spectra of multiply-occupied lattice sites
\cite{goban2018emergence}.  The reason for this failure lies in the
fact that the bare two-body Hamiltonian $H_{\t{int}}$ in
\eqref{eq:H_int} is not diagonal in the basis of single-particle
motional states.  Even at zero temperature, therefore, the presence of
excited motional states modifies many-body atomic energy spectra when
off-diagonal elements of $H_{\t{int}}$ approach the scale of low-lying
single-particle motional state energy gaps.  To address this problem,
we develop a low-energy effective theory for ultracold two-level
fermionic atoms on a lattice.  As our work is motivated by progress in
experiments which initialize all atoms in identical (i.e.~ground)
orbital and motional states, to simplify our theory we assume that any
$N$ atoms on a single lattice site occupy $N$ nuclear spin states.
Multiple occupation of a single nuclear spin state on the same lattice
site is initially forbidden by fermionic statistics, and subsequently
cannot be violated in the absence of tunneling and hyperfine coupling.
...
of each atom (e.g.~$I=9/2$ for ${}^{87}$Sr), and the sum ends at
...
excitations of this theory, we find that we can express multi-body
Hamiltonians in the simple form
\begin{align}
...
with $j=1,\cdots,M$ for which all $\mu_j$ are distinct, or
...
dimensionality of our target Hilbert space.
...
\begin{align}
  ...
  \label{eq:H_int_3}
\end{align}
where $\sp{X,Y}_+\equiv XY+YX$.  The naive idea of applying
$H_{\t{int}}$ directly on the single-particle motional ground states
and neglecting all excited states is thus equivalent to truncating our
expansion for $H_{\t{int}}^{\t{eff}}$ at first order.  In addition to
this first order term, the expansion involves {\it effective
  corrections} to the action of $H_{\t{int}}$ on the single-particle
motional ground states (i.e.~on $\H_{\t{ground}}^{\t{single}}$) in the
form of higher-order terms with {\it intermediate} or {\it virtual
  occupation} of excited states, manifest in $\I$.
...
performed over a basis for the orthogonal complement of
...
$p$ in $H_{\t{int}}$ can be represented by directed, weakly connected
graphs (i.e.~which are connected when ignoring edge direction)
containing $p$ internal vertices.  Each internal vertex and its
...
\begin{figure}
  ...
  \caption{\footnotesize An example second-order diagram and the
    ...
    features, as they are not necessary to uniquely identify a
    diagram.  We will also drop explicit appearances of the
    ground-state projector $\P_0$ in our expressions, with the
...
negative sign is as prescribed by $H_{\t{int}}^{(2)}$ in
\eqref{eq:H_int_1_2}.
...
order, we introduce an expanded coupling tensor
\begin{align}
...
indices, the expanded coupling tensor allows us to collect together
diagrams which have identical graph topologies, but address different
...
associated negative sign if they contain only one excited virtual
state, and a factor of $1/2$ if they contain a virtual ground state.
...
in a contribution to the effective Hamiltonian which is equal to
...
the second and third of these diagrams pick up a factor of $-1/2$, and
the sum of the disconnected diagrams vanishes.
...
$j=1,\cdots,N$, the $M$-body Hamiltonian $H_M$ has a single ground
...
\begin{table}
  ...
  \caption{\footnotesize Low-excitation eigenvalues of $M$-body
    ...
    the nuclear spins they occupy (i.e.~$\N$ with $\abs{\N}=N$) and
    whether they are in the orbital ground ($0$), singly-excited
...
$\bk{I,\mu;1,0|I,\mu}\sim\mu$ for a photon-induced
...
$j=1,\cdots,N$.  If we red-detune the interrogation laser by $\delta$
from a many-body orbital state excitation energy, i.e.~set
...
% These efforts will be necessary to go beyond the deep-lattice limit
% and study inter-site orbital-state and nuclear spin exchange
% dynamics.  Nonetheless, our work makes a major step towards the
% experimental investigation of multi-body SU($N$) physics.
...
contributions from A.~Goban.  This work is supported by NIST, DARPA,
W911NF-16-1-0576 through ARO, AFOSR, AFOSR-MURI, NSF-1734006, NASA,
and CTQM.
