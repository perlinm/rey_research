...
\title{SU($n$) spin models in cold atomic systems}
...
SU(2)-symmetric spin interactions, for example, can enable the development of quantum sensors that use entanglement to surpass classical limits on measurement precision \cite{he2019engineering, perlin2020spin}.
...
\red{[summarize paper and findings (phases etc.)]}
...
\begin{align}
  ...
  H_{\t{int}}
  &= \f{U}{N} \sum_{k,\ell,p,q,\mu,\nu}
  c_{k\mu}^\dag c_{\ell\mu} c_{p\nu}^\dag c_{q\nu}
  \times \delta_{k+p,\ell+q},
\end{align}
where $N$ is the total number of atoms on the lattice, we define $U\equiv \frac{U_0}{2}\times\frac{N}{L}$ for convenience, $\delta_{k+p,\ell+q}=1$ if $k+p=\ell+q$ and $0$ otherwise (enforcing conservation of momentum), and we work in units with lattice spacing $a=1$.

If the interaction energy $U_0$ is smaller than the single-particle bandwidth $\sim J$, then the mode-changing collisions in $H_{\t{int}}$ become off-resonant, motivating the frozen-mode approximation $\set{k,p}=\set{\ell,q}$.
In this case, the terms in $H_{\t{int}}$ with $k=\ell$ and $p=q$ become proportional to the total atom number $N$, and can be neglected in the absence of coherent atom number fluctuations.
...
\begin{align}
  H_{\t{int}} = -\f{U}{N} \sum_{p,q} \v s_p\c\v s_q
  = -\f{U}{N}\v S\c\v S,
  &&
...
The operators $\v s_p\c\v s_q$ in Eq.~\eqref{eq:H_int} simply swap the nuclear spin states of two atoms pinned to modes $p,q$.
...
It is in this sense that $\v s_p\c\v s_q$ is analogous to spin alignment in the case of SU(2), which assigns distinct definite energies to anti-symmetric spin-0 singlet $\ket{\up\dn}-\ket{\dn\up}$ and the symmetric spin-1 triplets $\set{\ket{\up\up},\ket{\dn\dn},\ket{\up\dn}+\ket{\dn\up}}$.

By summing over all pair-wise exchange terms $\v s_p\c\v s_q$, the interaction Hamiltonian $H_{\t{int}}$ energetically enforces a permutational symmetry among all spins, opening an energy gap $U$ between the (degenerate) manifold of all permutationally-symmetric (PS) states and the orthogonal complement of excited (e.g.~spin-wave) states that break permutational symmetry.
...
More generally, the state $\ket\psi$ of a $n$-level spin can be represented by a probability distribution\footnote{Strictly speaking, in order to make $Q_\psi$ and $Q_\rho$ probability distributions one has to divide them by a normalization factor $\frac{4\pi}{d}$.
We omit this normalization factor for convenience.} $Q_\psi$ on the sphere $\SS^2$, with the value $Q_\psi\p{\v v}$ at a point $\v v\in\SS^2$ equal to the overlap of $\ket\psi$ with a pure state $\ket{\v v}$ polarized along $\v v$: $Q_\psi\equiv\abs{\bk{\v v|\psi}}^2$ (see Figure \ref{fig:spin_dist}).
In the case of a mixed state $\rho$, this distribution is defined by $Q_\rho\equiv\bk{\v v|\rho|\v v}$.
The dimension $n$ is reflected in $Q_\rho$ by the fact that it can be decomposed into spherical harmonics $Y_{\ell m}$ with degree $\ell<n$ (that is, $Q_\rho$ is ``low resolution'' or ``band-limited'' at degree $n$) \cite{perlin2020qudit}, as well as in the normalization $\int_{\v v\in\SS^2}\dd^2\v v\,Q_\rho\p{\v v}=\frac{4\pi}{d}$ (independent of $\rho$).
...
The Hilbert space of a 10-level (spin-$\frac92$) nucleus of a ${}^{87}$Sr atom, for example, can be thought of as the Dicke manifold of 9 ``excess'' spins of its underlying protons and neutrons.
This representation of a $n$-level spin is a useful picture to keep in mind, and will be used freely throughout this work.
...
In order to ensure validity of the spin Hamiltonian $H_{\t{int}}$ in Eq.~\eqref{eq:H_int}, we consider initial preparation of a spin-polarized state, which has the added benefit of being simple to prepare experimentally.
However, a spin polarized state is an eigenstate of $H_{\t{int}}$, so we need additional ingredients to induce nontrivial behavior.
Here, we consider the effect of spin-orbit coupling (SOC) induced by external driving fields.

Before discussing SOC in the context $n$-level fermions on a lattice, we first consider the well-studied case of SOC for two-level atoms in one spatial dimension \cite{wall2016synthetic, bromley2018dynamics}.
...
The drive in Eq.~\eqref{eq:drive_2} is typically implemented by coupling the electronic states of an atom with an external laser \cite{wall2016synthetic, bromley2018dynamics}.
Whereas the coupling between laser fields and nuclear spins is too weak to drive nuclear spin transitions directly, it will nonetheless be useful to consider the the $n$-level (spin-$s$) generalization of Eq.~\eqref{eq:drive_2}.
This generalization will capture the important features of SOC for spin-$s$ nuclei, while avoiding complications that we defer to Section \ref{sec:controls}.
...
An identical gauge gauge transformation $c_{q\mu}^\dag\to c_{q-\mu\phi,\mu}^\dag$ again takes $H_{\t{drive}}^{(\phi)}\to \Omega S_\x$ as in Eq.~\eqref{eq:drive_trans}, where now
\begin{align}
  \Omega = \f{\Omega_0}{\xi},
  &&
...
\begin{align}
  ...
  \label{eq:spin_x_op}
\end{align}
is a $n$-level spin-$x$ operator for spin $q$, such that $e^{-\i\theta s_\x}$ rotates the state of spin $q$ about the $x$ axis by an angle $\theta$.
...
\caption{
Spin-orbit coupling for dimension-2 (spin-$\frac12$; a,c) and dimension-4 (spin-$\frac32$; b,d) spins.
Colors indicate different nuclear spin projections $\mu$.
In the ``lab frame'' (a,b), kinetic energy is insensitive to spin, but the drive imparts a momentum kick $q\to q+\phi$ into any atom that absorbs a photon.
Changing to the ``gauged frame'' (c,d) makes the drive diagonal in momentum, but comes at the cost of making kinetic energy spin-dependent.
}
\label{fig:soc_panels}
\end{figure}

Spin-orbit coupling cannot be ``gauged away'' entirely, of course.
...
where
\begin{align}
  w_{+,q}^{(\phi)} \equiv \cos\p{\mu\phi} s_{\mu\mu q},
  &&
  w_{-,q}^{(\phi)} \equiv \sin\p{\mu\phi} s_{\mu\mu q}
\end{align}
...
\vspace{3cm}
\red{todo: discuss changes to the interaction Hamiltonian}
\begin{align}
  H_{\t{int}} \to H_{\t{int}}^{(\phi)}
  \equiv -\f{U}{N} \sum_{p,q,\mu,\nu}
  c_{p-\mu\phi,\mu}^\dag c_{p-\nu\phi,\nu}
  c_{q-\nu\phi,\nu}^\dag c_{q-\mu\phi,\mu}
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{External controls, observables, and initial states}
\label{sec:controls}

We now take a closer look at external control fields, which determine the initial states we can prepare and the observables we have access to.
...
Furthermore, laser $\v v$ will generally imprint a phase $e^{-\i\kappa\v v\c\v\ell j}$ on lattice site $j$, where $\kappa$ is the laser's wavenumber (in units with lattice spacing $a=1$) and $\v\ell$ is a fixed unit vector parallel to the lattice.
...
The spin-raising operator $s_{+,j}$ is then twice the spin-raising part of $s_{\x,j}$, just as in the case of SU(2) where $s_\x=\frac12\op{\up}{\dn}+\t{h.c.}$, and $s_{-,j}\equiv s_{+,j}^\dag$.
The operator $s_{\v v,+\,j}$ that raises spin along $\v v$ is acquired by a rotation of $s_{+,j}$ that maps the $z$ axis to $\v v$:
\begin{align}
  ...
  \label{eq:spin_rot}
\end{align}
where we have dropped the site index $j$ for brevity, and $s_\y\equiv-\frac\i2s_++\t{h.c.}$.
...
Choosing $\v v_+$ as the spin quantization axis, we can define new drive amplitudes $\Omega_0\equiv-(\Omega_{\v v_0,+}+\Omega_{\v v_0,-})$ and $\Omega_\pm\equiv\pm\Omega_{\v v_\pm,+}$, as well as an SOC angle $\phi\equiv\kappa\v v_+\c\v\ell=\kappa\cos\theta$, in terms of which the drive Hamiltonian becomes
\begin{multline}
  H_{\t{3LD}}^{\t{full}}
  = \sum_{j,m} \Omega_m
...
In the far-detuned limit $\Delta\gg\abs{\Omega_m}$, a second-order perturbative treatment of ${^3}\t{P}_0$ excitations ($\ket\e$) yields an effective drive Hamiltonian that only addresses ground-state nuclear spins.
...
Finally, we comment on initial states that these external controls allow us to prepare.



\red{comment about initial states (``intra-spin correlations'')}

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{A simple, realistic model}
