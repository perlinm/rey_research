...
We show that in an appropriate parameter regime, this system can be mapped onto a multilevel Heisenberg spin model with all-to-all SU($n$)-symmetric couplings.
The use of Raman pulses to address internal spin states modifies the atomic dispersion relation and induces spin-orbit coupling, which can act as a synthetic inhomogeneous magnetic field that competes with the SU($n$) exchange interactions.
We investigate the non-equilibrium mean-field dynamical phases of the resulting model as a function of $n$ and different initial configurations that are accessible with the Raman pulses.
In agreement with previous studies with $n=2$, we find that for some initial states the spin model exhibits two distinct dynamical phases that obey simple scaling relations with $n$.
When $n>2$, we find that dynamical behavior can be highly sensitive to initial intra-spin coherences.
...
In the simplest example, this symmetry arises through the independence of atomic orbital and interaction parameters on the $n$ nuclear spin states of alkaline-earth(-like) atoms, with e.g.~$n=10$ in the case of ${}^{87}$Sr \cite{wu2003exact, cazalilla2009ultracold, cazalilla2014ultracold}.
As a result, experiments can directly probe the role SU($n$) interactions in controllable settings.
...
In the spirit of quantum simulation, further investigations in controlled settings will play an important role in understanding SU($n$) symmetries' consequences for fundamental questions in physics, as well as their practical use in technological applications.
...
The prospect of similarly exploiting larger SU($n$) symmetries to achieve a technological advantage is still an unexplored avenue of research with untapped potential.
...
We take a look at control fields to address nuclear spins (and generate SOC) in Section \ref{sec:controls}, finding a simple three-laser driving scheme that allows for the preparation of interesting states with nontrivial intra-spin correlations when $n>2$.
Putting all of these ingredients together, we examine mean-field dynamical behaviors of the SU($n$) spin model in Section \ref{sec:mean_field}, finding that
\begin{enumerate*}
\item long-time-averaged observables obey simple scaling relations with the spin dimension $n$, exhibiting (for spin-polarized initial states) a magnetized and a de-magnetized dynamical phase, as previously seen for the case of $n=2$ \cite{smale2019observation, lewis-swan2021cavityqed}, and
\item when $n>2$ long-time dynamics can be highly sensitive to the intra-spin coherences of the initial state.
\end{enumerate*}
We conclude and discuss future outlooks in Section \ref{sec:conclusions}.
...
Before discussing SOC for $n$-level fermions, we first consider the well-studied case of two-level SOC on a one-dimensional lattice \cite{wall2016synthetic, kolkowitz2016spinorbitcoupled, bromley2018dynamics, he2019engineering}.
...
The permutation $\Pi_\z$ commutes with $S_\y$, while the rotation $R_\x^\pi$ takes $S_\y\to-S_\y$, so
\begin{align}
  \bk{S_\y}
...
\end{align}
at all times.
In turn, conservation of $\bk{S_\y}=\bk{S_\z}=0$ and permutational symmetry together imply that the magnetization $\sigma_\MF$ is determined by the time-average of $\sigma_\x\equiv s_\x/s$ for a single spin:
\begin{align}
...
Taking a closer look at the external controls that induce SOC, we found a simple three-laser drive that enables homogeneously addressing nuclear spins with a variety of spin Hamiltonians, enabling for example the preparation of ``multi-cat'' states in which each spin is in a coherent superposition of states spin-polarized in opposite directions.
...
We hope this work provides a starting point for simulating SYK-like models in cold atomic platforms.
In follow-up work, it would be interesting to study the the relationship between initial states and dynamical phases of our SU($n$) spin model more systematically, or to consider the effect of quantum corrections to mean-field behavior.
There is also room to improve on the three-laser drive we described in this work, where it is natural to ask what ingredients and techniques are necessary to implement universal control of individual nuclear spins.
...
Unlike our present work, the super-exchange regime does not require weak SOC, and therefore has a larger parameter space in which to explore dynamical behaviors.
