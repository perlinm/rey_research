...
Closely related spherical representations of multilevel spin operators are discussed in Ref.~\cite{li2013weylwignermoyal}.
...
We now return to the spin Hamiltonian $H_{\t{spin}}$ in Eq.~\eqref{eq:H_spin}, which is valid in the weak-SOC limit $s\phi\ll1$, and henceforth work exclusively in the ``gauged frame'' of this $H_{\t{spin}}$ and the three-laser drive $H_{\t{3LD}}$ in Eq.~\eqref{eq:drive_all}.
At the mean-field (MF) level, the spin Hamiltonian becomes
\begin{align}
...
\begin{figure}
\centering
\includegraphics{mean_op_ss_X.pdf}
\caption{
Long-time averages of $\bar\sigma_\x$ and $\ss$ for different spin dimensions $n$ (indicated in the legend) as determined by mean-field simulations of $N=100$ spins initially in the $x$-polarized state $\ket\x^{\otimes N}$.
Insets show same data with a rescaling $J\phi/U\to J\phi/U \times (n/2)^{1/3}$, $\bbk{\bar\sigma_x}_\MF\to\bbk{\bar\sigma_x}_\MF/\gamma\p{n/2}$, and $\bbk{\ss}_\MF\to\bbk{\ss}_\MF/\gamma\p{n}-1$, where $\gamma\p{k}$ is defined in Eq.~\eqref{eq:gamma}.
}
...
The ordered phase has a non-zero transverse magnetization $\bbk{\sigma_\x}_\MF$ and a large interaction energy $\bbk{\ss}_\MF$, while the disordered phase has no transverse magnetization and a minimal interaction energy.
...
As shown in insets of Figure \ref{fig:mean_op_ss_X}, mean-field results for different spin dimensions $n$ collapse onto each other when normalizing the field $J\phi/U$ to its critical value, $\p{J\phi/U}\to\p{J\phi/U}\times\p{n/2}^{1/3}$, and rescaling $\bbk{\bar\sigma_x}_\MF\to\bbk{\bar\sigma_x}_\MF/\gamma\p{n/2}$ and $\bbk{\ss}_\MF\to\bbk{\ss}_\MF/\gamma\p{n}-1$, where
\begin{align}
...
\begin{align}
  ...
  = \Tr\sp{\p{\diag\op\x}^2}
  = \gamma\p{n} \approx \f1{\sqrt{2\pi s}}.
\end{align}
The same result can be obtained by considering the interaction energy of two counter-rotating spins, whose long-time average is essentially the time-averaged overlap of their phase-space distributions (see Figure \ref{fig:spin_dist}).
...
At second order in perturbation theory, the spin Hamiltonian thus becomes
\begin{align}
  H_{\t{spin}}^{\t{eff}}
  = \f{\p{J\phi}^2}{\p{N-1}U}
  \times \sp{S_\z^2 - N\sum_q s_{\z,q}^2}.
\end{align}
However, mean-field theory fails to capture inter-spin correlations that accumulate throughout evolution under $H_{\t{spin}}^{\t{eff}}$.
For example, $\ss$ is constant (equal to 1) in the PS manifold, but $\lim_{J\phi/U\to0^+}\bbk{\ss}_\MF<1$ when $n>2$ in Figure \ref{fig:mean_op_ss_X}.
Treating $H_{\t{spin}}^{\t{eff}}$ at the mean-field level would also yield a homogeneous Hamiltonian that conserves $\ss$, which is again inconsistent with the weak-field limits of $\bbk{\ss}_\MF$.
This inconsistency between mean-field theories stems from the fact that applying perturbation theory does not commute with a mean-field factorization of spin operators.
In order to reproduce the weak-field asymptotic values of $\bbk{\bar\sigma_\x}_\MF$ and $\bbk{\ss}_\MF$, therefore, we must instead consider the mean-field Hamiltonian $H_\MF$ in Eq.~\eqref{eq:H_MF}.
