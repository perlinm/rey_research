...
\caption{
Time-averaged magnetization $m_\MF$ and interaction energy $\bbk{\ss}_\MF$ for different spin dimensions $n$ (indicated in the legend) as determined by mean-field simulations of $N=100$ spins initially in the $x$-polarized state $\ket\x^{\otimes N}$ for a time $T=10^5/U$.
...
\end{align}
where we have used the fact that the axial magnetizations $\bk{s_{\z,q}}=\frac1N\bk{S_\z}$ within the PS manifold, and $\bk{S_\z}=0$ is conserved by $H_{\t{spin}}^{\t{eff}}$.
The weak-field effective Hamiltonian preserves permutational symmetry, so $\bbk{\ss}_\MF\to1$ as $J\phi/U\to0^+$.
Moreover, permutational symmetry together with the fact that $\bk{S_\z}=\bk{S_\y}=0$ are conserved\footnote{Both the initial state $\ket\x^{\otimes N}$ and the spin Hamiltonian $H_{\t{spin}}$ are invariant under the action of $U_\x=e^{-\i\pi S_\x}\sum_{q>0}\v s_q\c\v s_{-q}$, so $\bk{S_\y}=\bk{U_\x^\dag S_\y U_\x}=-\bk{S_\y}=0$ at all times.} implies that the magnetization $m_\MF$ is determined by the time-average of $\sigma_\x\equiv s_\x/s$ for a single spin:
\begin{align}
...
We now consider the same setup as above, but with the initial ``multi-cat'' states $\ket{\x\x}^{\otimes N}$ and $\ket{\x\x_\i}^{\otimes N}$, where
\begin{align}
  \ket{\x\x} &\equiv \f{\ket{\x} + \ket{-\x}}{\sqrt{2}}
  \simeq e^{-\i\frac{\pi}{2}s_\y^2} \ket{-s}, \\
  \ket{\x\x_\i} &\equiv \f{\ket{\x} + \p{-1}^s \ket{-\x}}{\sqrt{2}}
  \simeq e^{-\i\frac{\pi}{2}s_\x} \ket{\x\x}.
\end{align}
Here $\ket{-\x}\equiv e^{-\i\frac{\pi}{2}s_\y}\ket{-s}$, and $\simeq$ denotes equality up to a global phase.
