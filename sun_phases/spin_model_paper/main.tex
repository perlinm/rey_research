...
Recent progress includes the experimental observations of bosonization and faster thermalization in multi-component Fermi gas with SU($n$) symmetry \cite{hazzard2012hightemperature, messio2012entropy, bonnes2012adiabatic, stellmer2013degenerate, yip2014theory, pagano2014onedimensional, choudhury2020collective, song2020evidence, sonderhouse2020thermodynamics}, SU($n$) Hubbard phases and phase transitions \cite{taie2012su, hofrichter2016direct, taie2020observation}, two-orbital SU($n$) magnetism \cite{cappellini2014direct, scazza2014observation, zhang2014spectroscopic, beverland2016realizing}, and multi-body SU($n$)-symmetric interactions \cite{goban2018emergence, perlin2019effective}.
...
We will freely use this correspondence to import standard results and constructions for the Dicke manifold (e.g.~the spin operators $\s_\x,\s_\y,\s_\z$) into the context of a multilevel spin.
...
Changing to the ``gauge frame'' ({\bf c},{\bf d}), essentially by shifting the momentum label $q$ for each spin state $\mu$, makes the drive diagonal in the momentum index, but comes at the cost of making kinetic energy spin-dependent.
}
...
The general idea will be a Ramsey-type setup in which we first prepare an initial state with the three-laser drive (using fast pule sequences), let this state evolve freely for some time under $\H_\spin$, and then again use the three-laser drive to map observables of interest onto spin projection measurements (e.g.~with spin qudit tomography \cite{newton1968measurability, perlin2020qudit}).
...
See Appendices \ref{sec:MFT} and \ref{sec:bosons} additional details about our numerical simulations and the Schwinger boson equations of motion.
...
Moreover, the initial y-magnetization $\bk{\S_\y}=0$ is conserved by $\H_\spin$, so long-time-averaged magnetization $\sigma_\MF$ is determined by the time-average of $\hat\sigma_\x\equiv\s_\x/s$ for a single spin:
\begin{align}
  \lim_{h\to0} \sigma_\MF
...
\begin{align}
  ...
  = \gamma\p{\f{n}{2}}.
\end{align}
When going beyond mean-field theory, inter-spin correlations generated by $\S_\z^2$ in Eq.~\eqref{eq:H_spin_eff} will cause $\bk{\S_\x}$ (and thereby the magnetization $\bk{\vec\sigma}$) to decay on time scales that diverge as $N\to\infty$.
...
Taking a closer look at the effect of the spin-orbit coupling (SOC) induced by the driving lasers, we found that maintaining integrity of the spin model requires weak SOC, which in turn gives rise to a (synthetic) inhomogeneous magnetic field.
...
\end{enumerate*}
If the excited-state hyperfine manifold has total spin $s+r$ (with $r\in\set{0,+1,-1}$), then the spin-raising/lowering operators $\s_\pm$ that appear in Eq.~\eqref{eq:H_drive_full} (where $\s_{\v v,\pm,j}$ raises or lowers the spin of atom $j$ along axis $\v v$) become
\begin{align}
  \s_\pm^{(r)}
