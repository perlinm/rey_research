...
Nonetheless, for sufficiently weak SOC (small $\phi$) the effect of $H_{\t{int}}^{(\phi)}$ may be approximated by $H_{\t{int}}^{(0)}=H_{\t{int}}$, which has previously been benchmarked for SU(2)-symmetric interactions \cite{he2019engineering, smale2019observation}.
...
We now return to the spin Hamiltonian $H_{\t{spin}}$ in Eq.~\eqref{eq:H_spin}, and henceforth work exclusively in the ``gauged frame'' of this Hamiltonian and the three-laser drive $H_{\t{3LD}}$ in Eq.~\eqref{eq:drive_all}.
At a fixed atom number $N$, the spin Hamiltonian $H_{\t{spin}}$ essentially has one free parameter: the ratio $J\phi/U$ that determines the relative strength of the single-particle and interaction terms.
On a high level, for an initial $N$-fold tensor product state $\ket\psi^{\otimes N}$ one should expect distinct dynamical behaviors when $J\phi/U\ll1$, in which case spin-locking through strong $\v S\c\v S$ interactions restricts dynamics to the permutationally symmetric manifold, as opposed to $J\phi/U\gg1$, in which case individual spins should dephase due to the inhomogeneous magnetic field.
To investigate these behaviors quantitatively at the mean-field level, we first decompose individual spin operators into Schwinger bosons as $s_{\mu\nu q} = b_{\mu q}^\dag b_{\nu q}$, such that the spin Hamiltonian $H_{\t{spin}}$ in Eq.~\eqref{eq:H_spin} becomes
\begin{align}
  H_{\t{boson}} = -\f{U}{2N} \sum_{p,q,\mu,\nu}
  b_{\mu p}^\dag b_{\nu p} b_{\nu q}^\dag b_{\mu q}
  + J\phi\sum_{q,\mu} \mu\, b_{\mu q}^\dag b_{\mu q}.
\end{align}
...
\begin{align}
  ...
  + \mu\, b_{\mu q}.
\end{align}
Our mean-field theory (MFT) then treats all boson operators in these equations of motion as complex numbers, $b_{\mu q}\to\bk{b_{\mu q}}_\MFT$, with the initial value $\bk{b_{\mu q}\p{t=0}}_\MFT$ equal to the initial amplitude of spin $q$ in state $\mu$.
Specifically, we will consider initial $N$-fold product states $\ket\psi^{\otimes N}$ with $\ket\psi = \sum_\mu \psi_\mu \ket\mu$, for which we set all $\bk{b_{\mu q}\p{t=0}}_\MFT = \psi_\mu$.
For pure initial product states, this mean-field treatment of the boson operators $b_{\mu q}$ is mathematically equivalent to a mean-field treatment of the spin operators $s_{\mu\nu q}$, but reduces the number of variables to keep track of by a factor of $\sim n$.

\vspace{1cm}

\red{[Text that will depend on the figures]}

\vspace{1cm}

To identify dynamical phases, we examine long-time-averaged observables of the form
\begin{align}
  \bbk{\O}_\MFT = \lim_{T\to\infty} \f1T \int_0^T \dd t\, \bk{\O\p{t}}_\MFT,
\end{align}
where $\bk{\O\p{t}}_\MFT$ is the mean-field value of $\O$ at time $t$.
Specifically, we consider the observables
\begin{align}
  \bar\sigma_\x \equiv \f{S_\x}{\norm{S_\x}},
  &&
  \ss = \f{\v S}{N}\c\f{\v S}{N},
\end{align}
where $S_\x=\sum_q s_{\x,q}$ is a collective spin-$x$ operator with operator norm $\norm{S_\x}=Ns$, and $\bar{\v s}\equiv\frac1N\sum_q\v s_q$ is the average spin matrix (related to the average single-particle density operator $\bar\rho$ by $\bar{\v s} = \bar\rho^\dag$).
By design, these observables to have norm $\norm{\bar\sigma_\x}=\norm{\ss}=1$ independent of the system size $N$ or spin dimension $n$.

\vspace{1cm}

\red{[Discussion of figures]}

\vspace{1cm}

\begin{figure}
\centering
\includegraphics{mean_x_ss.pdf}
\caption{
Long-time averages of $\bar\sigma_\x$ and $\ss$ for different spin dimensions $n$ (indicated in the legend) as determined by mean-field simulations of $N=100$ spins up to a maximal time $T=10^4/U$.
}
\label{fig:mean_x_ss}
\end{figure}
...
\caption{
The critical value of $\p{J\phi/U}_{\t{crit}}$ as determined by mean-field simulations of $N=100$ spins, and a single-parameter fit to $\p{J\phi/U}_{\t{crit}}=\p{n/2}^\alpha$, which finds $\alpha=-0.333(4)$.
The fit agrees with all mean-field results to within an uncertainty determined by the resolution of $J\phi/U$ in mean-field simulations.
}
