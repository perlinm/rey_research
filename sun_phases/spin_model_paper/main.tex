...
\begin{align}
  \H_{\t{int}}^{(\phi)}
  \equiv \f{u}{2N} \sum_{k,\ell,p,q,\mu,\nu}
  \c_{k\mu}^{(\phi)\dag} \c_{\ell\mu}^{(\phi)}
  \c_{p\nu}^{(\phi)\dag} \c_{q\nu}^{(\phi)}
  \times \delta_{k+p,\ell+q},
\end{align}
and $\c_{k\mu}^{(\phi)}\equiv\c_{k-\mu\phi,\mu}$.
However, $\H_{\t{int}}^{(\phi)}$ cannot be reduced to a spin Hamiltonian that addresses the same spin degrees of freedom as $\H_{\t{kin}}^{(\phi)}$.
Again making a frozen-mode approximation, or equivalently keeping only terms that preserve the coherences of an initially spin-polarized state, the gauge-transformed interaction Hamiltonian becomes
\begin{align}
  \H_{\t{int}}^{(\phi)}
  \equiv -\f{u}{2N} \sum_{p,q,\mu,\nu}
  \c_{p\mu}^{(\phi)\dag} \c_{p\nu}^{(\phi)}
  \c_{q\nu}^{(\phi)\dag} \c_{q\mu}^{(\phi)}.
\end{align}
However, $\H_{\t{int}}^{(\phi)}$ cannot be reduced to a spin Hamiltonian that addresses the same spin degrees of freedom as $\H_{\t{kin}}^{(\phi)}$.
Nonetheless, for sufficiently weak SOC (small $s\phi$) the effect of $\H_{\t{int}}^{(\phi)}$ may be approximated by $\H_{\t{int}}^{(0)}$, and in turn $\H_{\t{int}}$ in Eq.~\eqref{eq:H_int_start}.
The validity of this approximation has been previously benchmarked for SU(2)-symmetric interactions \cite{he2019engineering, smale2019observation}, and we provide additional benchmarking for SU(4) and SU(6) in Appendix \ref{sec:benchmarking}.
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{benchmarking_SU4.pdf}
\caption{
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{benchmarking_SU6.pdf}
\caption{
