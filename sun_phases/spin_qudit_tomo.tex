...
This complexity of this task can be greatly reduced by the fact that the degree $\ell$ of the transition operator $T_{\ell,0}$ is preserved under rotations, which implies that the unitary
\begin{align}
...
Constructing the block $M_{V\ell}$ and computing its singular values takes at most $O(\abs{V}d^2)$ time (see Appendix \ref{sec:compute} for additional details on efficiently computing $\S_{V\ell}$ [\red{todo: write tutorial}]).
Altogether, if we assume that $\abs{V}\sim d$, then computing $\S_V^2$ to certify the bound in Eq.~\eqref{eq:bound_eps} takes $O(d^4)$ serial or $O(d^3)$ parallel runtime (see Figure [\red{REF}]).
...
Therein, we find that if measurement error is dominated by shot noise, then the estimate $\tilde\rho_V$ built from $n$ independent measurements of spin projection along every axis $\v v\in V$ approximates the true state $\rho$ to within a mean squared distance
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  \le \epsilon_V^2
  \equiv \f1{nd} \sum_{\ell>0} \p{2\ell+1} \gamma_\ell^2 \S_{V\ell},
  \label{eq:bound}
\end{align}
where
\begin{align}
...
\end{align}
is defined in terms of the Clebsch-Gordan coefficients
\begin{align}
  \tau_{\ell\mu} \equiv \bk{s\mu;\ell,0|s\mu}.
\end{align}
For reference, we note that $\epsilon_V^2<\S_V^2/n$.
...
The reconstruction error bound in Eq.~\eqref{eq:bound_eps} provides a straightforward answer: the number of measurement axes should be chosen to minimize the product $\epsilon^2\S_V^2$, which bounds the expected error in the estimate $\tilde\rho_V$ of an unknown state $\rho$.
...
\end{align}
We now seek to maximize $g_\ell\p{p}$, which will provide an upper bound for $\bbk{\epsilon_{i\ell}^2}$.
In general, variance $\sigma_p^2\p{X}$ is maximal when $p$ has equal weight on the largest and smallest values of $X$, which implies that
\begin{align}
...
However, the bound on $\gamma_\ell^2\le1$ is not tight in general, as we could have alternately bounded
\begin{align}
