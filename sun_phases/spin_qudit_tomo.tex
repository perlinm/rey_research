...
\begin{abstract}
  ...
  We conjecture that $r_d^{\t{min}}=2d-1$, which have verified for all $d\le200$ using numerical experiments.
  We then provide a parallelized algorithm with $O\p{r^3}$ runtime for certifying an upper bound on the error with which measurements of spin projection onto $r$ quantization axes can reconstruct an unknown quantum state.
...
Nonetheless, the tomography protocol in the next section can be used to numerically verify that $2d-1$ measurement axes suffice for all $d\le200$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Reconstruction error bounds}
\label{sec:protocol}
...
Altogether, diagonalizing each of $d$ blocks of $G_V$ takes $O(\abs{V}^3)$ time, so the scale $\M_V$ can be computed in $O(d\abs{V}^3)$ serial or $O(\abs{V}^3)$ parallel runtime.
...
If the experimental measurements $E_j$ of $\bk{T_j}_\rho$ are accurate to within an error $\lesssim\epsilon$, then the state $\tilde\rho$ defined in Eq.~\eqref{eq:reconstructed_state} is an approximation of the true state $\rho$ to within a 2-norm distance of $\lesssim\epsilon\M_V$.
...
As a last point, we mention that the cost function $\M_V$ can also evaluate the quality of measurement axes chosen with existing methods for sparse signal recovery on the sphere \cite{rauhut2011sparse, alem2012sparse}.
...
We conjecture, and numerically verify for $d\le200$, that measurements of spin projection along $2d-1$ axes suffice to reconstruct an arbitrary state of a $d$-level spin qudit [\red{todo: make figure of $1/\M_V$ (minimized over, say, 100 random choices of measurement axes) as a function of the measurement axis number, for different values of $d$}].
...
At a fixed total number of experimental measurements, having to estimate more observables means that fewer measurements are devoted to estimating any particular observable, which in turn increases the error $\epsilon$ on those estimates.
This trade-off begs the question: how many measurement axes should be chosen to perform spin qudit tomography?

The reconstruction error bound in Eq.~\eqref{eq:bound} provides a straightforward answer: the number of measurement axes should be chosen to minimize the product $\epsilon^2\M_V^2$, which bounds the expected error in the estimate $\tilde\rho$ of an unknown state $\rho$.
...
The variance in these estimates is then $\epsilon^2\sim 1/(N/r)=r/N$, or simply $\epsilon^2\sim r$ at a fixed total number of measurements (assuming all measurements are independent).
Altogether, the number of measurement axes $r$ should be chosen in such a way as to minimize $r\M_V^2$.
