...
Due to advancements in experimental capabilities to address nuclear spin states (hyperfine levels) in ultracold atomic systems \cite{daley2011quantum, mischuck2012control, smith2013quantum, cazalilla2014ultracold}, as well as developments in the control of ultracold molecular systems \cite{bohn2017cold, zeppenfeld2012sisyphus, kozyryev2017sisyphus, puri2017synthesis, wu2017cryofuge, marco2019degenerate, liu2019molecular, anderegg2019optical, chou2020frequencycomb, lin2020quantum}, a particular setting of growing interest is the spin qudit, or a multilevel quantum angular momentum degree of freedom.
...
The actual shot-noise-limited error in the reconstruction $\tilde\rho_V$ of a particular state $\rho$ may be considerably smaller, and depends on $\rho$ itself.
...
The matrix $\N_{V\ell}$ is defined in terms of the measurement matrix $M_{V\ell}$ and its left inverse $M_{V\ell}^{-1}$ by
\begin{align}
...
\item Select a random set of measurement axes $V$ by uniformly sampling points on the sphere \footnote{To sample a point $\p{\alpha,\beta}$ from the uniform distribution on the sphere (with azimuthal angle $\alpha$ and polar angle $\beta$), you can sample a point $\p{a,b}\in[0,1]\times[0,1]$ from the uniform distribution on the unit square, and then set $\alpha=2\pi a$ and $\beta=\arccos\p{1-2b}$.}, and use any standard minimization algorithm to optimize the $2\abs{V}$ parameters in $V$ (two angles for each point $\v v\in V$) by minimizing the quantum error scale $\epsilon_V$ in Eq.~\eqref{eq:bound}.
  ...
  Save all measurement matrix data associated with the final measurement axes $V$ for later use. % todo: cite codes
...
\item Denoting the nonzero singular values of $M_{V\ell}$ by $M^V_{\ell k}$ and the corresponding left singular vectors by $\v x^V_{\ell k} = \sum_{\v v} x^V_{\ell k \v v} \ket{\v v}$, compute the  operators and coefficients
  ...
  \end{align}
\end{enumerate}
If reconstruction error is dominated by shot noise, then the total error in $\tilde\rho_V$, or its root-mean-square distance from $\rho$, is approximately $\E_V\p{\tilde\rho_V}\approx\E_V\p{\rho}$, which can be computed from Eq.~\eqref{eq:error}. % todo: cite codes
...
% Yb (5/2), Cr (3), Sr (9/2), Dy (15/2?), Er (19/2)
