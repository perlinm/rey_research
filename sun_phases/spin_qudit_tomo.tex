...
Due to advancements in experimental capabilities to address nuclear spins in ultracold atomic systems \cite{cazalilla2014ultracold} [\red{more references?}], as well as developments in the control of ultracold molecular systems \cite{zeppenfeld2012sisyphus, kozyryev2017sisyphus, puri2017synthesis, wu2017cryofuge, marco2019degenerate, liu2019molecular, anderegg2019optical, chou2020frequencycomb, lin2020quantum, covey2018approach, jamadagni2019quantum}, a particular setting of growing interest is the spin qudit, or a multilevel quantum angular momentum degree of freedom.
Spin qudits can provide advantages over their qubit counterparts for quantum sensing \cite{evrard2019enhanced}, enable quantum simulations of SU($d$) magnetism \cite{cazalilla2014ultracold, banerjee2013atomic, zhang2014spectroscopic, scazza2014observation, goban2018emergence}, and offer unique capabilities for quantum error correction \cite{albert2020robust, gross2020encoding}.
...
We will also frequently consider polarization operators with order $m=0$.
Symbols with a suppressed order therefore implicitly set this order to zero, which is to say that $T_\ell\equiv T_{\ell,0}$, $T_{\v v\ell}\equiv T_{\v v\ell,0}$, and $D^\ell_m\equiv D^\ell_{m,0}$.
...
The transformation rules for polarization operators in Eq.~\eqref{eq:trans_rot}, together with the fact that $\bk{s|T_{\ell m}|s}=0$ unless $m=0$, suffice to show that their phase-space distributions are
\begin{align}
  \tilde T_{\ell m}\p{\v v} = c_\ell Y_{\ell m}\p{\v v}^*,
\end{align}
where the proportionality factor $c_\ell$ simply enforces that $\obk{T_{\ell m}|T_{\ell m}}=1$ (see Appendix \ref{sec:rotations}).
...
For any fixed axis $\v v$, the sets $\set{\Pi_{\v v\mu}}$ and $\set{T_{\v v\ell}}$ are both complete bases for the space of operators that are diagonal in the basis $\set{\ket{\mu_{\v v}}}$.
Measuring the projectors $\set{\Pi_{\v v\mu}}$ is therefore equivalent to measuring the polarization operators $\set{T_{\v v\ell}}$, and provides data on the expectation values $\bk{T_{\v v\ell}}_\rho$.

In order to reconstruct an arbitrary density operator $\rho$ from the expectation values $\bk{T_{\v v\ell}}_\rho$, we essentially need to to find a set of coefficients $C_{\ell mk}\p{\v v}$ that would allow us to recover any matrix element $\rho_{\ell m}$ of $\rho$ through
\begin{align}
...
Flattening each $d\times d$ matrix $T_{\v v\ell}$ into the $d^2$-component column vector $\oket{T_{\v v\ell}}$, we construct the {\it measurement matrix}
\begin{align}
...
\begin{align}
  \tilde T_{\v v\ell}
  \equiv \sum_\mu \bk{\mu|T_\ell|\mu} \tilde\Pi_{\v v\mu}
\end{align}
of $\bk{T_{\v v\ell}}_\rho$, which can in turn be used to construct the estimate $\tilde\rho_V$ of $\rho$.
...
\begin{align}
  ...
  x^V_{\ell k\v v} \, \tilde T_{\v v\ell},
\end{align}
of
\begin{align}
...
To this end, we define the probability $p^i_\mu\equiv\bk{\Pi_{i\mu}}_\rho$, collect these probabilities into the classical probability distribution $p^i = \sum_\mu p^i_\mu \ket{\mu}$, and define the vector $t_\ell \equiv \sum_\mu t_{\ell\mu} \ket{\mu}$ of all $t_{\ell\mu}\equiv\bk{\mu|T_\ell|\mu}$.
