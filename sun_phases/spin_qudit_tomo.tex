...
\begin{abstract}
  ...
  We map this quantum tomography problem onto the classical problem of signal recovery on the sphere, thereby proving that full reconstruction of arbitrary mixed states is possible and requires a minimum number of measurement axes, $r_d$, that is bounded by $2d-1\le r_d\le d^2$.
  Our mapping comes with a straightforward prescription for converting existing signal recovery algorithms into spin qudit tomography protocols, and numerical experiments show that $r_d=2d-1$ for $d\le20$.
