...
We also note that the (squared) reconstruction error in Eq.~\eqref{eq:error} and its bound in Eq.~\eqref{eq:bound} obey the ``standard quantum limit'' of $\sim1/\sqrt{n}$ scaling in the number of measurements.
In principle, this scaling can be improved to $\sim1/n$ by preparing and measuring entangled copies of many qudits \cite{giovannetti2006quantum}.
...
The remainder of this section is devoted to defining $\epsilon_V^2$, and can be skipped without consequence for the discussions in the rest of this paper.
To write out the quantum error scale $\epsilon_V^2$ explicitly, we define a matrix of structure constants
\begin{align}
  g_{L\ell} \equiv \sum_{m,n} g^L_{\ell mn} \op{m}{n},
  &&
  g^L_{\ell mn} \equiv \obk{T_{L,m+n}|T_{\ell m}T_{\ell n}},
\end{align}
and the inversion operator $\I\equiv\sum_m\p{-1}^m\op{m}{m}$, which define components of the $d^2$-component vector
\begin{align}
  \ket{\chi_V}
  &\equiv \sum_{L=0}^{d-1} \sum_{M=-L}^L \chi^V_{LM} \ket{LM}, \\
  \chi^V_{LM} &\equiv \sum_\ell \obk{\N_{V\ell}|\D_M|\I g_{L\ell}}, \\
  \D_M &\equiv \sum_{m,n} \delta_{M,m+n} \op{mn}.
\end{align}
We then define the state vector
\begin{align}
  \oket{\rho_V^\star} = \f1d \oket{\1_d} + U \sum_{\ell>0} \sp{\N_{V\ell} + \I\N_{V\ell}^*\I}^{-1} \ket{\chi_{V\ell}},
\end{align}
where $\1_d$ is the $d\times d$ identity matrix, $\ket{\chi_{V\ell}} \equiv \sum_{m=-\ell}^\ell \ket{\ell m} \bk{\ell m|\chi_V}$ is the projection of $\ket{\chi_V}$ onto the subspace of fixed degree $\ell$, the unitary $U$ defined in Eq.~\eqref{eq:trans_basis} takes $\ket{\ell m}\to \oket{T_{\ell m}}$, and $\N_{V\ell}^*$ is the complex conjugate of $\N_{V\ell}$.
The quantum error scale $\epsilon_V^2$ is then
\begin{align}
  \epsilon_V^2 \equiv \sum_{\ell>0} \obk{\N_{V\ell}|\C_\ell\sp{\rho_V^\star}}.
  \label{eq:scale}
\end{align}
We derive Eq.~\eqref{eq:scale} in Appendix \ref{sec:error}, and discuss its efficient calculation in Appendix \ref{sec:compute}.
Note that the ``state'' $\rho_V^\star$, which maximizes the reconstruction error in Eq.~\eqref{eq:error}, has trace $\tr\rho_V^\star=1$, but may generally have negative eigenvalues.
In principle, there is therefore room for improvement on the bound in Eq.~\eqref{eq:bound} by maximizing reconstruction error over the (restricted) domain of states $\rho$ with a non-negative spectrum.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Exact reconstruction error}
\label{sec:error_exact}
...
We discuss the details of efficiently computing the error in Eq.~\eqref{eq:error_SM} in Appendix \ref{sec:compute}, and devote the rest of this Appendix to finding upper bounds for this error.
...
\begin{align}
  ...
  \bk{m|\p{\N_{V\ell} + \I \N_{V\ell}^* \I}|n}
...
\end{align}
Here $\N_{V\ell}^*$ is the complex conjugate (and also the transpose) of $\N_{V\ell}$.
...
\begin{align}
  \oket{\rho_V^\star}
  = \f1d \oket{\1_d} + \sum_{\ell>0} \L_{V\ell}^{-1} \oket{\chi_{V\ell}},
\end{align}
where $\1_d$ is the $d\times d$ identity matrix (added artificially to enforce $\tr\rho_V^\star=1$; the identity component of $\rho_V^\star$ does not actually contribute to reconstruction error), and $\L_{V\ell}^{-1}$ is the inverse of $\L_{V\ell}$.
...
% We note that when computing this bound in practice, the vector $\oket{\I g_{L\ell}}$ should be pre-computed once and recycled for every new choice of axes $V$, and the inner product $\obk{\N_{V\ell}|\D_M|\I g_{L\ell}}$ to compute $\chi^V_{LM}$ should be seen as an inner product of $O(d)$-sized off-diagonal bands of $\I g_{L\ell}$ and $\N_{V\ell}$.
% Also, the construction of $\oket{\chi_V}$, $\L_{V\ell}^{-1}$, and $\oket{\rho_V^\star}$ is most efficiently performed in the transition operator basis $\set{\oket{T_{\ell m}}}$.
