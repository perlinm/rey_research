...
Nonetheless, these protocols obfuscate the actual requirements for performing full state tomography, provide no straightforward error bounds or guarantees of accuracy, and extract only a small faction of the information contained in measurement data.
...
To this end, we start with Eq.~\eqref{eq:bound_apndx} from Appendix \ref{sec:bound} to write
\begin{align}
  \E_V\p{\rho}^2
  = \sum_{\ell,m,i,i'} \p{q^V_{\ell mi}}^* q^V_{\ell mi'} \bbk{\epsilon_{i\ell}\epsilon_{i'\ell}}
...
A loose upper bound, which may nonetheless be tighter than that in Eq.~\eqref{eq:bound} of the main text (derived in Appendix \ref{sec:bound}), might be found by the fact that physical qudit states $\rho$ satisfy $\obk{\rho|\rho}\le1$.
