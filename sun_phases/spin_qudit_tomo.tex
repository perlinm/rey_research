...
\begin{abstract}
  ...
  We then provide an algorithm with $O\p{dr^3}$ serial runtime for certifying strict upper bounds on the error with which a tomography protocols with $r$ measurement axes can reconstruct an unknown quantum state.
...
Equivalently, this minimization accounts for the fact that we can perform the analysis between Eqs.~\eqref{eq:overlap_matrix} and \eqref{eq:bound} identically with any $G_V\to U G_V U^\dag$, where $U$ is an arbitrary unitary that mixes the projectors in $\v\Pi_V$.
