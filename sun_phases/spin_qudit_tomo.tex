...
\begin{align}
  ...
  \label{eq:meas_mat}
\end{align}
whose columns are flattenings of the operators in $\v T_V$.
A necessary and sufficient for $V$ to allow for full state tomography is for the operators in $\v T_V$, or equivalently the columns of $M_V$, to span the entire ($d^2$-dimensional) space of operators on a $d$-level spin qudit.
...
\begin{align}
  ...
  \label{eq:scale}
\end{align}
which we call the {\it error scale} of $V$, is the 2-norm of the pseudoinverse $M_V^{-1}$ of $M_V$.
...
The complexity of computing the error scale $\S_V$ is nominally determined by the need to
\begin{enumerate*}
\item construct the measurement matrix $M_V$, and
\item compute the singular values of $M_V$.
\end{enumerate*}
By the expansion $T_{\v v\ell,0} = R\p{\v v} T_{\ell,0} R\p{\v v}^\dag$, each of the $\abs{V}d$ columns of $M_V$ takes $O(d^3)$ time to compute, for an overall $O(\abs{V}d^4)$ serial or $O(d^3)$ parallel runtime.
In order to compute the singular values of $M_V$, we first note that the orthogonality of transition operators with different degrees, i.e.~$\obk{T_{\v v\ell,0}|T_{\v v',\ell',0}} = 0$ for $\ell\ne\ell'$, implies that $M_V$ can be decomposed into a direct sum of the rectangular blocks
\begin{align}
  M_{V\ell} \equiv \sum_{\v v} \oket{T_{\v v\ell,0}} \bra{\v v},
\end{align}
using which we can expand
\begin{align}
...
Finding the singular values of each block $M_{V\ell}$ takes $O(\abs{V}^2d^2)$ time at most \footnote{The calculation of $\S_{V\ell}$ can be sped up by the fact that it is independent of $d$ as long as $d\ge\ell+1$, so we can essentially set $d=\ell+1$ for the purposes of computing $\S_{V\ell}$}.
Altogether, if we assume that $\abs{V}\sim d$ then certifying the bound in Eq.~\eqref{eq:bound} by computing $\S_V$ takes $O(d^5)$ serial or $O(d^4)$ parallel runtime (see Figure [\red{REF}]).  [\red{todo: add figure showing runtime for each routine as a function of $d$ with $\abs{V}=2d-1$; modify text appropriately after making the figure}].
We suspect that more efficient representations of the transition operators $T_{\ell m}$, namely as $\p{2\ell+1}$-component vectors rather than $d\times d$ matrices, together with appropriate changes to the representation of the adjoint rotation operator $\R\p{\v v}$, can reduce the complexity of computing $\S_V$ by a factor of $O(d)$.
...
If the experimental measurements $E_j$ of $\bk{T_j}_\rho$ are accurate to within an error $\lesssim\epsilon$, then the state $\tilde\rho_V$ defined in Eq.~\eqref{eq:reconstructed_state} is an approximation of the true state $\rho$ to within a 2-norm distance $\lesssim\epsilon\S_V$.
