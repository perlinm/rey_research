...
\end{align}
For comparison, we note that the error scale $\epsilon_V^2<\S_V^2/2$.
...
These spin projections determine the expectation values $\bk{T_{\v v\ell,0}}_\rho$, whose estimates can be used to construct the state $\tilde\rho$ defined in Eq.~\eqref{eq:reconstructed_state}.
If spin projection was measured $n$ times along every axis, then the reconstructed state $\tilde\rho$ approximates the true state $\rho$ to within a mean squared (Euclidean) distance $\epsilon_V^2/n$.
...
At a fixed total number of measurements, $N=n\abs{V}$, increasing the number of measurement axes $\abs{V}$ reduces the number of measurements $n$ along each axis $\v v\in V$.
...
Though the result in Eq.~\eqref{eq:variance_bound} is sufficient to proceed with our derivation of an improved tomographic error bound, we first take some time to better understand the bound on $\bbk{\epsilon_{i\ell}^2}$ analytically.
...
\end{align}
We can get a tighter analytical bound on $\bbk{\epsilon_{i\ell}^2}$ by considering that if $\mu_{\t{max}}\ne0$, then
\begin{align}
  g_\ell\p{p} \le \sum_\mu t_{\ell\mu}^2 p_\mu
  \le t_{\ell\mu_{\t{max}}}^2
  ...
  = \f12,
\end{align}
where we used the fact that $t_{\ell\mu_{\t{max}}}^2 = t_{\ell,-\mu_{\t{max}}}^2$ due to the symmetries of the Clebsch-Gordan coefficients.
If $\mu_{\t{max}}=0$, then
\begin{align}
...
\begin{align}
  \sqrt{\f{2\ell+1}{d}}\, \gamma_\ell
  = \f12 \p{t_{\ell\mu_{\t{max}}} - t_{\ell\mu_{\t{min}}}}
  \le \f12 \p{t_{\ell\mu_{\t{max}}} + \abs{t_{\ell\mu_{\t{min}}}}}
  \stackrel{\mu_{\t{max}}=0}{\le}
  \f12 t_{\ell\mu_{\t{max}}}
  + \f12\sqrt{\f{1 - t_{\ell\mu_{\t{max}}}^2}{2}}
  \equiv \Gamma\p{t_{\ell\mu_{\t{max}}}}.
\end{align}
If is straightforward to show that $\Gamma\p{M}$ has a single maximum at $M=\sqrt{2/3}$, and that $\Gamma^\star\equiv\Gamma\p{\sqrt{2/3}}=\sqrt{3/8}$, so
\begin{align}
  g_\ell\p{p} \le \f{2\ell+1}{d}\, \gamma_\ell^2
  \stackrel{\mu_{\t{max}}=0}{\le} \p{\Gamma^\star}^2
  = \f38 < \f12.
\end{align}
...
\begin{align}
  ...
  \label{eq:bound_full}
\end{align}
We note that whereas the bound on $\bbk{\epsilon_{i\ell}^2}$ in Eq.~\eqref{eq:variance_bound} is tight, the bounds in Eq.~\eqref{eq:bound_single} and Eq.~\eqref{eq:bound_full} are not: the upper bound on $\bbk{\epsilon_{i\ell}^2}$ cannot be achieved simultaneously for all axes $\v v_i$ or degrees $\ell$.
In principle, there is therefore room for improvement on the bound in Eq.~\eqref{eq:bound_full}.
