...
\begin{abstract}
  ...
  We conjecture that $r_d=2d-1$, which we have verified for all $d\le100$ using numerical experiments.
  We then provide an algorithm with $O\p{r^3}$ runtime for certifying strict upper bounds on the error with which measurements of spin projection along $r$ measurement axes can be used to reconstruct an unknown quantum state.
  Our algorithm motivates a simple randomized tomography protocol, which we use to study the trade-off between the number of measurement axes and state reconstruction error [\red{note: this part is not yet complete}].
...
\begin{align}
  ...
  = \rho^V_k.
\end{align}
The eigenvalues and eigenvectors of the overlap matrix $G_V$ provide strict upper bounds on the distance between the empirical estimate $\tilde\rho$ and the true state $\rho$.
...
Seen otherwise, this minimization accounts for the fact that we can repeat the analysis between Eqs.~\eqref{eq:overlap_matrix} and \eqref{eq:bound} with any $G_V\to U G_V U^\dag$, where $U$ is an arbitrary unitary that mixes the projectors in $\v\Pi_V$.

To better understand the bound in Eq.~\eqref{eq:bound}, we observe that by a change of operator basis, or equivalently a particular choice of unitary $U_T$, measuring the projectors $\set{\Pi_{\v v\mu}}$ is equivalent to measuring the transition operators $\set{T_{\v v\ell,0}}$.
Orthogonality of transition operators with different degrees $\ell$ then implies that the overlap matrix
\begin{align}
  G_V^{\t{trn}} \equiv U_T G_V U_T^\dag
  = \sum_{\ell,\v v,\v v'} \obk{T_{\v v\ell,0}|T_{\v v'\ell,0}}
  \op{\v v\ell}{\v v'\ell}
\end{align}
has a block-diagonal structure, with $d$ blocks of size $\abs{V}\times\abs{V}$.
Eigenvectors of $G_V$ can therefore have at most $\abs{V}$ nonzero components in the transition operator basis, which implies that
\begin{align}
  \N_V \le \sum_k \f{\norm{U_T\v x^V_k}_1^2}{G^V_k}
  \le \sum_k \f{\abs{V}\,\norm{U_T\v x^V_k}_2^2}{G^V_k}
  = \sum_k \f{\abs{V}}{G^V_k}.
\end{align}
Note that these sums are performed over the $d^2$ largest eigenvalues of $G_V$ (or $G_V^{\t{trn}}$), and therefore may be divergent if $G_V$ has fewer than $d^2$ nonzero eigenvalues.
Altogether, diagonalizing each of $d$ blocks of $G_V^{\t{trn}}$ takes a time that is $O(\abs{V}^3)$, so an upper bound for $\N_V$ can be computed in $O(d\abs{V}^3)$ serial or $O(\abs{V}^3)$ parallel runtime.
[\red{Note: rather than an absolute error $\epsilon$ on experimental measurements, we can assume that $E_j$ is equal to $\bk{\Pi_j}_\rho$ up to Gaussian noise with variance $\epsilon^2$.  In that case, rather than a strict error bound we can make statements about the expected (squared) distance between $\tilde\rho$ and $\rho$, which turns out to be simply $\epsilon^2\sum_k1/G^V_k$.  In retrospect, I think that this is a much nicer result, but I haven't written it up yet, which will require me to restructure this section somewhat.}]
...
\begin{align}
  \tilde\N_V^2 \equiv \sum_k \f{\norm{\v y^V_k}_1^2}{G^V_k}
  \ge \N_V^2,
\end{align}
where $\v y^V_k$ are the (normalized) eigenvectors of the overlap matrix $G_V^{\t{trn}}$ in the transition operator basis, with corresponding eigenvalues $G^V_k$.
If $\abs{V}$ is too large for variational optimization to be practical, one can instead generate many random sets of measurement axes, and then choose the set that minimizes $\tilde\N_V^2$.
Upon choosing a set of measurement axes $V$, experimentally characterize the expectation values $\bk{\Pi^V_j}_\rho$.
If these measurements can approximate the expectation values $\bk{\Pi^V_j}_\rho$ to within a known error $\epsilon$, then the empirical estimate $\tilde\rho$ defined in Eq.~\eqref{eq:reconstructed_state} is an approximation of the true state $\rho$ to within a 2-norm distance of $\epsilon\N_V$.
If the empirical estimate $\tilde\rho$ has negative eigenvalues, its 2-norm distance from $\rho$ can be further reduced via maximum-likelihood corrections \cite{smolin2012efficient}.
In terms of classical computing costs before and after the estimation of expectation values $\bk{\Pi^V_j}_\rho$, this tomography protocol is bottlenecked by the need to diagonalize the overlap matrix $G_V^{\t{trn}}$, which has $d$ blocks of size $\abs{V}\times\abs{V}$.
Altogether, this protocol has an $O(d\abs{V}^3)$ serial or $O(\abs{V}^3)$ parallel runtime.
...
As a last point, we mention that the cost function $\tilde\N_V^2$ can also evaluate the quality of measurement axes chosen using existing methods for sparse signal recovery on the sphere \cite{rauhut2011sparse, alem2012sparse}.
In practice, we find that uniform random sampling outperforms these methods, in the sense that it generates low-cost measurement axes with higher probability.
Two observations may explain the poor performance of the sparse signal recovery methods: first, that spin qudit tomography essentially boils down to recovering sparse signals with a particular harmonic structure, namely with a fixed degree $\ell$.
...
Second, spin qudit tomography essentially requires recovering signals that are band-limited at degree $d$, but have $O(d)$ non-zero harmonic coefficients (out of $d^2$ coefficients in total).
