...
Moreover, the degree-$\ell$ polarization operators $T_{\ell m}$ transform identically to spin-$\ell$ particles and spherical harmonics $Y_{\ell m}$ under 3D rotations (see Appendix \ref{sec:rotations}).
...
The polarization operators $T_{\ell m}$ share a connection to the spherical harmonics $Y_{\ell m}$ that goes beyond their indexing and the rules for their transformation under 3D rotations.
...
As a special case, the phase-space representation $\rho^{\t{PS}}$ of a qudit state $\rho$ is common known as its Husimi distribution.
...
In principle, the representation $\rho^{\t{PS}}$ of a finite-dimensional qudit state $\rho$ can be reconstructed from the values $\rho^{\t{PS}}\p{\v v}=\bk{s_{\v v}|\rho|s_{\v v}}$ at a finite number of points $\v v$ on the sphere.
...
In addition to providing a new perspective on the problem of spin qudit tomography, the connection will allow us to place upper bounds on the minimal number of measurement axes $\v v$ necessary for full recovery of an unknown state.
...
\begin{align}
  \sum_{\v v} C_{\ell m}\p{\v v} D^\ell_{n,0}\p{\v v}
  = \delta_{mn}
...
\item For each $\ell\in\set{0,1,\cdots,d-1}$, build the measurement matrix $M_{V\ell}$ defined in Eq.~\eqref{eq:block}, and compute its singular value decomposition to identify the singular values $M^V_{\ell k}$ and corresponding right singular vectors $\v x^V_{\ell k} = \sum_{\v v} x^V_{\ell k \v v} \ket{\v v}$, indexed by an integer $k\in\set{1,2,\cdots,2\ell+1}$. % todo: cite codes
\item Compute the complex numbers
  \begin{align}
  ...
  \end{align}
\end{enumerate}
If reconstruction error is dominated by shot noise, then the total error in $\tilde\rho_V$, or its root-mean-square distance from $\rho$, is approximately $\E_V\p{\tilde\rho_V}\approx\E_V\p{\rho}$.
...
Implicitly associating the Euler angle doublet $\v v = \p{\alpha,\beta}$ with the triplet $\p{\alpha,\beta,0}$, the transformation rules in Eq.~\eqref{eq:trans_rot_apndx} imply that we can expand the phase-space representation of $T_{\ell m}$ as
\begin{align}
  T_{\ell m}^{\t{PS}}\p{\v v}
