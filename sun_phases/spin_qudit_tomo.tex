...
We generally defer mathematical details and derivations to the appendices, keeping the main text focused on a high-level discussion of our results.
...
Finally, for any fixed degree $\ell$, finding solutions to the reduced recovery problem in Eq.~\eqref{eq:tomo_recovery} is equivalent to the recovery of a function in the degree-$\ell$ subspace of $L^2\p{\Omega_2}$, which is possible with $\abs{V}=2\ell+1$ samples (equal to the dimension of the degree-$\ell$ subspace) \cite{freeden2008spherical}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Optimized tomography protocols}
\label{sec:protocol}
...
\item process the collected data to reconstruct the state of the spin qudit.
  \label{tomo:process}
\end{enumerate*}
Whereas step \ref{tomo:measure} can involve a host of platform-dependent technical challenges, here we discuss the steps before and after measurement data collection.

We begin by discussing the selection of measurement axes $V=\set{\v v}\subset\Omega_2$.
In the absence of analytical tools to find ``optimal'' deterministic choice of axes, we compare three randomized selection methods that are motivated by past works on sparse signal recovery \cite{rauhut2011sparse, alem2012sparse}.
Conceptually, the simplest method of randomly choosing axes would be to choose $\abs{V}$ points from the uniform distribution on the sphere.
To select a point $\v v=\p{\alpha_{\v v},\beta_{\v v}}$ from the uniform distribution on the sphere, we first select a point $\p{a_{\v v},z_{\v v}}\in[0,1]\times[0,1]$ from the uniform distribution on the unit square, and set $\alpha_{\v v}=2\pi a_{\v v}$ and $\beta_{\v v}=\arccos\p{1-2z_{\v v}}$.
We will call this procedure the {\it uniform} sampling method.

The work in Ref.~\cite{rauhut2011sparse} proposes an alternate sampling method for sparse signal recovery on the sphere, which consists of uniformly sampling angles $\p{\alpha_{\v v},\beta_{\v v}}\in[0,2\pi]\times[0,\pi]$.
We will refer to this procedure as the {\it biased} sampling scheme, as it places a higher weight on points near the poles of the sphere.
For general-purpose recovery of band-limited functions $f=\sum_{\ell,m} f_{\ell m} Y_{\ell m}$ with only a small number of non-zero spherical harmonic coefficients $f_{\ell m}$, Ref.~\cite{rauhut2011sparse} found that the biased sampling scheme was more likely to result in ``good'' points that allow for accurate signal recovery.

Finally, we consider the {\it spiral} sampling scheme laid out in Ref.~\cite{alem2012sparse}, which first generates a spiral of points that go from the north pole to the south, using this spiral to define a grid of points on the entire sphere, and then randomly choosing $\abs{V}$ points on this grid.

[\red{todo: make figure visualizing these sampling schemes}]
