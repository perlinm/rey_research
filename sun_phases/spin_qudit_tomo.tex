...
By a simple change of operator basis, measuring the projectors $\set{\Pi_{\v v\mu}}$ is therefore equivalent to measuring the transition operators $\set{T_{\v v\ell,0}}$, and provides data on the expectation values $\bk{T_{\v v\ell,0}}_\rho$, where $\bk{X}_\rho \equiv \obk{\rho|X} = \tr\p{\rho X}$ for a qudit state $\rho$.
...
We discuss details of efficiently computing $\xi_{V\ell}$ in Appendix \ref{sec:compute}.
...
\end{align}
is the variance of the entries in $X$ when sampled according to the probability distribution $p$.
...
\end{align}
The block diagonal structure of this matrix implies that, rather than an integer index $k\in\set{1,2,\cdots,d^2}$, we can identify the singular values and right singular vectors of $M_V$ by a block index $\ell\in\set{0,1,\cdots,d-1}$ and a sub-index $m=\set{-\ell,-\ell+1,\cdots,\ell}$.
...
\begin{align}
  ...
  = \f1n \sum_{\ell,i} C^V_{\ell i} \cov_\rho\p{T_{i\ell},T_{i\ell}},
  &&
  C^V_{\ell i} \equiv \sum_m \abs{q_{\ell mi}}^2.
  \label{eq:error_SM_start}
\end{align}
Identifying the singular value decomposition $M_{V\ell} = U_{V\ell} \Sigma_{V\ell} W_{V\ell}^\dag$, the vector $\tilde{\v q}_{\ell i} = (q_{\ell,-\ell,i},q_{\ell,-\ell+1,i},\cdots q_{\ell,\ell,i})$ is essentially the $i$-th row of $\Sigma_{V\ell}^{-1} W_{V\ell}^\dag$, which is to say that $\tilde{\v q}_{\ell i} = \Sigma_{V\ell}^{-1} W_{V\ell}^\dag \ket{\v v_i}$.
...
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \f1n \sum_{i,\ell,m,n}
  C^V_{\ell i} D^\ell_{m,0}\p{\v v_i}^* D^\ell_{n,0}\p{\v v_i}
  \cov_\rho\p{T_{\ell m}^\dag, T_{\ell n}}.
\end{align}
Altogether, the result in Eq.~\eqref{eq:error_SM_start} can be expressed more compactly using the $\p{2\ell+1}\times\p{2\ell+1}$ matrices
\begin{align}
  \xi_{V\ell} \equiv
  M_{V\ell} \diag\sp{M_{V\ell}^{-1} \p{M_{V\ell}^{-1}}^\dag} M_{V\ell}^\dag,
  &&
  \T_{\rho\ell} \equiv
