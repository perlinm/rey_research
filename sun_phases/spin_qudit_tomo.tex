...
\begin{abstract}
  ...
  We conjecture that $r_d^{\t{min}}=2d-1$, which we verify for all $d\le200$ with numerical experiments.
...
We first introduce, in Section \ref{sec:polarization_ops}, a basis of operators that have a simple physical interpretation, and transform nicely under rotations in 3D space, thereby respecting the symmetries of the tomography task at hand.
...
Nonetheless, in the following sections we numerically verify that $2d-1$ measurement axes suffice for all $d\le200$.
...
By a simple change of basis, measuring these projectors is equivalent to measuring the set of polarization operators $\v T_V\equiv\set{T_{\v v\ell}}$.
By flattening the $d\times d$ matrices $T_{\v v\ell}$ in $\v T_V$ into $d^2$-component column vectors $\oket{T_{\v v\ell}}$, we can construct the {\it measurement matrix}
\begin{align}
...
A necessary and sufficient condition for $V$ to allow for full state tomography is that the measured polarization operators in $\v T_V$, or equivalently the columns of $M_V$, span the entire ($d^2$-dimensional) space of operators on a $d$-level spin qudit.
...
Note that the classical error scale $\S_V$ diverges if the measurement matrix $M_V$ is singular, in which case measuring spin projections along all axes in $V$ does not provide sufficient information to reconstruct arbitrary quantum states.

Computing the classical error scale $\S_V$ to certify the bound in Eq.~\eqref{eq:bound_eps} requires constructing the measurement matrix $M_V$ and finding its singular values.
...
If we assume that $\abs{V}\sim d$, then computing $\S_V$ to certify the bound in Eq.~\eqref{eq:bound_eps} takes $O(d^4)$ serial or $O(d^3)$ parallel runtime (see Figure \ref{fig:times}).

\begin{figure}
  \centering
  \includegraphics{qudit_errors/qudit_times.pdf}
  \caption{Serial runtime to compute $\S_V$, $\epsilon_V$, or $\E_V\p{\rho}$ with $\abs{V}=2d-1$ randomly chosen measurement axes and a randomly chosen qudit state $\rho$.
    ...
    Dashed lines shows a fit to runtime $t=c d^\alpha$ for all $d\ge100$, finding $\alpha\approx 3.7$ [\red{todo: update/correct after more simulations}].
    These results do not count fixed $O(d^3)$ runtimes for pre-computing quantities that can be recycled for every new choice of $V$ or $\rho$.
    The simulations used to benchmark runtime found axes $V$ with $\epsilon_V<d$ for all $d\le200$, thereby verifying for these cases that $2d-1$ measurement axes are sufficient to perform full state tomography.}
  \label{fig:times}
\end{figure}
...
\begin{align}
  ...
  \label{eq:bound}
\end{align}
where the {\it quantum error scale} $\epsilon_V$ is defined in terms of (half of) the spectral range of $T_\ell$:
\begin{align}
...
Though straightforward to compute, the bound in Eq.~\eqref{eq:bound} is not tight, as it is acquired by bounding the statistical variance $\epsilon_{\v v\ell}^2$ in the empirical estimate $\tilde T_{\v v\ell}$ of $\bk{T_{\v v\ell}}_\rho$ as $\bbk{\epsilon_{\v v\ell}^2}\le\Gamma_\ell^2$.
...
While the true error in $\tilde\rho_V$ cannot be known exactly without knowing $\rho$, this error can be estimated after constructing $\tilde\rho_V$ by computing $\E_V\p{\tilde\rho_V}$.
...
\end{align}
Here $g_{L\ell}$ is a matrix of structure constants for the polarization operator algebra (see Appendix \ref{sec:trans_prod}), and $\D_M$ simply picks off the $M$-th diagonal of the matrix it acts on, such that $\obk{\N_{V\ell}|\D_M|\I g_{L\ell}}$ is an inner product of the $M$-th diagonal of $\I g_{L\ell}$ with the $\p{-M}$-th diagonal of $\N_{V\ell}$.
Overall, the complexity of computing $\E_V\p{\rho}$ is the same as that of $\epsilon_V$ and $\S_V$ (see Figure \ref{fig:times}).
...
The estimates $\tilde\Pi_{\v v\mu}$ of $\bk{\Pi_{\v v\mu}}_\rho$ determine estimates $\tilde T_{\v v\ell}$ of $\bk{T_{\v v\ell}}_\rho$, which can be used to construct the estimate $\tilde\rho_V$ of $\rho$
The error in $\tilde\rho_V$ can be estimated by computing the right-hand side of Eq.~\eqref{eq:error} with the replacement $\rho_V\to\tilde\rho_V$.
...
As a last point, we mention that the quantum error scale $\epsilon_V$ can also be used to evaluate the ``quality'' of measurement axes $V$ chosen with existing methods for sparse signal recovery on the sphere \cite{rauhut2011sparse, alem2012sparse}.
...
The reconstruction error bound in Eq.~\eqref{eq:bound} nominally provides a straightforward answer: at a fixed total number of measurements, $N=n\abs{V}$, the number of measurement axes should be chosen to minimize $\E_V\p{\rho}^2<\epsilon_V^2/n\propto\epsilon_V^2\abs{V}$.
...
\begin{align}
  ...
  - \f1n D_V\p{\rho,\sigma_V^\star}^2,
  &&
  D_V\p{X,Y}^2 \equiv
  \sum_\ell \bk{\p{X-Y}_\ell | \N_{V\ell} | \p{X-Y}_\ell},
\end{align}
where $Z_\ell\equiv\sum_m \obk{T_{\ell m}|Z} \ket{m}$ is a vector of the degree-$\ell$ components of $Z$ in the polarization operator basis.
Finding the maximum of $\E_V$ over all qudit states $\rho$ thus amounts to finding the closest physical qudit state $\rho$ to $\sigma_V^\star$, with distance measured by the metric $D_V$ induced by the noise matrices $\N_{V\ell}$.
This minimization problem is essentially solved for the case of $\N_{V\ell}=1$ in Ref.~\cite{smolin2012efficient}, but it is not obvious how to minimize $D_V\p{\rho,\sigma_V^\star}$ for general $\N_{V\ell}$.
We leave the general problem of minimizing $D_V\p{\rho,\sigma_V^\star}$ to future work, noting that this task may be assisted by positivity conditions such as those derived in Ref.~\cite{kryszewski2006positivity}.
A loose upper bound, which may nonetheless be tighter than that in Eq.~\eqref{eq:bound} of the main text (derived in Appendix \ref{sec:improved_bound}), might be found by the fact that physical qudit states $\rho$ satisfy $\obk{\rho|\rho}\le1$.
