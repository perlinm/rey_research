...
Spin qudits can provide advantages over their qubit counterparts for quantum sensing \cite{hemmer2018squeezing, evrard2019enhanced}, enable quantum simulations of SU($d$) magnetism \cite{cazalilla2014ultracold, banerjee2013atomic, zhang2014spectroscopic, scazza2014observation, goban2018emergence}, % todo: cite SU(n) spin model paper!
...
\begin{figure}
  ...
  \caption{Empirical measurement-adjusted error scales $\tilde\beta(p)$ with $p$ excess measurement axes, determined by minimizing over $10^3$ choices of measurement axes $V$ or 5 minutes of runtime (for each $p$), whichever comes first.
    ...
    Horizontal reference lines mark the corresponding (normalized) minimal quantum error scales $\min_\theta\epsilon_{V(\theta)}/\tilde\beta(0)$ achievable with the method in Ref.~\cite{newton1968measurability}, which measures along $2d-1$ axes $V(\theta)$ equally spaced at a polar angle $\theta$.}
  \label{fig:axes}
\end{figure}
...
% todo: paragraph on old method's appearance in fig:axes

% todo: appendix on optimizing the old method that appears in fig:axes
...
\bibliography{spin_qudit_tomo.bib}
\onecolumngrid
\newpage
\appendix
