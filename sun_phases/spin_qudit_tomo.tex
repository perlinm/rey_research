...
In practice, for modest values of $d$ we find that vectorized operations can reduce serial runtime to $\sim O(r^4)$, and that serial execution time is divided about evenly between computing rotated transition operators $T_{\v v\ell,0}$ and constructing the overlap matrix $G_V$, with a negligible amount of time spent on diagonalization (see Figure [\red{REF}]).  [\red{todo: add figure showing runtime for each routine as a function of $d$ with $\abs{V}=2d-1$; modify text appropriately after making the figure}].
We suspect that more efficient representations of the transition operators $T_{\ell m}$, namely as $\p{2\ell+1}$-component vectors rather than $d\times d$ matrices, together with appropriate changes to the representation of the adjoint rotation operator $\R\p{\v v}$, should reduce the computational complexity of computing $\M_V^2$ by a factor of $O(d)$.
