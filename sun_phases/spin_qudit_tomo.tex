...
The transition operators are spherical tensor operators, whose degree ($\ell$) is preserved under 3D rotations generated by the spin operators $S_\x,S_\y,S_\z$.
...
\begin{align}
  D^\ell_{mn}\p{\v\omega}
  \equiv \bk{\ell m|R\p{\v\omega}|\ell n}
\end{align}
...
We are thus nominally restricted to measuring projectors $\Pi_{\v v\mu} \equiv \op{\mu_{\v v}}$, where $\ket{\mu_{\v v}}\equiv R\p{\v v}\ket{\mu}$ is a state with spin projection $\mu$ onto a measurement axis determined by a point $\v v$ on the sphere.
...
Any solution to the full recovery problem with basis functions $Y_{\ell m}=D^\ell_{m,0}$ automatically solves the reduced recovery problem.
...
By flattening the operators in $\v T_V$ into column vectors $\oket{T_{\v\ell,0}}$, we can construct the {\it measurement matrix}
\begin{align}
  M_V \equiv \sum_{\v v,\ell} \oket{T_{\v v\ell,0}} \bra{\v v\ell},
  \label{eq:meas_mat}
\end{align}
A necessary and sufficient condition for $V$ to allow for full state tomography is that the transition operators in $\v T_V$, or equivalently the columns of $M_V$, span the entire ($d^2$-dimensional) space of operators on a $d$-level spin qudit.
...
\end{align}
where for shorthand we use a combined index $j=\p{\v v_j,\mu_j}$ to specify both a measurement axis $\v v_j$ and a spin projection $\mu_j$, which identify the transition operator $T_j\equiv T_{\v v_j\mu_j}$.
...
\begin{align}
  M_{V\ell} \equiv \sum_{\v v,m} \ket{m} \obk{T_{\ell m}|T_{\v v\ell,0}} \bra{\v v}
  = \sum_{\v v,m} D^\ell_{m,0}\p{\v v} \op{m}{\v v}.
\end{align}
...
Moreover, the transition operators $T_{\ell m}$ transform identically to spin-$\ell$ particles under the adjoint action of the spin operators $S_\z$ and $S_\pm$.
