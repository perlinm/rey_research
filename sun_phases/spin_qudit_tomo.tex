...
Written out in full (and derived in Appendix \ref{sec:error}),
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \f1n \sum_{i,\ell} \sigma_\rho^2\p{T_{\v v_i\ell,0}}
  \sum_m \abs{q_{\ell mi}^V}^2,
  \label{eq:error}
\end{align}
where $i$ indexes an axis $\v v_i\in V$; $\sigma_\rho^2\p{X} \equiv \bk{X^2}_\rho - \bk{X}_\rho^2$ is the variance of $X$; and $\v q_{\ell m}^V$ is a right singular vector of $M_{V\ell}$ corresponding to the singular value $M^V_{\ell m}$, with normalization $\abs{\v q_{\ell m}^V}=1/M^V_{\ell m}$.
...
As a final comment, we note that the statistical error bound in \eqref{eq:bound_full} is a ``worst-case'' bound that is agnostic to the state $\rho$.
...
\begin{align}
  ...
  = \f1n \sum_{i,\ell} \sigma_\rho^2\p{T_{i\ell}}
  \sum_m \abs{q^V_{\ell mi}}^2,
  \label{eq:error_full}
\end{align}
where $\sigma_\rho^2\p{X}\equiv\bk{X^2}_\rho-\bk{X}_\rho^2$ is the variance of $X$.
While the error in Eq.~\eqref{eq:error_full} cannot be known exactly without knowing $\rho$, this error can be estimated by computing the right-hand side of Eq.~\eqref{eq:error_full} with the replacement $\rho\to\tilde\rho_V$.
