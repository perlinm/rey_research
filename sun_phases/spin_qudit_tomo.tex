...
Seen otherwise, this minimization accounts for the fact that we can perform the analysis between Eqs.~\eqref{eq:overlap_matrix} and \eqref{eq:bound} identically with any $G_V\to U G_V U^\dag$, where $U$ is an arbitrary unitary that mixes the projectors in $\v\Pi_V$.

To better understand the bound in Eq.~\eqref{eq:bound}, we observe that by a change of operator basis, or equivalently a particular choice of unitary $U=U_T$, measuring the projectors $\set{\Pi_{\v v\mu}}$ is equivalent to measuring the transition operators $\set{T_{\v v\ell,0}}$.
...
\begin{align}
  ...
  \ge \N_V^2,
\end{align}
where $U_T\v x^V_k$ are (normalized) eigenvectors of the overlap matrix $G_V$ in the transition operator basis, obtained by diagonalizing $G_V^{\t{trn}}$.
