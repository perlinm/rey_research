...
\begin{abstract}
  ...
  We conjecture that $r_d=2d-1$, which we have verified for all $d\le120$ using numerical experiments.
...
[\red{Actually, I don't think this minimization is quite correct as stated, but it won't matter after some pending changes to this section.  Rather than an absolute error $\epsilon$, we can assume that $E_j$ approximates $\bk{\Pi_j}_\rho$ up to Gaussian noise with variance $\epsilon^2$.  In that case, I think the mean (squared) distance between $\tilde\rho$ and $\rho$ is $\epsilon^2\sum_k1/G^V_k$, which in retrospect is a much nicer result.  I am still working to write this up, which will involve some changes to this section.}]
...
In terms of classical computing costs before and after estimating the expectation values $\bk{\Pi^V_j}_\rho$, this tomography protocol is bottlenecked by the need to diagonalize the overlap matrix $G_V^{\t{trn}}$, which has $d$ blocks of size $\abs{V}\times\abs{V}$.
