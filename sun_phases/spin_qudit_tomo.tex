...
\end{align}
Ideally, we would maximize this function over all valid probability distributions, but doing so is rather difficult.
Instead, we can find a loose upper bound by ignoring the negative terms:
\begin{align}
  g_\ell\p{p} \le \sum_\mu t_{\ell\mu}^2 p_\mu
  \le \max_\mu t_{\ell\mu}^2
  = \f{2\ell+1}{d} \max_\mu \bk{s\mu;\ell,0|s\mu}^2
  \le \f{2\ell+1}{d},
\end{align}
which implies that
\begin{align}
  \bbk{\epsilon_{i\ell}^2} = \f1n g_\ell\p{p^{(i)}} \le \f{2\ell+1}{nd}.
  \label{eq:variance_bound}
\end{align}
Note that $t_{0,\mu}=1/\sqrt{d}$, so $f_0\p{p}=0$ and $\bbk{\epsilon_{i,0}^2}=0$.
...
\begin{align}
  ...
  \le \f{2\ell+1}{nd} \p{1-\delta_{\ell,0}}.
\end{align}
...
\begin{align}
  ...
  \le \f1{nd} \sum_{\ell>0} \p{2\ell+1} \S_{V\ell}^2,
\end{align}
where
\begin{align}
  \S_{V\ell} \equiv \norm{M_{V\ell}^{-1}}
