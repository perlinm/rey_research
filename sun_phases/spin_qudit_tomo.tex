...
Nonetheless, we have numerically verified that $2d-1$ measurement axes suffice for all $d\le200$ (see Section \ref{sec:bound}).
...
The assumption in Eq.~\eqref{eq:error_assumption} is reasonable when measurement error is dominated by experimental sources of noise, but breaks down when measurement error is limited by fundamental quantum (shot) noise.
We relax the assumption of Eq.~\eqref{eq:error_assumption} and perform a proper treatment of measurement error in Appendix \ref{sec:error}.
Our full treatment of measurement error yields identical conclusions, with an explicit form for $\epsilon$ in terms of the qudit dimension $d$ and the number of spin projection measurements made along every axis $\v v\in V$.
To simplify our analysis, we therefore maintain the assumption of Eq.~\eqref{eq:error_assumption} for now, in which case the mean squared error with which $\tilde\rho^V_k$ approximates $\rho^V_k$ is
\begin{align}
...
\begin{align}
  \S_V \equiv \norm{M_V^{-1}} = \sqrt{\sum_k\f1{\p{M^V_k}^2}},
  \label{eq:scale}
\end{align}
is the (Hilbert-Schmidt, or Frobenius) norm of the right inverse $M_V^{-1}$ of $M_V$.
Note that the error scale $\S_V$ diverges if $M_V$ is singular, in which case measuring spin projections along all axes in $V$ does not provide sufficient information to reconstruct arbitrary quantum states.
...
Altogether, constructing the block $M_{V\ell}$ and computing its singular values takes at most $O(\abs{V}d^2)$ time (see Appendix \ref{sec:compute} for additional details on efficiently computing $\S_{V\ell}$ [\red{todo: write tutorial}]).
...
For comparison, we note that existing algorithms for signal recovery on the sphere \cite{mcewen2011novel, rauhut2011sparse, alem2012sparse, khalid2014optimaldimensionality} similarly have $O(d^3)$ parallel runtime with $\abs{V}\gtrsim d$.
...
[\red{todo: add a state-dependent error bound}]
...
If $\abs{V}$ is too large for optimization to be practical, one can instead generate many random sets of measurement axes, and then choose the set with the smallest error scale $\S_V$.
...
If the experimental estimates of $\bk{T_{\v v\ell,0}}_\rho$ are accurate to within an error $\lesssim\epsilon$, then the state $\tilde\rho_V$ defined in Eq.~\eqref{eq:reconstructed_state} is an approximation of the true state $\rho$ to within a (Euclidean) distance $\lesssim\epsilon\S_V$.
Furthermore, if the empirical estimate $\tilde\rho_V$ has negative eigenvalues, its distance from $\rho$ can be further reduced with maximum-likelihood corrections \cite{smolin2012efficient}.
...
This assumption is reasonable if measurement error is dominated by experimental sources of noise, and it yields a simple derivation of the reconstruction bound in Eq.~\eqref{eq:bound}.
...
We then define the projectors $\Pi_{i\mu}\equiv\op{\mu_{\v v_i}}$ onto states $\ket{\mu_{\v v_i}}$ with definite spin projection $\mu$ onto axis $\v v_i\in V$, and define $\Pi_{i\mu}^{(j)}$ to be an $N$-qudit operator with $\Pi_{i\mu}$ on the $\p{i,j}$-th tensor factor and the identity elsewhere.
...
An empirical estimate of the expectation value $\bk{\Pi_{i\mu}}_\rho$ is then provided by the fraction of times that outcome $\mu$ was observed when measuring spin projection along axis $\v v_i$, that is
\begin{align}
...
Due to finite sampling error (i.e.~shot noise), an empirical estimate $\tilde\O$ of the observable $\O$ will have statistical error with mean zero:
\begin{align}
...
The covariances between statistical errors in the transition operator basis are then
\begin{align}
  \bbk{\epsilon_{i\ell} \epsilon_{i'\ell'}}
  = \sum_{\mu,\mu'} t_{\ell\mu} t_{\ell'\mu'}
  \bbk{\epsilon_{i\mu} \epsilon_{i'\mu'}}
  = \delta_{ii'} \times \f1n \sum_{\mu,\mu'} t_{\ell\mu} t_{\ell'\mu'}
  \sp{\delta_{\mu\mu'} p_\mu^{(i)} - p_\mu^{(i)} p_{\mu'}^{(i)}}.
\end{align}
In order to establish an upper bound on reconstruction error for an unknown quantum state, we need an upper bound on the variance $\bbk{\epsilon_{i\ell}^2}$ in particular.
To this end, for any multinomial probability distribution $p\equiv\p{p_1,p_2,\cdots,p_d}$ we define the function
\begin{align}
  g_\ell\p{p} \equiv \sum_{\mu,\mu'} t_{\ell\mu} t_{\ell\mu'}
  \p{\delta_{\mu\mu'} p_\mu - p_\mu p_{\mu'}}
  = \sum_\mu t_{\ell\mu}^2 p_\mu - \p{\sum_\mu t_{\ell\mu} p_\mu}^2
  = \sigma_p^2\p{t_\ell},
\end{align}
where $t_\ell \equiv \p{t_{\ell,-s},t_{\ell,-s+1},\cdots,t_{\ell s}}$ is a vector of all $t_{\ell\mu}$, and
\begin{align}
  \sigma_p^2\p{X}
  \equiv \sum_\mu p_\mu \p{X_\mu - \sum_\nu p_\nu X_\nu}^2
  = \sum_\mu p_\mu X_\mu^2 - \p{\sum_\mu p_\mu X_\mu}^2
\end{align}
is the variance of $X$ when sampled according to the probability distribution $p$.
By expanding
\begin{align}
  t_{\ell\mu} = \sqrt{\f{2\ell+1}{d}}\, \tau_{\ell\mu},
  &&
  \tau_{\ell\mu} \equiv \bk{s\mu;\ell,0|s\mu},
\end{align}
we can simplify
\begin{align}
  g_\ell\p{p} = \f{2\ell+1}{d} \times \sigma_p^2\p{\tau_\ell}.
\end{align}
We now seek to find an upper bound for $g_\ell\p{p}$, which will provide an upper bound for $\bbk{\epsilon_{i\ell}^2}$.
To bound $g_\ell\p{p}$, we note that variance $\sigma_p^2\p{X}$ is maximal when $p$ has equal weight on the largest and smallest values of $X$, which implies that
\begin{align}
  \sigma_p^2\p{\tau_\ell} \le \gamma_\ell^2,
  &&
  \gamma_\ell \equiv \f{\max\tau_\ell - \min\tau_\ell}{2}.
\end{align}
Altogether, using additionally the fact that $g_0\p{p}=0$, we can bound
\begin{align}
  \bbk{\epsilon_{i\ell}^2}
  = \f1n \times g_\ell\p{p^{(i)}}
...
\end{align}
where $\mu_{\t{max}}$ ($\mu_{\t{min}}$) is the index that maximizes (minimizes) $\tau_{\ell\mu}$.
In addition, the  Clebsch-Gordan coefficients $\tau_{\ell\mu}$ are at most $1$ in magnitude, which implies that $\gamma_\ell^2\le 1$.
However, this bound on $\gamma_\ell^2$ is not tight in general, as we could have alternately bounded
\begin{align}
  g_\ell\p{p} \le \sum_\mu t_{\ell\mu}^2 p_\mu
  \le \sum_\mu t_{\ell\mu}^2 = 1,
\end{align}
which must also hold for the probability distribution $p^\star$ that maximizes $g_\ell$, i.e.
\begin{align}
  g_\ell\p{p^\star} = \f{2\ell+1}{d} \times \gamma_\ell^2 \le 1,
  &&
  \t{so}
  &&
  \gamma_\ell^2 \le \min\Set{1, \f{d}{2\ell+1}}.
\end{align}
...
Making the replacement $k\to\p{\ell,m}$, we can thus repeat the derivations through Eq.~\eqref{eq:bound} of Section \ref{sec:bound} without the assumption of Eq.~\eqref{eq:error_assumption}, finding that
\begin{align}
  ...
  \bbk{\epsilon_{i\ell}^2}
  \le \f{2\ell+1}{nd} \times \gamma_\ell^2 \times \p{1-\delta_{\ell,0}}.
\end{align}
...
\begin{align}
  ...
  \le \f1{nd} \sum_{\ell>0} \p{2\ell+1} \gamma_\ell^2 \S_{V\ell}^2,
\end{align}
where
\begin{align}
  \S_{V\ell} \equiv \norm{M_{V\ell}^{-1}}
  = \sqrt{\sum_m \p{M^V_{\ell m}}^{-2}}
\end{align}
is the (Hilbert-Schmidt, or Frobenius) norm of the right inverse of $M_{V\ell}$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Computing the error scale}
\label{sec:compute}
