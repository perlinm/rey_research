...
\begin{abstract}
  ...
  We conjecture that $r_d=2d-1$, which we have verified for all $d\le200$ using numerical experiments.
  We then provide a parallelized algorithm with $O\p{r^3}$ runtime for certifying upper bounds on the error with which measurements of spin projection along $r$ quantization axes can be used to reconstruct an unknown quantum state.
...
Nonetheless, we have used the tomography protocol in the next section to numerically verify that $2d-1$ measurement axes suffice for all $d\le200$ \footnote{Specifically, for all $d\le200$ we chose $2d-1$ measurement axes $V=\set{\v v}\subset\Omega_2$ by randomly sampling points on the sphere, and checked that the corresponding overlap matrix $G_V$ defined in Eq.~\eqref{eq:overlap_matrix} has $d^2$ nonzero eigenvalues, which is a necessary and sufficient condition to reconstruct an arbitrary $d$-level spin qudit state from measurements of spin projection onto the axes in $V$}.
...
\begin{align}
  ...
  \label{eq:norm}
\end{align}
is a norm determined by the set of measurement axes $V$.
...
As a last point, we mention that the cost function $\N_V$ can also evaluate the quality of measurement axes chosen with existing methods for sparse signal recovery on the sphere \cite{rauhut2011sparse, alem2012sparse}.
