...
\end{align}
where $\bk{X}_\rho\equiv\tr\p{\rho X}$, and $\rho^\dag=\rho$ implies that $\rho_{\ell m}^*=\p{-1}^m\rho_{\ell,-m}$.
...
\begin{align}
  ...
  = \sum_{\v v,k} C_{\ell mk}\p{\v v} \bk{T_{\v vk,0}^\dag}_\rho.
...
To write out $\epsilon_V^2$ explicitly, we define a matrix of structure constants
\begin{align}
  g_{L\ell} \equiv \sum_{m,n=-\ell}^\ell
  \obk{T_{L,m+n}|T_{\ell m}T_{\ell n}} \op{m}{n},
\end{align}
and the inversion operator $\I\equiv\sum_m\p{-1}^m\op{-m}{m}$, which are used to define the $\p{2\ell+1}$-component vectors
\begin{align}
  \ket{\chi_{V\ell}}
  &\equiv \sum_{m=-\ell}^\ell \chi^V_{\ell m} \ket{m}, \\
  \chi^V_{LM} &\equiv \sum_\ell \obk{\N_{V\ell}|\D_M|\I g_{L\ell}}, \\
  \D_M &\equiv \sum_{m,m'} \delta_{M,m'-m} \op{mm'}.
\end{align}
Here $\D_M$ simply picks off the $M$-th diagonal of the matrix it acts on, such that $\obk{\N_{V\ell}|\D_M|\I g_{L\ell}}$ is an inner product between the $M$-th diagonals of $\N_{V\ell}$ and $\I g_{L\ell}$.
The vectors $\ket{\chi_{V\ell}}$ allow us to express reconstruction error in the simple form
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \f1n \sum_\ell \sp{\bk{\chi_{V\ell}|\rho_\ell}
    - \bk{\rho_\ell|\N_{V\ell}|\rho_\ell}},
\end{align}
where $\ket{\rho_\ell}\equiv\sum_m \rho_{\ell m}\ket{m}$.
This error is straightforward to maximize with respect to all states $\rho$, which yields the bound in Eq.~\eqref{eq:bound} with $\epsilon_V^2 \equiv \sum_\ell \epsilon_{V\ell}^2$ and
\begin{align}
  \epsilon_{V\ell}^2 \equiv
  \bk{\chi_{V\ell}|\p{\L_{V\ell}^{-1}
      - \L_{V\ell}^{-1}\N_{V\ell}\L_{V\ell}^{-1}}|\chi_{V\ell}},
\end{align}
where
\begin{align}
...
\begin{align}
  ...
  = \f1n \sum_{i,\ell,m,m'}
  \abs{\tilde{\v q}_{\ell i}}^2
  D^\ell_{m,0}\p{\v v_i}^* D^\ell_{m',0}\p{\v v_i}
  \cov_\rho\p{T_{\ell m}^\dag, T_{\ell m'}}.
\end{align}
...
In total, reconstruction error can be written in the form
\begin{align}
  n \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \obk{\chi_V|\rho}
  - \sum_\ell \obk{\rho|U_\ell\N_{V\ell}U_\ell^\dag|\rho},
  &&
  \oket{\chi_V} \equiv \sum_{L,M} \chi^V_{LM} \oket{T_{LM}},
  &&
  U_\ell \equiv \sum_m \oket{T_{\ell m}} \bra{m}.
\end{align}
Performing the change of basis $\oket{T_{\ell m}}\to\ket{\ell m}$ by defining
\begin{align}
  \ket{\rho} \equiv U^\dag \oket{\rho}
  = \sum_{\ell,m} \rho_{\ell m}\ket{\ell m},
  &&
  \ket{\chi_V} \equiv U^\dag \oket{\chi_V}
  = \sum_{\ell,m} \chi^V_{\ell m} \ket{\ell m},
  &&
  U \equiv \sum_{\ell,m} \oket{T_{\ell m}} \bra{\ell m},
\end{align}
as well as the degree-$\ell$ projections
\begin{align}
  \ket{\rho_\ell} \equiv \sum_m \rho_{\ell m} \ket{m},
  &&
  \ket{\chi_{V\ell}} \equiv \sum_m \chi^V_{\ell m} \ket{m},
\end{align}
...
\begin{align}
  ...
  = \f1n \sum_\ell \sp{\bk{\chi_{V\ell}|\rho_\ell}
    - \bk{\rho_\ell|\N_{V\ell}|\rho_\ell}}.
\end{align}
Given the expansion in Eq.~\eqref{eq:error_comps}, this error is straightforward to maximize by considering its variation with respect to $\rho_{\ell m}$:
\begin{align}
...
\begin{align}
  \ket{\chi_{V\ell}} - \L_{V\ell} \ket{\rho_{V\ell}^\star} = 0,
  &&
  \L_{V\ell} \equiv \N_{V\ell} + \I N_{V\ell} \I,
\end{align}
which implies that
\begin{align}
  \ket{\rho_{V\ell}^\star} = \L_{V\ell}^{-1} \ket{\chi_{V\ell}},
\end{align}
and in turn
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
...
where
\begin{align}
  \epsilon_V^2 \equiv \sum_\ell \epsilon_{V\ell}^2,
  &&
  \epsilon_{V\ell}^2 \equiv \bk{\chi_{V\ell}|\p{\L_{V\ell}^{-1}
      - \L_{V\ell}^{-1}\N_{V\ell}\L_{V\ell}^{-1}}|\chi_{V\ell}}.
\end{align}
Note that the bound in Eq.~\eqref{eq:bound_SM} is obtained by maximizing reconstruction error over all possible $\rho$ regardless of their spectrum, which implies that the maximizer $\rho_V^\star$ may generally be an unphysical state with negative eigenvalues.
