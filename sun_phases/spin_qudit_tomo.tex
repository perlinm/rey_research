...
To better understand the bound in Eq.~\eqref{eq:bound}, we observe that by a change of operator basis, or equivalently a particular choice of unitary $U=U_0$, measuring the projectors $\set{\Pi_{\v v\mu}}$ is equivalent to measuring the transition operators $\set{T_{\v v\ell,0}}$.
Orthogonality of transition operators with different degrees $\ell$ then implies that the overlap matrix $U_0 G_V U_0^\dag$ has a block-diagonal structure, with $d$ blocks of size $\abs{V}\times\abs{V}$.
Eigenvectors of $G_V$ in the transition operator basis can therefore have at most $\abs{V}$ nonzero components, which implies that
\begin{align}
  \min_U \norm{U\v x^V_k}_1^2
  \le \norm{U_0\v x^V_k}_1^2
  \le \abs{V}\, \norm{\v x^V_k}_2^2
  = \abs{V},
\end{align}
so
