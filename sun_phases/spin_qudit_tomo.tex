...
Computing the classical error scale $\S_V$ and finding estimates $\tilde\rho^V_k\approx\rho^V_k$ requires building the measurement matrix $M_V$ and computing its singular value decomposition.
...
\begin{align}
  ...
  \label{eq:bound}
\end{align}
where the {\it quantum error scale} $\epsilon_V$, so called because it is set by quantum noise, is defined in terms of (half of) the spectral range of $T_\ell$:
\begin{align}
...
The tomography protocol we outline above leaves open the question of {\it how many} measurement axes to choose.
Though $2d-1$ measurement axes may be {\it sufficient} for full state tomography, this is not necessarily the {\it best} choice of $\abs{V}$.
...
\begin{figure}
  ...
  \caption{Smallest ``normalized'' error scale $\beta(p)$ found with $p$ excess measurement axes, i.e.~with $\abs{V}=2d-1+p$.
    Color indicates the qudit dimension $d$ as specified by the legend.
    Here $\beta(p) \equiv \epsilon_{\t{min}}(p)\times\sqrt{2d-1+p}$ and $\epsilon_{\t{min}}(p) \equiv \min\set{\epsilon_V:\abs{V}=2d-1+p}$ is minimized over $10^3$ choices of random measurement axes or 5 minutes of runtime (for each $p$), whichever comes first.}
  \label{fig:axes}
\end{figure}
