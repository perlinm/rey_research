...
\end{align}
If $d$ is even, then we can get a tighter analytical bound on $\bbk{\epsilon_{i\ell}^2}$ by considering
\begin{align}
  g_\ell\p{p} \le \sum_\mu t_{\ell\mu}^2 p_\mu \le t_{\ell\mu_{\t{max}}}^2,
\end{align}
where $\mu_{\t{max}}$ is any index maximizing $t_{\ell\mu}$.
The symmetries of the Clebsch-Gordan coefficients imply that $t_{\ell\mu_{\t{max}}}^2 = t_{\ell,-\mu_{\t{max}}}^2$, so if $\mu_{\t{max}} \ne0$ (which is always true when $d$ is even because $s,\mu\notin\ZZ$), then
\begin{align}
  g_\ell\p{p} \le t_{\ell\mu_{\t{max}}}^2
  = \f12 \p{t_{\ell\mu_{\t{max}}}^2 + t_{\ell,-\mu_{\t{max}}}^2}
  \le \f12 \sum_\mu t_{\ell\mu}^2
  = \f12,
  &&
  \t{so}
  &&
