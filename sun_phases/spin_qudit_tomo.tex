\documentclass[notitlepage,nofootinbib,twocolumn]{revtex4-2}
...
\begin{align}
  ...
  = \obk{T_{\ell m} | R\p{\v\omega}^\dag
    \otimes R\p{\v\omega}^* |T_{\ell n}}
\end{align}
are matrix elements of the rotation operator $R\p{\v\omega}$ for spin-$\ell$ particles.

Implicitly associating the Euler angle doublet $\v v\equiv\p{\alpha,\beta}$ with the triplet $\p{\alpha,\beta,0}$, the transformation rules in Eq.~\eqref{eq:trans_rot_apndx} imply that we can expand the phase-space distribution of $T_{\ell m}$ as
\begin{align*}
  \tilde T_{\ell m}\p{\v v}
