...
By a simple change of operator basis, measuring the projectors $\set{\Pi_{\v v\mu}}$ is therefore equivalent to measuring the polarization operators $\set{T_{\v v\ell,0}}$, and provides data on the expectation values $\bk{T_{\v v\ell,0}}_\rho$.
...
By a simple change of basis, measuring these projectors is equivalent to measuring the set of polarization operators $\v T_V\equiv\set{T_{\v v\ell,0}}$.
...
\begin{figure}
  ...
  \caption{Serial runtime to compute $\S_V$, $\epsilon_V$, and $\E_V\p{\rho}$ with $\abs{V}=2d-1$ randomly chosen measurement axes and a randomly chosen qudit state $\rho$.
...
The factors $\Gamma_\ell^2$ are quick to compute and can be recycled for every new choice of axes $V$, so the complexity of computing $\epsilon_V^2$ is identical to that of $\S_V^2$ (see Figure \ref{fig:times}).
...
Overall, the complexity of computing $\E_V\p{\rho}$ is identical to that of $\epsilon_V^2$ and $\S_V^2$ (see Figure \ref{fig:times}).
...
The estimates $\tilde\Pi_{\v v\mu}$ of $\bk{\Pi_{\v v\mu}}_\rho$ determine estimates $\tilde T_{\v v\ell,0}$ of $\bk{T_{\v v\ell,0}}_\rho$, which can be used to construct the estimate $\tilde\rho_V$ of $\rho$
...
While $\E_V\p{\sigma_V^\star}^2$ is a strict upper bound on $\E_V\p{\rho}^2$ over all $\rho$ with $\tr\p{\rho}=1$, this bound turns out to be useless in practice, because $\sigma_V^\star$ will generally be a non-physical ``state'' with negative eigenvalues.
To find tight bound on $\E_V\p{\rho}^2$ over the space of physical qudit states $\rho$, we also need to constrain $\rho$ to have no negative eigenvalues.
Equipped with $\sigma_V^\star$ and $\E_V\p{\sigma_V^\star}^2$, we can expand
\begin{align}
...
This minimization problem is essentially solved for the case of $\N_{V\ell}=1$ in Ref.~\cite{smolin2012efficient}, but it is not obvious how to minimize $D_V\p{\rho,\sigma_V^\star}^2$ for general $\N_{V\ell}$.
We leave the general problem of minimizing $D_V\p{\rho,\sigma_V^\star}$ to future work, noting that this task may be assisted by the positivity conditions derived in Ref.~\cite{kryszewski2006positivity}.
