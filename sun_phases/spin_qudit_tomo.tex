...
To write out this error in full (see Appendix \ref{sec:error} for a derivation), we identify the singular value decomposition
\begin{align}
  M_{V\ell} = U_{V\ell} D_{V\ell} W_{V\ell}^\dag,
\end{align}
and define the $\p{2\ell+1}\times\p{2\ell+1}$ matrices
\begin{align}
  \xi_{V\ell} &\equiv M_{V\ell} \diag\p{W_{V\ell} D_{V\ell}^{-2} W_{V\ell}^\dag} M_{V\ell}^\dag, \\
  \T_{\rho\ell} &\equiv \sum_{m,n}
  \cov_\rho\p{T_{\ell m}^\dag, T_{\ell n}} \op{m}{n},
\end{align}
where $\diag\p{X}$ sets all off-diagonal entries of $X$ to zero, and $\cov_\rho\p{X,Y} \equiv \bk{XY}_\rho - \bk{X}_\rho \bk{Y}_\rho$.
We can then compactly express
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
...
\begin{align}
  ...
  = \f1n \sum_{i,\ell} C^V_{\ell i} \cov_\rho\p{T_{i\ell},T_{i\ell}},
\end{align}
where
\begin{align}
  C^V_{\ell i} \equiv \sum_m \abs{q_{\ell mi}}^2
  = \bk{\v v_i| W_{V\ell} D_{V\ell}^{-2} W_{V\ell}^\dag | \v v_i}
\end{align}
can be defined in terms of the singular value decomposition
\begin{align}
  M_{V\ell} = U_{V\ell} D_{V\ell} W_{V\ell}^\dag,
\end{align}
with a unitary matrix $U_{V\ell}$, diagonal matrix $D_{V\ell}$, and isometric matrix $W_{V\ell}$.
Using the fact that all $T_{i\ell}=T_{i\ell}^\dag$, we can expand
\begin{align}
...
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \f1n \sum_{\ell,m,n,i}
  D^\ell_{m,0}\p{\v v_i}^* D^\ell_{n,0}\p{\v v_i} C^V_{\ell i}
  \cov_\rho\p{T_{\ell m}^\dag, T_{\ell n}}.
\end{align}
Altogether, we can define the $\p{2\ell+1}\times\p{2\ell+1}$ matrices
\begin{align}
  \xi_{V\ell} \equiv M_{V\ell}
  \diag\p{W_{V\ell} D_{V\ell}^{-2} W_{V\ell}^\dag} M_{V\ell}^\dag
  &&
  \T_{\rho\ell} \equiv
  \sum_{m,n} \cov_\rho\p{T_{\ell m}^\dag, T_{\ell n}} \op{m}{n},
\end{align}
where $\diag\p{X}$ sets all off-diagonal entries of $X$ to zero, and compactly express
\begin{align}
  \Bbk{\norm{\tilde\rho_V-\rho}^2}
  = \f1n \sum_\ell \p{\xi_{V\ell}|\T_{\rho\ell}}.
  \label{eq:error_SM}
