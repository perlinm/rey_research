...
\begin{abstract}
  ...
  We then provide an algorithm with $O\p{dr^3}$ runtime for certifying strict upper bounds on the error with which a tomography protocols with $r$ measurement axes can reconstruct an unknown quantum state.
...
\begin{align}
  \abs{\v g^V_k}^2
  = \sum_{j=1}^{\abs{V}\times d} \abs{g^V_{kj}}^2
  = \f1{G^V_k},
\end{align}
...
\begin{align}
  ...
  \le \sum_k \abs{\epsilon_k^V}^2
  = \abs{\v\epsilon_V}^2,
\end{align}
where the vector $\v\epsilon_V \equiv \p{\epsilon^V_1,\epsilon^V_2,\cdots,\epsilon^V_{d^2}}$ depends only on the approximation error $\epsilon$ and choice of measurement axes $V$.
To better understand the behavior of this bound,
