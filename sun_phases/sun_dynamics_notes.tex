...
\end{align}
where $q$ is a quasi-momentum (in units with lattice spacing $a=1$); $\mu\in\set{I,I-1,\cdots,-I}$ is a nuclear spin projection onto a quantization axis, with $I\equiv\p{n-1}/2$ the total nuclear spin; $\xi$ is a spin-orbit-coupling angle, and $J$ is the single-particle bandwidth (proportional to the tunneling rate, $J/4$).
...
\begin{align}
  \eta_{\mu\nu q}
  \equiv -\f12 \sp{\cos\p{q+\mu\xi} - \cos\p{q+\nu\xi}}
  = \sin\p{\xi_{\mu\nu}^-} \sin\p{p+\xi_{\mu\nu}^+},
  &&
  \xi_{\mu\nu}^\pm \equiv \f{\mu\pm\nu}{2}\,\xi.
\end{align}
...
At ``short'' times $t$ for which $tJ\sin\p{I\xi}\ll1$, we can Taylor expand the Bessel functions in \eqref{eq:ops_time} about $t=0$ to get
\begin{align}
  \bk{\Psi|s_{\mu\nu}\p{t}|\Psi}
  &\approx \bk{\psi\op{\mu}{\nu}\psi}
  \times \sp{1 - \f14 \p{t J}^2 \sin\p{\xi_{\mu\nu}^-}^2}, \\
  \bk{\Psi|s^2\p{t}|\Psi}
  &\approx 1 - \f12 \p{tJ}^2 \sum_{\mu,\nu}
  \abs{\bk{\psi\op{\mu}{\nu}\psi}}^2 \sin\p{\xi_{\mu\nu}^-}^2.
\end{align}
In the limit of a small angle $\xi$ for which $I\xi\ll1$, we can further simplify
\begin{align}
  \bk{\Psi|s_{\mu\nu}\p{t}|\Psi}
  &\approx \bk{\psi\op{\mu}{\nu}\psi}
...
\end{align}
Note that the result in \eqref{eq:small_time_angle} is valid for all $t\ll\p{JI\xi}^{-1}$, which may be a long time given the small-angle assumption $I\xi\ll1$.
...
\begin{align}
  ...
  - \f{J}{2} \sum_{\mu,q} \cos\p{q + \mu\xi} s_{\mu\mu q}
\end{align}
with $J\ll U$.
