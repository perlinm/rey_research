...
% https://arxiv.org/pdf/1901.06282.pdf
% https://arxiv.org/abs/1911.00099
% https://arxiv.org/abs/2005.10910
% https://journals.aps.org/pra/abstract/10.1103/PhysRevA.86.012118
% https://journals.aps.org/pra/abstract/10.1103/PhysRevA.94.010301
% https://arxiv.org/pdf/2003.08668.pdf
% https://www.osapublishing.org/ol/abstract.cfm?uri=ol-44-10-2558
...
Denoting the polar and azimuthal angles of $\v v$ respectively by $\alpha_{\v v}$ and $\beta_{\v v}$, we can use the fact that $s_\x,s_\y,s_\z$ form an $\su(2)$ algebra to expand
\begin{align}
  s_{\v v\pm}
  &= e^{-\i\beta_{\v v}s_\z} e^{-\i\alpha_{\v v}s_\y}
  s_\pm e^{\i\alpha_{\v v}s_\y} e^{\i\beta_{\v v}s_\z} \\
  &= -\sin\alpha_{\v v}\, s_\z
  + \f12 \p{\cos\alpha_{\v v}+1} e^{\mp\i\beta_{\v v}} s_\pm
  + \f12 \p{\cos\alpha_{\v v}-1} e^{\pm\i\beta_{\v v}} s_\mp.
\end{align}
...
\begin{table}
  ...
  \caption{Drive Hamiltonians that can be implemented with various amplitude-matching conditions when all $\eta_m=0$.
    Here $\v s_{\t{3D}}^2=s_\x^2+s_\y^2+s_\z^2=I\p{I+1}$ is a constant.}
  \begin{tabular}{c|c}
    ...
    $\p{0,0,1}$ & $-s_\y^2$
    \\ \hline
    $\p{0,1,\pm1}$ & $\pm s_\z + s_\z^2 - \v s_{\t{3D}}^2$
    \\ \hline
    $\p{1,0,\pm1}$ & $\pm s_\x + s_\x^2 - \v s_{\t{3D}}^2$
    \\ \hline
    $\p{1,\pm1,0}$
    & $\pm\p{s_\z s_\x+s_\x s_\z} + s_\y^2 - \v s_{\t{3D}}^2$
  \end{tabular}
  \label{tab:drives}
\end{table}
