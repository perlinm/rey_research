...
\begin{align}
  ...
  \label{eq:drive_full}
\end{align}
where $j$ indexes a lattice site; $s_\pm$ are spin-raising ($+$) and spin-lowering ($-$) operators (containing all appropriate Clebsch-Gordan coefficients); $s_0\coloneqq s_\z$ is a spin-$z$ operator; and $\1$ is an identity operator on an $n$-level spin.
