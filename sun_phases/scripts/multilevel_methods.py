...
def trans_prod_coef_exact(dim, l1, m1, l2, m2, L, M):
    I = sym.S(dim-1)/2
    return ( (-1)**(2*I+L) * sym.sqrt((2*l1+1)*(2*l2+1)) *
             sym_cg(l1, m1, l2, m2, L, M).doit() *
             sym_wigner_6j(l1, l2, L, I, I, I) )

def trans_prod_coef(dim, l1, m1, l2, m2, L, M):
    return float(trans_prod_coef_exact(dim, l1, m1, l2, m2, L, M))
