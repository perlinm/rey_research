...
def act_on_subsets(mat, N):
    qubits = simplify(log(mat.cols)/log(2))

    if qubits == N: return mat
    if qubits > N: return zeros(2**N)
...
def symmetric_projector(qubits):
    projector = zeros(2**qubits)
    projector[0,0] = 1
    projector[-1,-1] = 1
    for n in range(1,qubits):
        symmetric_state = zeros(2**qubits,1)
...
def print_eigensystem(M, print_text = None):
    ...
    if print_text == None:
        ...
        print(print_text)
    dim = M.rows
    atom_number = simplify(log(dim)/log(2))
    for n in range(atom_number+1):
        ...
        for val, num, vecs in M[inds,inds].eigenvects():
            ...
            for vec in vecs:
                print("--------------------")
                full_vec = zeros(dim,1)
                for ii in range(len(vec)):
                    full_vec[inds[ii]] = simplify(vec[ii])
                qvec_print(full_vec)
