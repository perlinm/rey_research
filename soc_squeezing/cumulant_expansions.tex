...
\end{align}
where $\v m \equiv \p{m_+,m_\z,m_-}$; $\v j$ is an ordered list of
...
words, $\S_{\v m}$ is an exchange-symmetric $\p{m_++m_\z+m_-}$-spin
...
\begin{align}
  \S_{\v m}\S_{\v n}
  = \sum_{s\ge0} \sum_{\p{\v r,\v J,\v K;\v m,\v n,s}}
...
over $\p{\v r,\v J,\v K;\v m,\v n,s}$ denotes a sum over all values of
$\v r,\v J,\v K$ with the restrictions
\begin{align}
  r_{\mu\nu} \ge 0 ~\forall~ \mu,\nu,
  &&
  \sum_\nu r_{\mu\nu} \le m_\mu,
...
$\sigma_\mu\sigma_\nu\equiv\sum_\rho\eta_{\mu\nu\rho}\sigma_\rho$,
where unlike the sums over $\mu,\nu$ in most of the work above the sum
over $\rho$ here includes an index for the identity operator
$\sigma_0\equiv\1$, we can simplify
\begin{align}
  Q_{\v K}
  = \prod_{\mu,\nu,a} \sum_\rho
  \eta_{\mu\nu\rho} \sigma_\rho^{(K_a^{\mu\nu})},
\end{align}
and collect terms for each $\p{\mu,\nu}$ that are unique up to a
permutation of indices and have $c_\rho$ single-spin operators of
the type $\sigma_\rho$, giving us
\begin{align}
  Q_{\v K}
  \cong \prod_{\mu,\nu} \sum_{\sum_\lambda c_\lambda=r_{\mu\nu}}
  g^{\mu\nu}_{\v c\v r} \prod_\rho \p{\eta_{\mu\nu\rho}}^{c_\rho}
  \prod_{a=1}^{c_\rho} \sigma_\rho^{(K^{\mu\nu\rho}_{\v ca})},
\end{align}
where $\cong$ denotes equality under the sum in \eqref{eq:SS_PQ} (as
we have freely permuted spin indices and introduced explicit
dependence on $\v r$ for convenience); the factor
$g^{\mu\nu}_{\v c\v r}$ counts the number of ways to choose operators
in $Q_{\v K}$ according to $\v c$ (i.e.~the number of choices that are
equivalent up to permutation of indices in $\v K$), or
\begin{align}
  g^{\mu\nu}_{\v c\v r} \equiv \prod_\rho
  { r_{\mu\nu} - \sum_{\lambda<\rho} c_\lambda \choose c_\rho }
  = \f{r_{\mu\nu}!}{\prod_\rho c_\rho!};
\end{align}
and
\begin{align}
  K^{\mu\nu\rho}_{\v ca}
  \equiv K^{\mu\nu}_{\sum_{\lambda<\rho}c_\lambda+a}
\end{align}
indexes the $a$-th operator of type $\sigma_\rho$, given a choice of
$\v c$.
