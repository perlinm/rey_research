...
\begin{align}
  ...
  g_{\v m\v n\v r} Q_{\v J} \tilde Q_{\v K},
  \label{eq:SS_PQ}
\end{align}
...
over $\p{\v r,\v J,\v K;\v m,\v n,s}$ implies a sum over all possible
values of $\v r,\v J,\v K$ with the restrictions
\begin{align}
  r_{\mu\nu} \ge 0 ~\forall~ \mu,\nu,
  &&
  \sum_\nu r_{\mu\nu} \le m_\mu,
...
\end{align}
the factor $g_{\v m\v n\v r}$ counts the number of ways to pair
...
and finally
\begin{align}
  Q_{\v J} \equiv \prod_{\mu,a} \sigma_\mu^{(J_a^\mu)},
  &&
  \tilde Q_{\v K} \equiv \prod_{\mu,\nu,a}
  \sigma_\mu^{(K_a^{\mu\nu})} \sigma_\nu^{(K_a^{\mu\nu})}
  = \prod_{\mu,\nu,a} \sum_\rho
  \eta_{\mu\nu\rho} \sigma_\rho^{(K_a^{\mu\nu})},
  \label{eq:Q_JK}
\end{align}
where $\eta_{\mu\nu\rho}$ is defined by the product
$\sigma_\mu\sigma_\nu\equiv\sum_\rho\eta_{\mu\nu\rho}\sigma_\rho$.
Note that unlike the sums over $\mu$ and $\nu$ in the work above, the
sum over $\rho$ in \eqref{eq:Q_JK} now includes an index for the
identity operator $\sigma_0\equiv\1$.
