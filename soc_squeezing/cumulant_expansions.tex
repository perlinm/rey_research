...
into \eqref{eq:SS_PQ}, which (removing the tilde from
$\tilde{\v\rho}$) gives us
\begin{align}
  \S_{\v m} \S_{\v n}
  = \sum_{s\ge0} \sum_{\p{\v\ell,\v r,\v\rho;\v m,\v n,s}}
  f_{\v m\v n\v r\v\rho} P_{\v\ell},
  \label{eq:SS_simp}
\end{align}
where the net the prefactor for each term is
\begin{align}
  f_{\v m\v n\v r\v\rho}
  \equiv \sp{\prod_\mu \p{m_\mu}_{\sum_\kappa r_{\mu\kappa}}
    \p{n_\mu}_{\sum_\kappa r_{\kappa\mu}}}
  \sp{\prod_{\mu,\nu,\kappa}
    \f{\p{\eta_{\mu\nu\kappa}}^{\rho^{\mu\nu}_\kappa}}
    {\rho^{\mu\nu}_\kappa!}}
  \sp{N-\sum_\mu\p{m_\mu+n_\mu}+s}
  ^{\sum_{\alpha,\beta}\rho^{\alpha\beta}_0},
\end{align}
and in addition to \eqref{eq:rest_r} the sum over
