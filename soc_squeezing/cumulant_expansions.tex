...
\begin{align}
  ...
  \mu^\ell \epsilon_{mn}^{p\ell} S_\z^\ell,
\end{align}
with
\begin{align}
  \epsilon_{mn}^{p\ell}
...
\begin{align}
  ...
  \sum_{\ell=0}^p \mu^\ell \epsilon_{mn}^{p\ell} S_\z^\ell,
  \label{eq:comm_mu}
\end{align}
and
\begin{align}
...
\begin{align}
  Z_{mn;\mu}^{(p)}
  \equiv \sum_{\ell=0}^p \mu^\ell \epsilon_{mn}^{p\ell}
  \p{-\mu\sp{m-p} + S_\z}^\ell
  = \sum_{q=0}^p \mu^q \zeta_{mn}^{pq} S_\z^q,
\end{align}
with
\begin{align}
  \zeta_{mn}^{pq}
  \equiv \sum_{\ell=q}^p \p{-1}^{\ell-q}
  \epsilon_{mn}^{p\ell} { \ell \choose q } \p{m-p}^{\ell-q}
  = \p{-1}^{p-q} 2^q \sum_{s=q}^p
...
\begin{align}
  \sp{S_\mu^p S_\z^q S_\bmu^r, S_\mu^\ell S_\z^m S_\bmu^n}_\pm
  = \S^{pqr}_{\ell mn;\mu} \pm \S^{\ell mn}_{pqr;\mu},
\end{align}
where
\begin{align}
  \S^{pqr}_{\ell mn;\mu}
  \equiv S_\mu^p S_\z^q S_\bmu^r S_\mu^\ell S_\z^m S_\bmu^n
  = \sum_{k=0}^{\min\set{r,\ell}} k! { r \choose k } { \ell \choose k }
  S_\mu^{p+\ell-k} \tilde Z_{qr\ell m;\mu}^{(k)} S_\bmu^{r+n-k},
  \label{eq:general_product}
\end{align}
and, in terms of $\zeta_{r\ell}^{ka}$ as defined in
\eqref{eq:zeta_mnpq},
\begin{align}
  \tilde Z_{qr\ell m;\mu}^{(k)}
  &\equiv \p{\mu\sp{\ell-k}+S_\z}^q
  Z_{r\ell;\bmu}^{(k)} \p{\mu\sp{r-k}+S_\z}^m \\
  &= \sum_{a=0}^k \p{-1}^a \zeta_{r\ell}^{ka}
  \sum_{b=0}^q \p{\ell-k}^{q-b} { q \choose b }
  \sum_{c=0}^m \p{r-k}^{m-c} { m \choose c }
  \mu^{q+m+a-b-c} S_\z^{a+b+c}.
\end{align}
...
products of collective spin operators.  Making (heavy) use of
identities \eqref{eq:push_z_mu_sS} and \eqref{eq:push_mu_Ss} to push
single-spin operators through transverse collective-spin operators, we
work through all combinations that are unique up to Hermitian
conjugation, finding
\begin{align}
...
\begin{align}
  \sum_j \sigma_\z^{(j)} S_\mu^\ell S_\z^m S_\bmu^n \sigma_\mu^{(j)}
  - \f{\mu}{2} \sp{S_\mu, S_\mu^\ell S_\z^m S_\bmu^n}_+
...
\begin{align}
  ...
  \p{S+\mu S_\z} \p{-\mu+S_\z}^m S_\bmu^{n-2}, \\
  M_{\ell mn;\mu}
  &\equiv \mu n S_\mu^\ell \sp{2\p{S+\mu S_\z}\p{-\mu+S_\z}^m
    - \p{\f12\sp{n-1}+\mu S_\z} S_\z^m} S_\bmu^{n-1}.
\end{align}
Defining for completion
\begin{align}
  P_{\ell mn;\mu}
  \equiv \sum_j
  \sigma_\mu^{(j)} S_\mu^\ell S_\z^m S_\bmu^n \sigma_\mu^{(j)}
  = n S_\mu^{\ell+1} S_\z^m S_\bmu^{n-1}
  - n\p{n-1} S_\mu^\ell \p{S+\mu S_\z} \p{-\mu+S_\z}^m S_\bmu^{n-2},
\end{align}
and
...
\end{align}
Note that the sum $\ell+m+n$ does not ever grow under this form of
decoherence.
...
\end{align}
Note that the sum $\ell+m+n$ grows by one if $\Gamma_\mu\ne0$ or
...
\begin{align}
  ...
  &= \p{-\mu}^m \f{N!}{2^N} \sum_{k=-S+\max\set{\ell,n}}^S k^m
...
we can say
\begin{align}
  \bk{\phi|S_\mu^\ell S_\z^m S_\bmu^n|\phi}
  = e^{i\mu\p{\ell-n}\phi} \bk{\X|S_\mu^\ell S_\z^m S_\bmu^n|\X},
\end{align}
where
\begin{align}
...
\begin{align}
  \bk{\X|S_\mu^\ell S_\z^m S_\bmu^n|\X}
  = \f{\mu^m\p{N!}^2}{2^N\p{N-\ell}!\p{N-n}!}
