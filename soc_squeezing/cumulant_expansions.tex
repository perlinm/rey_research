...
\title{Short-time simulation of collective quantum spin systems with
  decoherence}
...
of collective spin systems with arbitrary-strength
permutationally-symmetric decoherence.  This method is based on a
short-time expansion of exact solutions to the equations of motion for
Heisenberg operators.  Evaluating this expansion requires use of the
structure factors of a collective spin operator algebra; the
calculation of these structure factors is one of the main technical
...
$s_{n=\x,\y,\z}\equiv\sigma_n/2$ and $s_\pm\equiv s_\x\pm is_\y$ with
Pauli operators $\sigma_n$, we denote an operator which acts with
$s_n$ on the spin indexed by $j$ and trivially (i.e.~with the identity
$\1$) on all other spins by $s_n^{(j)}$.  We then define the
collective spin operators $S_n\equiv\sum_{j=1}^Ns_n^{(j)}$ for
...
\end{align}
The commutator in Eq.~\eqref{eq:H_general} can be determined from the
structure factors of $f_{\v n\v m\v\ell}$ of the general product
...
$W$.  The truncation in \eqref{eq:weight_truncation} will close the
system of differential equations defined by \eqref{eq:EOM}, and allow
us to solve it using standard numerical methods.  Initial conditions
for this system of differential equations, i.e.~expectation values of
collective spin operators with respect to states that are simple to
prepare experimentally, are provided in Appendix
...
therefore devise an alternate truncation strategy built around point
\ref{pt:limitation}.
...
\end{align}
where $\delta_{\v m\v n}=1$ if $\v m=\v n$ and 0 otherwise.  For
...
\begin{align}
  ...
  \sum_{\v n} T_{\v m\v n;k} \bk{\S_{\v n}}_{t=0}.
\end{align}
Unlike the truncation strategy in Eq.~\eqref{eq:weight_truncation},
the nonzero entries $T_{\v m\v n;k}$ for $k=0,1,\cdots,M$ now
explicitly tell us which operators are relevant for computing the
expectation value $\bk{\S_{\v m}}$ to a fixed order $M$.  Furthermore,
after computing all necessary matrix elements $T_{\v m\v n;k}$ and
expectation values $\bk{\S_{\v n}}_{t=0}$, computing $\bk{\S_{\v m}}$
takes only an $\O\p{M}$ amount of computer time independent of the
system time $t$.  Finally, we note that the relation
...
\begin{align}
  H_{\t{OAT}}
  = \chi S_\z^2
  = \f14 \chi \sum_{j,k} \sigma_\z^{(j)} \sigma_\z^{(k)}
  = \f12 \chi \sum_{j<k} \sigma_\z^{(j)} \sigma_\z^{(k)} + \f14 N \chi,
\end{align}
where $\sigma_z^{(j)}$ represents a Pauli-$z$ operator acting on spin
$j$.  This Hamiltonian admits an exact solution previously worked out
in Ref.~[\citenum{foss-feig2013nonequilibrium}].  Adapting the
expectation values therein with respect to the initial state
$\ket{\X}$ satisfying $S_\x\ket{\X}=S\ket{\X}$, in terms of
...
\begin{align}
  ...
  s_X \equiv X + i\gamma,
\end{align}
defined respectively in terms of uncorrelated single-spin decay,
excitation, and dephasing rates $\gamma_-$, $\gamma_+$, $\gamma_\z$
(respectively $\Gamma_{\t{ud}}$, $\Gamma_{\t{du}}$, and
$\Gamma_{\t{el}}$ in Ref.~[\citenum{foss-feig2013nonequilibrium}]).
...
parameter at any time throughout evolution of the initial state
$\ket{\X}$ under $H_{\t{OAT}}$.
