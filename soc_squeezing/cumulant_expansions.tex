...
\begin{align}
  ...
  \allowdisplaybreaks \\
  L_{\ell mn;\mu}^{(\gamma)}
  &\equiv \mu \gamma_{\z,\mu}^{(+)} S_\mu^{\ell+1}
  \sp{\p{\mu+S_\z}^m-S_\z^m} S_\bmu^n
  + 2\mu n \gamma_\bmu^* \gamma_z R_{\ell m,n-1;\mu} \notag \\
  &\qquad - 2\mu \ell n\p{n-1} \gamma_z^* \gamma_\mu
  R_{\ell-1,m,n-2;\mu},
  \allowdisplaybreaks \\
  M_{\ell mn;\mu}^{(\gamma)}
  &\equiv 2n \gamma_{\z,\mu}^{(-)} S_\mu^\ell S_\z^{m+1} S_\bmu^{n-1}
  \notag \\
  &\qquad - 2\mu n \sp{\p{S-\ell-\f12n+\f12} \gamma_\z^* \gamma_\mu
...
\begin{align}
  Q_{\ell mn;\mu}^{(\gamma)}
  &\equiv L_{\ell mn;\mu}^{(\gamma)} + M_{\ell mn;\mu}^{(\gamma)}
  + \gamma_\bmu^* \gamma_\mu P_{\ell mn;\mu}.
\end{align}
