...
\begin{align}
  ...
  &&
  \bk{S_\z^2} = S \sp{\f12 + \p{S-\f12} \f{\bk{S_z}^2}{S^2}}.
  \label{eq:Sz_OAT}
\end{align}
...
\end{align}
Working through all combinations that are unique up to conjugation, we
then compute
\begin{align}
  ...
  \label{eq:sand_z_mu} \allowdisplaybreaks \\
  ...
  \label{eq:sand_z_bmu} \allowdisplaybreaks \\
  ...
  \label{eq:sand_mu_mu} \allowdisplaybreaks \\
...
\begin{align}
  ...
  &= S_\mu^\ell \p{S - \ell - n - \mu S_\z} \p{\mu + S_\z}^m S_\bmu^n
  - S_\mu^\ell\p{S - \f12\sp{\ell+n} - \mu S_\z}
  S_\z^m S_\bmu^n \notag \\
...
\begin{align}
  ...
  \label{eq:S_mu_acomm}
\end{align}
This anti-commutator together with the results in Sections
