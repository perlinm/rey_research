...
are worked out Appendices \ref{sec:comm_general},
...
ordering on products of spin operators, which will ensure that we only
...
\begin{align}
  ...
  = \sum_j \sigma_\z^{(j)} \O \sigma_\z^{(j)} - 2 S \O.
\end{align}
From \eqref{eq:san_z_z}, we then have
\begin{align}
  \D\p{\sigma_\z; S_\mu^\ell \p{\mu S_\z}^m S_\bmu^n}
  = -2\p{\ell+n} S_\mu^\ell \p{\mu S_\z}^m S_\bmu^n
...
number of $\sigma_\mu$ operators in each term of $\S_{\v m}$; $\v j$
is an ordered list of indices for the spins addressed by $P_{\v j}$;
and the sum over $\p{\v j;\v m}$ denotes a sum over all possible
$\v j$ for which all indices in $\v j$ are distinct and
...
\eqref{eq:Q_K_rho_simp}; and
the number of $\sigma_\mu$ operators in each term of
$\tilde\S_{\v\ell_{\v m\v n\v r\v\rho}}$ is
\begin{align}
