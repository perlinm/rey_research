...
\begin{align}
  ...
  f_{\v m\v n\v r} g_{\v\rho} \v\eta_{\v\rho} c_{\v\ell\v\rho} \S_{\v\ell},
  \label{eq:SS_simp}
\end{align}
...
\eqref{eq:Q_K_rho_simp}.  For clarity, we note here that all indices
on
$m_\mu, n_\mu, r_{\mu\nu}, \rho^{\mu\nu}_\kappa, \eta_{\mu\nu\kappa},
\ell_\mu$ take on the same values as the indices on elements
$\sigma_\mu$ of some complete basis for single-spin operators; of
these, only the last index ($\kappa$) on
$\rho^{\mu\nu}_\kappa,\eta_{\mu\nu\kappa}$ ever matches the index for
the identity element $\sigma_0\equiv\1$.
