...
\begin{align}
  c_{\v\ell\v\rho}
  \equiv \p{ N - \abs{\v\ell} }_ {\sum_{\mu,\nu}\rho^{\mu\nu}_0},
\end{align}
with $\abs{\v\ell}=\sum_\mu\p{m_\mu+n_\mu}-s$ equal to the number of
elements in $\v\ell$, accounts for the multiplicity of terms which are
equivalent up to a permutation of indices involving the identity
operators in \eqref{eq:Q_K_rho_simp}.
