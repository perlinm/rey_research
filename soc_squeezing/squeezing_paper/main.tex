\documentclass[aps,prx,superscriptaddress,notitlepage,twocolumn,longbibliography]{revtex4-1}
...
On its own, the collective Heisenberg term ($\sim\hat{\v S}\c\hat{\v S}$) in Eqn.~\eqref{eq:SpinModel} opens an energy gap $fU$, with $f\equiv N/L$ the filling fraction of spatial modes, between the collective Dicke states $\ket{S=N/2, M_S}$ and the remainder of Hilbert space\cite{rey2008manybody, martin2013quantum, norcia2018cavitymediated,Smale2018} with $S<N/2$.
...
%The only additional detail that needs to be considered for experimental implementation of our protocol is the fact that SOC needs to be suppressed during clock interrogation.  In a shallow lattice, SOC can be turned off by use of a second laser with SOC angle $\phi=0$, and a simple pulse sequence that transforms our squeezed state into the appropriate gauge (see Appendix \ref{sec:clock_interrogation}).}
