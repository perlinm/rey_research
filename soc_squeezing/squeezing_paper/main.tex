...
%In this weak SOC limit (i.e.~of all $B_q/fU\ll1$), we can treat the axial field perturbatively and perform a Schrieffer-Wolff transformation to determine its effective action on states restricted to the Dicke manifold (see Appendix \ref{sec:derivation_OAT}).
...
To address this problem, we will fix a gauge in which the Fermi-Hubbard Hamiltonian exhibits SOC, as in Eqn.~\eqref{eq:FermiHubbard}, and construct a simple pulse sequence that transforms the $\phi=0$ clock laser into homogenous form in our working gauge.
...
\begin{align}
  ...
  s_\y^{(j)} &\equiv \f{i}{2} \p{c_{j,\dn}^\dag c_{j,\up}
    - c_{j,\up}^\dag c_{j,\dn}},
\end{align}
we observe that gauge transformation $\hat c_{j,\up}\to e^{i\theta j}\hat c_{j,\up}$ takes (up to an overall phase)
\begin{align}
  S_\x = \sum_j s_\x^{(j)}
  \to \sum_j \p{\cos\p{\theta j}  s_\x^{(j)}
    + \sin\p{\theta j} s_\y^{(j)}}
  \equiv S_\x^{(\theta)},
\end{align}
This transformation thus rotates local spin operators by a site-dependent angle $\theta j$, generally taking $S_{\x,\y}^{(\alpha)}\to S_{\x,\y}^{(\alpha+\theta)}$ (with $S_\y^{(\alpha)}$ defined similarly to $S_\x^{(\alpha)}$) and leaving $S_\z$ unaffected (taking $S_\z\to S_\z$).
A clock laser with SOC angle $\phi$ induces a Hamiltonian proportional to $S_\x^{(-\phi)}$ ($S_\x^{(-\phi+\theta)}$) before (after) a gauge transformation by $\theta$, which is why transforming with $\theta=\phi$ made the SOC-inducing clock laser homogenous in the main text.

To ``undo'' the effect of our gauge transformation on spin operators, we can construct the unitary ``gauge-switching'' operator
\begin{align}
  G_\theta = \prod_j \exp\p{-i \theta j s_\z^{(j)}}
  \label{eq:gauge_rotation}
\end{align}
for which
\begin{align}
  G_\theta^\dag S_{\x,\y}^{(\alpha+\theta)} G_\theta = S_{\x,\y}^{(\alpha)}.
\end{align}
We then decompose any rotation into a product of two reflections:
\begin{align}
  \exp\p{-i\alpha s_\z^{(j)}}
  = \exp\p{-i\pi s_\x^{(j)}} \exp\p{-i\pi s_{\alpha/2}^{(j)}},
  \label{eq:rotation_reflections}
\end{align}
where $s_{\alpha/2}^{(j)} \equiv \cos\p{\alpha/2}s_\x^{(j)} + \sin\p{\alpha/2}s_\y^{(j)}$.
This decomposition implies
\begin{align}
  G_\theta = \exp\p{-i\pi S_\x^{(\theta/2)}} \exp\p{-i\pi S_\x^{(\theta)}}.
  \label{eq:gauge_decomposition}
\end{align}
The unitary in Eqns.~\eqref{eq:gauge_rotation} and \eqref{eq:gauge_decomposition} can therefore be implemented by a sequence of two $\pi$-pulses: one $\pi$-pulse by a clock laser with SOC angle $\phi=0$ (with Hamiltonian $\sim S_\x^{(\theta)}$ in the gauged frame), and a second $\pi$-pulse by a clock laser with SOC angle $\phi=\theta/2$ (with Hamiltonian $\sim S_\x^{(\theta/2)}$ in the gauged frame).
This unitary can be used to reconcile the incommensurate SOC requirements of the squeezing and clock interrogation protocols.

Starting with a spin-down-ploarized initial state
\begin{align}
  \ket{-\t{Z}}\equiv\p{\prod_j c_{j,\dn}^\dag}\ket{\t{vacuum}},
\end{align}
our OAT protocol essentially prepares the squeezed state
\begin{align}
  \ket{\xi_{\t{OAT}}}
  \equiv e^{-iH_{\t{FH}}^\theta t}
  e^{-i\p{\pi/2} S_\x} \ket{-\t{Z}},
\end{align}
for some free evolution time $t$ and a Fermi-Hubbard Hamiltonian $H_{\t{FH}}^\theta$ exhibiting SOC with angle $\theta\ll1$, where $\theta=\phi$ in Eqn~\eqref{eq:FermiHubbard}.
The follow-up clock interrogation protocol needs to rotate this squeezed state, most generally by some unitary $\exp\p{-i\v\alpha\c\v S}$, and then measure collective spin observables of the form (for example)
\begin{align}
  \bk{S_\x}_{\t{OAT}}^{\v\alpha}
  \equiv \bk{\xi_{\t{OAT}}|e^{i\v\alpha\c\v S}
    S_\x e^{-i\v\alpha\c\v S}|\xi_{\t{OAT}}}.
\end{align}
In a clock interrogation protocol with a shallow lattice, the rotation $\exp\p{-i\v\alpha\c\v S}$ and measurement of $S_\x$ must be performed with an SOC-free ($\phi=0$) clock laser, which can effectively only access the gauge-transformed operators $S_{\x,\y}^{(\theta)}$.
We therefore expand
\begin{align}
  \bk{S_\x}_{\t{OAT}}^{\v\alpha}
  &= \bk{\xi_{\t{OAT}}| G_\theta^\dag e^{i\v\alpha\c\v S^{(\theta)}}
    S_\x^{(\theta)} e^{-i\v\alpha\c\v S^{(\theta)}}
    G_\theta |\xi_{\t{OAT}}} \notag \\
  &= \bk{\xi_{\t{OAT}}^\theta| e^{i\v\alpha\c\v S^{(\theta)}}
    S_\x^{(\theta)} e^{-i\v\alpha\c\v S^{(\theta)}} |\xi_{\t{OAT}}^\theta},
\end{align}
where $\v S^{(\theta)}\equiv G_\theta \v S G_\theta^\dag$ and
\begin{align}
  \ket{\xi_{\t{OAT}}^\theta} \equiv G_\theta \ket{\xi_{\t{OAT}}}.
\end{align}
We are thus free to use the SOC-free clock laser during clock interrogation if we append the operation $G_\theta$ to our squeezing protocol.

The last subtlety of clock interrogation and ``gauge-switching'' concerns the squeezing protocol itself.
Experimentally implementing $G_\theta$ to enable clock interrogation requires clock lasers with SOC angles $\phi=0$ and $\phi=\theta/2$, neither of which is homogenous in our working gauge.
Our squeezing protocols, however, need a homogenous clock laser for (i) an initial $\pi/2$ rotation of $\ket{-\t{Z}}$, (ii) spin-echo pulses throughout the squeezing protocol, and (iii) the drive used to transform OAT into TAT.
Any clock laser pulses in the squeezing protocols can still be implemented using $G_\theta$ and the SOC-free ($\phi=0$) clock laser via the same trick used for clock interrogation, e.g.~by decomposing
\begin{align}
  \ket{\xi_{\t{OAT}}^\theta}
  &= G_\theta e^{-iH_{\t{FH}}^\theta t}
  G_\theta^\dag e^{-i\p{\pi/2} S_\x^{(\theta)}} G_\theta \ket{-\t{Z}},
  \notag \\
  &\simeq G_\theta e^{-iH_{\t{FH}}^\theta t}
  G_\theta^\dag e^{-i\p{\pi/2} S_\x^{(\theta)}} \ket{-\t{Z}},
\end{align}
where $\simeq$ denotes equality up to a global phase, and we used the fact that the initial state $\ket{-\t{Z}}$ is an eigenstate of $G_\theta$.
Applying a continuous drive, meanwhile, seemingly requires an extra clock laser with SOC angle $\phi=\theta$.
In practice, it may be prohibitively difficult to properly calibrate the relative angles of three clock lasers.
...
In this case, converting OAT into TAT would need to be done with a pulsed drive protocol, as e.g.~in Ref.[\citenum{liu2011spin}].
