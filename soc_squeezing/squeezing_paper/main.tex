...
One of the most important tasks in modern quantum science is to coherently control and entangle many-body systems, realizing powerful quantum technologies such as quantum-enhanced sensors.
However, many-body entangled states are difficult to prepare and preserve due to internal dynamics and external noise.
Here, we introduce a protocol that counterintuitively exploits interactions and inhomogeneities, typical sources of dephasing, to generate metrologically useful and robust many-body entangled states.
Our protocol is fully compatible with state-of-the-art three-dimensional (3D) optical lattice clock (OLC) interrogation schemes, and can break through limits to sensitivity set by the intrinsic quantum noise of independent atoms.
Even with realistic experimental imperfections, our protocol may generate $\sim10$--$14$ dB of spin squeezing in $\sim1$ second with $\sim10^2$--$10^4$ atoms.
This capability exemplifies a new paradigm of using non-equilibrium systems to overcome limitations in quantum metrology, entering a new regime of enhanced sensing with correlated quantum states.
\end{abstract}
...
%The intriguing and non-trivial observation reported here is that weakly interacting fermions in a lattice with dynamics described by the Hubbard model --- which obey fermionic quantum statistics and interact solely via collisions --- can feature all-to-all collective spin interactions that naturally generate spin-squeezed states in a manner robust to realistic experimental imperfections.
%In the spirit of quantum simulation, our work illustrates that short-range interacting systems can be used to emulate  dynamical magnetic behaviors found in arrays of long-range interacting systems (such as arrays of trapped ions\cite{bohnet2016quantum} or cavity QED systems\cite{schleier-smith2010states, norcia2018cavitymediated} with interactions mediated by phonons or photons, respectively), thus enhancing their potential as a metrological resource.
...
%By demonstrating that the precise control of interactions, which becomes possible in the new generation of 3D optical lattice systems with fully quantized motional degrees of freedom, can naturally give rise to metrologically useful correlated many-body fermionic states, our work opens a path to not only generate entanglement but also to use it for a quantum advantage in a world-class sensor.
...
Such an advance will ultimately deliver gains to real-world applications including timekeeping, navigation, communication, and our understanding of the fundamental laws of nature.

% Despite numerous demonstration experiments on entangled states[CITE], with the exception of LIGO\cite{aasi2013enhanced, abbott2016gw150914} no quantum advantage has yet been realized in any state-of-the-art quantum sensing or measurement system.
% By demonstrating that the precise control of interactions, which becomes possible in the new generation of 3D optical lattice systems with quantized motional degrees of freedom in all three directions, can naturally give rise to metrologically useful correlated many-body fermionic states, our work opens a path to not only generate entanglement but also to apply it to world-class sensors which can ultimately deliver gains to real-world applications including timekeeping, navigation, communication as well as advances in our understanding of fundamental laws of nature.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Results}

\subsection{Spin squeezing with the Fermi-Hubbard model.}
...
The spin-squeezed state can be used for a follow-up clock interrogation protocol (see Methods).
}
...
\begin{align}
  ...
  + U \sum_j \hat n_{j,\up} \hat n_{j,\dn},
...
At second order in perturbation theory (see Methods), we thus find that SOC effectively yields a one-axis twisting (OAT) model widely known to generate squeezing dynamics\cite{kitagawa1993squeezed, ma2011quantum}:
\begin{align}
  \hat H_{\t{eff}}/\hbar
  = -\frac{U}{L} \hat{\v S} \c \hat{\v S}
...
\begin{figure}
\centering
\includegraphics{model_benchmarking.pdf}
...
We present a detailed discussion of the role of the harmonic trap in the Supplementary Information, where we demonstrate that the addition of harmonic confinement barely modifies the achievable spin squeezing with currently accessible trapping frequencies.
...
While the OAT model initially generates squeezing faster than the TAT model, the squeezing generation rate of OAT (measured in dB per second) falls off with time, while the squeezing generation rate for TAT remains approximately constant until reaching Heisenberg-limited amount of spin squeezing with $\xi^2_{\t{opt}}\sim N^{-1}$ (see the Supplementary Information)\cite{kitagawa1993squeezed}.
...
For simplicity, we consider the latter in this  work, although the pulsed protocol could provide additional advantages, as explained at the end of the Methods.
...
If the modulation frequency $\omega$ satisfies $\omega\gg N\chi$ and $\J_0\p{2\Omega_0/\omega}=\pm1/3$, where $\chi$ is the OAT squeezing strength in Eqn.~\eqref{eq:H_eff} and $\J_0$ is the zero-order Bessel function of the first kind, then up to (i) an $\sim\hat{\v S}\c\hat{\v S}$ term that contributes only overall phase factors, and (ii) an $\sim\hat S_\z$ term that can be eliminated with a simple dynamical decoupling pulse sequence (more details in Supplementary Information), the effective Hamiltonian becomes $\hat H_{\t{TAT}}^{(+)}/\hbar = (\chi/3)(\hat S_\z^2-\hat S_\x^2)$ or $\hat H_{\t{TAT}}^{(-)}/\hbar = (\chi/3)(\hat S_\y^2-\hat S_\x^2)$ (see Methods), which squeezes an initial state polarized along the $y$ or $z$ axis, respectively.

\subsection{Experimental implementation and practical considerations.}
...
We impose this constraint on $\widetilde{B}/U$ to ensure validity of the OAT Hamiltonian perturbatively derived in the Methods.
...
Sample plots of squeezing over time for particular choices of lattice size ($\ell$) and depth ($V_0/E_{\t{R}}$) are provided in the Supplementary Information.
}
\label{fig:optima_decoherence}
\end{figure}
\end{spacing}

Currently, light scattering from the lattice beams induces decoherence of the clock on a time scale of $\sim10$ seconds\cite{goban2018emergence, hutson2019engineering}, which is much shorter than the natural $^3P_0$ lifetime of $\sim160$ seconds (see the Supplementary Information).
This limitation imposes significant constraints on achievable spin squeezing, as shown in Figure \ref{fig:optima_decoherence} where the maximal squeezing with spin decay in the OAT case was determined using exact expressions for spin correlators derived in Ref.~[\citenum{foss-feig2013nonequilibrium}], while in the TAT case these correlators were determined by solving Heisenberg equations of motion for collective spin operators\cite{perlin2019shorttime} (see the Supplementary Information).
...
In addition to light scattering, $p$-wave losses from inelastic ${}^3 P_0$ collisions\cite{martin2013quantum, zhang2014spectroscopic, bishof2011inelastic} can also degrade the maximum achievable spin squeezing, particularly in shallow lattices.
More details on $p$-wave losses are discussed in the Supplementary Information, where we show that operating at lattices deeper than $8 E_{\t{R}}$ may be necessary to suppress the impact of inelastic collisions on spin squeezing, at the cost of slightly increasing  light scattering.
...
\subsection{Derivation of the effective one-axis-twisting model.}
...
\begin{align}
  ...
  \label{eq:dec_transformation_spin}
\end{align}
As the TAT Hamiltonians $H_{\t{eff}}^{(\pm)}$ are realized in a rotating frame, to properly account for decoherence throughout the TAT protocol one must transform jump operators according to Eqn.~\eqref{eq:dec_transformation_spin}.
...
Nonetheless, we find that for $N=10^2$ ($10^3$) atoms, $\sim5$ (10) $\pi$-pulses in a CPMG (Carr-Purcell-Meiboom-Gill) sequence\cite{carr1954effects, meiboom1958modified} suffice to mitigate the effects of the $S_\z$ term in the TAT protocol (see the Supplementary Information).
...
\subsection{Clock interrogation after squeezing.}
...
For simplicity, we restrict our discussion to the case of squeezing in 1D, as in the main text, with the understanding that a generalization of this discussion to higher dimensions is straightforward.
...
Increasing the lattice depth to turn off SOC is compatible with the current clock interrogation sequence, but is incompatible with ongoing efforts to mitigate light scattering from the lattice beams (currently the primary source of decoherence in the clock; see the Supplementary Information) by use of a shallow lattice\cite{hutson2019engineering}.
...
%(i) both the squeezing and clock interrogation protocols need a clock laser that is homogeneous -- i.e.~with Hamiltonian proportional to $S_\x$,
%(i) lasers with different SOC angles are homogeneous in different gauges, and 
%(ii) spin squeezing is a gauge-dependent quantity.
%More precisely,
...
Given the definitions of the rotated spin operators $S_\x^{(\theta)},S_\x^{(\theta)}$ in Eqn.~\eqref{eq:S_xy_theta}, a suitable candidate for the gauge-switching operator $G_\theta$ is the site-dependent rotation
\begin{align}
...
All data generated and analysed for this study are available from the corresponding authors on reasonable request.
...
Correspondence and requests for materials should be addressed to M.A.~Perlin~(email: mika.perlin@gmail.com) and A.M.~Rey~(email: arey@jilau1.colorado.edu).
