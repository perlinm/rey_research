...
\graphicspath{{./plots/}} % set path for all figures
...
\note{[MP: I removed a factor of 2 in front of $\xi$, as I believe it
  ...
  Peiru's codes.]}
% MP: as an additional side note, I personally recommend the use of
% \braket{X} or (\bk{X}, which I defined as a shorthand) in place of
% \langle X \rangle; I find this is a little easier to read.
% Similarly, you can use \ket{X} (\bra{X}) in place of |X\rangle in
% place of (\langle X|), and \op{X}{Y} in place of |X\rangle\langle Y|
% (where "op" stands for "outer product")
...
\begin{figure}
\centering
\includegraphics[width=3.5in]{MFcontrast.pdf}
\caption{
...
\begin{figure}
\centering
\includegraphics[width=3in]{Fig33.pdf}
...
squeezing time
$t_{\t{opt}}^{\t{OAT}}\approx1.1N^{-2/3}\chi^{-1}\sim\eta
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|c|l|}
    ...
    Primary lattice depth & $V_0$ & 3 $E_R$ \\
...
\item modulate the drive as $\Omega\p{t}=\beta\omega\cos\p{\omega t}$
  with $\omega\gg N\chi$,
\item move into the rotating frame of $-\Omega\p{t}S_x$, and finally
\item shift $H_{\t{drive,eff}}$ by the constant
  $\p{\chi/2}\sp{\J_0\p{2\beta}-1} S^2$, where $\J_0\p{x}$ is the
  zero-order Bessel function of the first kind evaluated at $x$.
\end{enumerate*}
After a secular approximation which relies on $\omega\gg N\chi$, this
procedure results in the effective Hamiltonian
\begin{align}
  H_{\t{drive,eff}}^{\t{mod}}
  = \f{\chi}{2} \sp{3\J_0\p{2\beta}-1} S_z^2
  + \f{\chi}{2} \sp{1-\J_0\p{2\beta}} \p{S_z^2 - S_x^2},
\end{align}
where choosing the modulation index $\beta$ such that
$\J_0\p{2\beta}=1/3$ or $\J_0\p{2\beta}=-1/3$ respectively results in
the two-axis twisting Hamiltonians
\begin{align}
  H_{\t{TAT}}^{z,x} = \f{\chi}{3} \p{S_z^2 - S_x^2},
  &&
  H_{\t{TAT}}^{y,x} = \f{\chi}{3}\p{S_y^2 - S_x^2}.
  \label{eq:H_TAT_drive}
\end{align}
Given the set of parameters in Table \ref{tab:parameters}, the
two-axis twisting Hamiltonian $H_{\t{TAT}}^{z,x}$ can be achieved with
$\omega=20\times2\pi$ Hz and $\beta=0.9057$.  The optimal squeezing
