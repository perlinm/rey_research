...
intensive investigations of rich phenomena, including studies of
...
long-range collisional, photon-, or phonon-mediated interactions.
...
jumps that generate large numbers of distinct trajectories that need
to be averaged.
...
will see in the following sections, our choice of basis elements
$\S_{\v m}=S_+^{m_+} S_\z^{m_\z} S_-^{m_-}$ allows us to accurately
compute spin correlators to times that are relevant e.g.~for spin
squeezing protocols.
...
method recently developed in ref.~\cite{zhang2018montecarlo}.  In this
work, these exact and quantum trajectory simulations will be used to
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    ...
    $M=40$ (dashed lines).  Solid circles mark the times at which the
    latter method gives an unphysical result with $\xi^2<0$.  Both
    methods agree exactly in the OAT case for all times shown.}
  \label{fig:coherent}
\end{figure}

\begin{figure}
  \centering
  % \includegraphics{weak_decoherence.pdf}
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$ with
    ...
    circles mark the times at which the latter method gives an
...
Ref.~[\citenum{schweigler2017solving}]?
