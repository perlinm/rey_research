...
numbers of distinct trajectories over which to average.  [Unclear?]
...
\end{align}
The commutator in Eq.~\eqref{eq:EOM} can be determined from the
...
entries $T^k_{\v m\v n}$ for $k=0,1,\cdots,M$ now explicitly tell us
...
\begin{align}
  ...
  \label{eq:X_hyper}
\end{align}
where $\alpha_{N\ell n}$ and $\beta_{N\ell mn}$ are defined in terms
of the generalized hypergeometric function $\F$ by
\begin{align}
  \alpha_{N\ell n}
