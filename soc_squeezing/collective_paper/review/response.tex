...
\item Concerning the final question, about the advantages of the TNT
  ...
  % One consequence of applying our method in the large system, strong
  % decoherence regime in figure 2 is that there is no way (barring a
  % detailed analysis of error and breakdown in the TST expansion) to
  % completely rule out numerical errors, as there are no other
  % simulation methods available to corroborate the results.  We had
  % hoped that the benchmarking in figure 1 and the discussion of
  % breakdown mechanisms would assure readers that the TST expansion
  % self-diagnoses its sudden departure from virtually exact results,
  % thereby increasing confidence that the TST expansion can be
  % trusted when no other methods are available.
