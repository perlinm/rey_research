...
We can expand Heisenberg operators in a Taylor series about time $t=0$
to write
\begin{align}
  \S_{\v m}\p{t}
  % = \left. \exp\p{t \f{d}{d\tau}} \S_{\v m}\p{\tau} \right|_{\tau=0}
  &= \sum_{k\ge0} \f{t^k}{k!}
  \left. \f{d^k}{d\tau^k} \S_{\v m}\p{\tau} \right|_{\tau=0} \\
  &= \sum_{k\ge0} \f{t^k}{k!}
  \sum_{\v n} T^k_{\v m\v n} \S_{\v n}\p{0},
...
$\bk{\S_{\v m}\p{t}}$ to a fixed order $M$.  Note that the relation
...
number of initial-time expectation values $\bk{\S_{\v m}\p{0}}$ and
...
allows computation of the correlator $\bk{\S_{\v m}\p{t}}$ to a
...
initial-time correlators $\bk{\S_{\v n}\p{0}}$ vanish unless
$n_+=n_-=0$ (see Appendix \ref{sec:initial_conditions}), so there is a
substantially smaller number of terms with non-zero contribution to
the correlator $\bk{\S_{\v m}}$, and therefore a substantially smaller
number of non-zero terms that are neglected in the truncated Taylor
expansion.
...
Ref.~[\citenum{he2019engineering}], exemplifying a concrete
...
correlation functions in collective spin systems.
...
% Compute some OTOCs?  There is a small chance that doing this will
% take a prohibitively long amount of time...

% Possibility: compare ``true'' OTOCs with decoherence
% vs.~``practical'' OTOCs in experiments with decoherence and
% time-reversal of coherent dynamics.
