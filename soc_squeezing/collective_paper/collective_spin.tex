...
(in Appendices \ref{sec:identities}--\ref{sec:prod_general}) is one of
the main technical results of this work, which we hope will empower
...
\end{align}
with scalar coefficients $\O_{\v m}\in\mathbb{C}$.  If $\hat\O$ is
...
$T_{\v m\v n}\in\mathbb{C}$ defined by
\begin{align}
...
$\A_-=\set{\hat s_-^{(j)}:j=1,2,\cdots,N}$.  The commutator in
...
\ref{sec:identities}--\ref{sec:prod_general}, and the effects of
...
for computing the expectation value $\bk{\hat\O\p{t}}$ to a fixed
order $M$.  Note that using the relation
...
Eq.~\eqref{eq:order_truncation} only allows for computation of the
...
$\bk{\hat S_\a \hat S_\b}$.  For the unitary dynamics discussed in this
work, these correlators are obtainable via exact simulations of
...
to benchmark the TST expansion in Eq.~\eqref{eq:order_truncation}.
...
both unitary dynamics (Figure \ref{fig:squeezing_coherent}), as well
as non-unitary dynamics in the presence of spontaneous decay,
excitation, and dephasing of individual spins at rates $\chi$ (Figure
\ref{fig:squeezing_incoherent}), respectively described by the sets of
jump operators $\A_\a=\set{\hat s_\a^{(j)}}$ with corresponding
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$ with
    ...
    the TST expansion in Eq.~\eqref{eq:order_truncation} with $M=35$.
    Solid circles mark the times at which the TST expansion gives an
...
\begin{figure*}
  ...
  \caption{Magnitude and phase of the two-time connected correlator
    ...
    jump operators $S_\a$ or $s_\a^{(j)}$.  Collective decoherence
    rates ($\Gamma_\a$) were chosen to be comparable to the OAT
    squeezing strength $\chi$, while single-spin decoherence rates
    ($\gamma_\a$) were scaled by an additional factor of $S=N/2$ in
    order to yield comparable effects on the correlator $C\p{t}$ at
    the times shown.  At short times, the correlations captured by
    $C\p{t}$ are essentially insensitive to collective dephasing
    ($\Gamma_\z$), while the magnitude $\abs{C\p{t}}$ is suppressed by
    other forms of decoherence.  The type of decoherence (namely spin
    decay, excitation, or dephasing) leaves a signature on the phase
    $\phi\p{t}$ of $C\p{t}$.}
  \label{fig:two_time}
\end{figure*}
