...
\item simulation results can only be trusted up to the time at which
  the initial values of operators $\hat\S_{\v m}$ with weight
...
\end{align}
with $\delta_{\v m\v n}=1$ if $\v m=\v n$ and zero otherwise.  For
...
\begin{align}
  ...
  \label{eq:TST}
\end{align}
We refer to Eq.~\eqref{eq:TST} as the truncated short-time (TST)
...
$\abs{\bk{\hat S_{\v n}\p{t}}}\lesssim S^{\abs{\v n}}$ (where
$S\equiv N/2$).  Finally, the TST expansion is essentially
perturbative in the time $t$, which implies that its validity as a
formal expansion eventually breaks down.  Precisely characterizing the
implications of these last two considerations for the TST expansion
requires additional analysis that we defer to future work.  An
investigation of connections between the TST expansion and past work
related to the Mori formalism\cite{mori1965continuedfraction,
  annett1994recursive}, for example, might answer questions about the
breakdown and convergence of the TST expansion.  As we show from
benchmarks of the TST expansion in Section \ref{sec:squeezing},
however, a detailed understanding of breakdown is not necessary to
diagnose the breakdown time $t_{\t{break}}^{(M)}$ beyond which the TST
expansion yields inaccurate results.  Empirically, we find that going
beyond order $M\approx35$ yields no significant gains in all cases
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    ...
    \sref{fig:squeezing_dec} non-unitary dynamics, computed using
...
with decoherence in Ref.~[\citenum{he2019engineering}], exemplifying a
...
commutators initially vanish by construction through a choice of
...
\begin{figure*}
  ...
  \caption{The two-time connected correlator $C\p{t}$ and squared
    ...
    squeezing Hamiltonians in Eqs.~\eqref{eq:OAT}--\eqref{eq:TNT}.
    Results are shown for both unitary dynamics (solid lines) and
    non-unitary dynamics with
    $\gamma_-=\gamma_+=\gamma_\z=\gamma_0=100\chi$ (dashed lines),
...
with the TST expansion.  These examples are straightforward to extend
to equilibrium settings and spin-boson systems.
...
\begin{acknowledgments}
  ...
  PFC-1734006; and the National Institute of Standards and Technology
  (NIST).
\end{acknowledgments}
...
\begin{align}
  ...
  \p{1 - n + 2\mu S_\z} S_\mu^k.
\end{align}
We now need to rearrange the operators in $C_{mn;\mu}$ into a standard
...
\end{align}
which satisfy the same commutation relations as the standard Pauli
...
bilinear in particle creation or annihilation operators.  When working
on the level of a spin model, therefore, we can at best only {\it
...
\begin{figure}
  \centering
  \includegraphics{unitary_small.pdf}
...
\begin{figure}
  \centering
  \includegraphics{correlator_example.pdf}
...
produce more squeezing than the TNT model with weak decoherence, this
...
\begin{figure}
  \centering
  \includegraphics{sqz_dec_scaling.pdf}
