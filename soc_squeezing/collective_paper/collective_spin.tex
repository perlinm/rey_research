...
gasses\cite{martin2013quantum, smale2019observation}, trapped
...
sensors\cite{jones2009magnetic}, among others.  These implementations
have triggered studies of a variety of rich subjects, including
quantum criticality\cite{latorre2005entanglement,
  alcalde2007functional, morrison2008dissipationdriven,
  morrison2008collective, sarandy2009classical, wang2012quantum,
  kessler2012dissipative, majd2014lmg}, non-equilibrium
phenomena\cite{walls1978nonequilibrium, morrison2008dynamical,
  ...
  lang2018concurrence, smale2019observation}, and precision
...
spin system and its environment, initially $\hat\E_\O\p{0}=0$.  The
expectation values of Heisenberg operators evolve according to
%
\begin{align}
  \f{d}{dt} \bk{\hat\O\p{t}}
  ...
  \label{eq:EOM}
\end{align}
with a time derivative (super-)operator $\check T$ whose matrix
elements $T_{\v m\v n}\in\C$ are defined by
\begin{align}
...
Eq.~\eqref{eq:TST} only allows us to compute the expectation value
$\bk{\hat\O\p{t}}$ to a maximal time $t_{M_{\t{max}}}$.  As we will
see in the following section, $M_{\t{max}}=35$ will suffice for the
computation of collective spin correlators up to times that are
...
all unit vectors $\v v$ orthogonal to the mean spin vector
$\bk{\uv S}$.  This squeezing parameter is entirely determined by one-
and two-spin correlators of the form $\bk{\hat S_\a}$ and
...
$\lvert\bk{\hat\S_{\v n}\p{t}}\rvert\gtrsim S^{\abs{\v n}}$.  The
sudden and drastic departure from virtually exact results occurs
...
large power of $t$, so their truncation will have a drastic effect on
the estimated value of $\bk{\hat\O\p{t}}$.  This breakdown mechanism
...
initial-time correlators $\bk{\hat\S_{\v m}}$ vanish unless
...
strong-decoherence regimes of large systems that are entirely
inaccessible to other methods.  As an example, Figure
...
e.g.~in Eq.~\eqref{eq:two_time} generally vanish, quadratic
contributions that would otherwise appear in Eq.~\eqref{eq:four_point}
generally do not.  We exclude these contributions because their
calculation generally requires additional assumptions about the
environment (i.e.~beyond Markovianity) that would be a digression for
the purposes of the present work; see Ref.~\cite{blocher2019quantum}
for a detailed discussion of noise terms and the quantum regression
theorem underlying the calculation of multi-time correlators.
%
...
\begin{align}
  \bk{T \S_{\v n}}
  = i\bk{\sp{H,\S_{\v n}}_-}
  + \sum_\J \gamma_\J \bk{\D\p{\J} \S_{\v n}}
  = \sum_{\v m} \bk{\S_{\v m}} T_{\v m\v n}.
\end{align}
...
$\widetilde\O\p{t}$ and $\O\p{t}$ that respectively satisfy
$\bk{\widetilde\O\p{t}}=\bk{e^{t\widetilde T}\O}$ and
$\bk{\O\p{t}}=\bk{e^{tT}\O}$, we thus find that for sufficiently small
times $t$ and weak perturbations $V$,
\begin{align}
  \bk{\widetilde\O\p{t} - \O\p{t}}
  = \Bk{\p{e^{t\widetilde T} - e^{tT}} \O}
