...
$\bk{\O\p{t}}$ to a fixed order $M$.  Note that using the relation
...
accessible via the truncated Taylor expansion of Heisenberg operators,
we consider the calculation of collective-spin two-time correlation
functions.  In particular, we consider the effect of decoherence on
the short-time behavior of the two-time connected correlator
\begin{align}
  C\p{t}
...
matter\cite{tucker2018shattered}.  Related multi-time correlators,
...
\begin{figure*}
  ...
  \caption{Real (solid lines) and imaginary (dashed lines) parts of
    ...
    can change sign or grow under single-spin ($\gamma_+$) or
    collective ($\Gamma_+$) excitation.}
  \label{fig:two_time}
\end{figure*}
...
decoherence, but seems relatively stable to collective dephasing (with
jump operator $S_\z$), and can qualitatively change behavior under in
the presence of single-spin or collective excitation (with jump
operators $s_+^{(j)}$ or $S_+$).  While we do not have an operational
understanding of the correlations captured by $C\p{t}$ in the context
of spin squeezing, Figure \ref{fig:two_time} serves as an example for
the type of quantity and behavior that is accessible at short times
with the truncated Taylor expansion.  This example is straightforward
to extend to multi-time and out-of-time-ordered correlators, as well
as equilibrium settings.
...
% different initial conditions <--> steady states or pseudo-steady states in models with a large separation of scales
% OTOCs; theoretical interest, scrambling; benchmarking "practical" OTOCs (with time-reversal of only coherent dynamics) against "true" OTOCs
