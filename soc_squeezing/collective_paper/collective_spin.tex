...
\section{Non-equilibrium linear response and two-time correlation
  functions}
\label{sec:NLR}

As a final example, we consider the calculation of two-time
correlation functions, which allow us to extract the short-time linear
response of Heisenberg operators to perturbations of a Hamiltonian.
Consider a target Hamiltonian $H$ perturbed by an operator $V$ with
$\norm{V}\ll\norm{H}$, such that the net Hamiltonian is
...
\begin{align}
  e^{t\widetilde T}
  \approx e^{tT} + \f12 i t \sp{\hat V, e^{t T}}_+.
\end{align}
...
\begin{align}
  \widetilde\O\p{t}
  \approx \O\p{t} + \f12 i t \p{\sp{V,\O\p{t}}_- + \sp{V,\O}_-\p{t}}.
\end{align}
