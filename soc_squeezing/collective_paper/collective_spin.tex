...
drastic departure that can be diagnosed by inspection.
% todo: comment on diagnosing breakdown by examining individual correlators, rather than squeezing
This drastic departure occurs because the TST expansion neglects
high-weight (i.e.~large-$\abs{\v m}$) operators $\hat\S_{\v m}$ whose
...
\begin{figure}
  ...
  $C\p{t}\equiv\abs{C\p{t}}\exp\sp{i\phi\p{t}}$. \label{fig:two_time}]
  {\includegraphics{two_time.pdf}} \\
  \subfloat[Squared commutator $D\p{t}$. \label{fig:four_point}]
...
% todo: add commentary/discussion about growth of C(t)
...
$\gamma_-=\gamma_+=\gamma_\z=100\chi$.  In the case of TAT and TNT,
after an initial ramp-up period these correlators grow exponentially
as $\sim\exp\p{a\chi t}$ with $a\sim N$.  The growth of these
correlators is definitively sub-exponential for OAT.  In the case of
unitary time evolution under OAT in particular, to an excellent
approximation we find that $D\p{t}=D\p{0}+\p{b\chi t}^2$ with
$b=N+O\p{1}$.  As may be expected, the growth rates of $C\p{t}$ and
$D\p{t}$ are generally suppressed by decoherence.  Figure
\ref{fig:multi_time} serves as an example for the type of behavior
that is accessible at short times with the TST expansion.  These
examples are straightforward extend to equilibrium settings and
spin-boson systems.
