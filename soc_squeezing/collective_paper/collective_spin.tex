...
$M_{\t{max}}=40$ will suffice for the accurate computation of
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    ...
    Eq.~\eqref{eq:order_truncation} with $M=40$ (darker dashed lines).
...
Eq.~\eqref{eq:order_truncation} with $M=40$.  Squeezing is shown for
both coherent dynamics (Figure \ref{fig:squeezing_coherent}), as well
as dynamics in the presence of uncorrelated emission, absorption, and
...
shows squeezing of $N=10^4$ spins initially in $\ket{\X}$, with decay
and dephasing of individual spins at rates
$\gamma_-=\gamma_\z=100\chi$.  Computations of this sort were used to
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    under incoherent evolution with decay and dephasing of individual
    spins at rates $\gamma_-=\gamma_\z=100\chi$, computed using the
    truncated Taylor expansion in Eq.~\eqref{eq:order_truncation} with
    $M=40$.  Solid circles mark the times at which the truncated
...
Compare ``true'' OTOCs with decoherence vs.~``practical'' OTOCs in
experiments with time-reversal of coherent dynamics and decoherence
