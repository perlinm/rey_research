...
\begin{abstract}
  ...
  collective spin systems with permutationally-symmetric decoherence.
  ...
  can be truncated at finite order to obtain nearly exact results at
  short times that are relevant for metrological applications such as
  spin squeezing.  In order to evaluate the expansion for Heisenberg
  operators, we compute the structure constants of a collective SU(2)
  operator algebra.  We demonstrate the utility of our method by
  computing spin squeezing and two-time correlation functions in
  strong-decoherence regimes that are inaccessible to existing
  numerical methods.  Our method is straightforward to generalize to
  the case of a collective spin coupled to bosonic modes, as can occur
  in trapped ion and optical cavity experiments, and may be used to
  compute quantities such as out-of-time-ordered correlators that
  contain short-time signatures of quantum chaos and information
  scrambling.
\end{abstract}
...
We can formally expand Heisenberg operators $\O\p{t}$ in a Taylor
...
\begin{figure}
  \centering
  \subfloat[Squeezing with coherent dynamics
...
(i.e.~large-$\abs{\v m}$) operators $\S_{\v m}$ whose contributions to
a Heisenberg operator $\O\p{t}$ of interest eventually become
non-negligible.  For sufficiently large times $t$, these individual
contributions may generally be large compared to the value of
$\bk{\O\p{t}}$, in which case their truncation will yield non-physical
results.  This breakdown mechanism is also the reason for prolonged
agreement between the TST expansion and benchmarking methods when
squeezing the initial state $\ket{-\Z}$ rather than $\ket{\X}$: for
initial states $\ket{-\Z}$, all initial-time correlators
...
correlators, as well as equilibrium settings.
...
\begin{acknowledgments}
  ...
  Office (ARO) grant W911NF-16-1-0576; the National Science Foundation
  (NSF) grant PHY-1820885; JILA-NSF grant PFC-173400; and the National
