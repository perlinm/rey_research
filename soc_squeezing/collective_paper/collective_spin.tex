...
\title{Structure constants and short-time simulations of open
  collective quantum spin systems}
...
for a range of applications including quantum-enhanced metrology,
quantum simulation, and quantum information.  Theoretical interest in
such systems dates back to the mid-twentieth century with the
introduction of the Dicke model\cite{dicke1954coherence} that
describes atoms cooperatively interacting with a single mode of the
radiation field and the Lipkin-Meshkov-Glick (LMG) model, a toy model
for testing many-body approximation methods in contemporary nuclear
...
states\cite{wineland1992spin, kitagawa1993squeezed, takano2009spin,
  ...
  bohnet2014reduced, cox2016deterministic, hosten2016measurement,
  hosten2016quantum, lewis-swan2018robust, norcia2018cavitymediated,
  ma2011quantum} that allow for measurement errors
...
that need to be averaged to compute expectation values.
...
operator algebra; the calculation of these structure constants is one
of the main technical results of this work, which we hope will enable
future studies to develop new analytical insights into the dynamics of
...
determined from the structure constants $f_{\v n\v m\v\ell}$ of the
general product
...
fixed maximal truncation order $M_{\t{max}}$ is accessible with
limited computational resources, such that
Eq.~\eqref{eq:order_truncation} only allows computation of the
correlator $\bk{\S_{\v m}}$ to a maximal time $t_{M_{\t{max}}}$.  The
maximal accessible truncation order $M_{\t{max}}$ generally depends on
the choice of operator basis for collective spin systems, which we
discuss briefly in Section \ref{sec:breakdown} and Appendix
\ref{sec:new_basis}.  As we will see in the following sections, our
choice of basis elements $\S_{\v m}=S_+^{m_+} S_\z^{m_\z} S_-^{m_-}$
allows us to accurately compute spin correlators to times that are
...
benchmark the expansion in Eq.~\eqref{eq:order_truncation}.

\begin{figure}
  \centering
  \subfloat[Squeezing under coherent dynamics \label{fig:coherent}]
  {\includegraphics{coherent.pdf}} \\
  \subfloat[Squeezing with decoherence:
  $\gamma_-=\gamma_+=\gamma_\z=\chi$ \label{fig:incoherent}]
  {\includegraphics{decoherence_weak.pdf}}
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    under \sref{fig:coherent} coherent and \sref{fig:incoherent}
    incoherent evolution, as computed using exact methods (solid
    lines), quantum trajectory simulations (dots), and the expansion
    in Eq.~\eqref{eq:order_truncation} with $M=40$ (darker dashed
    lines).  Solid circles mark the times at which the last of these
    methods gives an unphysical result with $\xi^2<0$.}
...
both benchmarking simulations and the prescription in
Eq.~\eqref{eq:order_truncation} with $M=40$.  This squeezing is shown
for both coherent dynamics (Figure \ref{fig:coherent}), as well as
dynamics in the presence of uncorrelated emission, absorption, and
dephasing of individual spins at a rate $\chi$ (Figure
\ref{fig:incoherent}), respectively described by the sets of jump
operators $\J_n=\set{s_n^{(j)}}$ with corresponding decoherence rates
$\gamma_n=\chi$ for $n\in\set{-,+,\z}$.  The squeezing results shown
in Figure \ref{fig:benchmarking} were computed in a rotated basis with
$\p{s_\z,s_\x}\to\p{s_\x,-s_\z}$ and
$\ket\X\to\ket{-\Z}\equiv\ket\dn^{\otimes N}$; the only effect of this
rotation on the results presented in this figure is to roughly double
the time for which all TNT results agree.  The reason for different
results in rotated bases has to do with the breakdown of the expansion
in Eq.~\eqref{eq:order_truncation}, which we discuss further in
...
\section{Out-of-time-ordered correlators}

Compare ``true'' OTOCs with decoherence vs.~``practical'' OTOCs in experiments with time-reversal of coherent dynamics and decoherence

Discuss extension of this work to spin-boson systems
