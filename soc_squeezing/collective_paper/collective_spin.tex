...
with a corresponding decoherence rate $\gamma_\J$; and is $\D\p{\J}$
is a Heisenberg-picture dissipator, or Lindblad superoperator, defined
by
\begin{align}
  \D\p{\J} \O
...
allows us to solve it using standard numerical methods.  Some initial
conditions for this system of differential equations, namely
expectation values of collective spin operators with respect to
spin-polarized (Gaussian) states that are generally simple to prepare
experimentally, are provided in Appendix \ref{sec:initial_conditions}.
...
of matrix elements $T_{\v m\v n}$ that we may need to explicitly
...
\begin{align}
  ...
  H_{\t{TAT}} &= \chi \f13 \p{S_\z^2 - S_\y^2}, \label{eq:TAT} \\
...
benchmark the truncated Taylor expansion in
Eq.~\eqref{eq:order_truncation}.
...
\begin{figure}
  ...
  \caption{Spin squeezing of $N=10^4$ spins initially in $\ket\X$
    ...
    (dots), and the truncated Taylor expansion in
    Eq.~\eqref{eq:order_truncation} with $M=35$ (darker dashed lines).
    Solid circles mark the times at which the truncated Taylor
...
rates $\gamma_-=\gamma_+=\gamma_\z=100\chi$.  These results show that
TNT is much more robust to this kind of decoherence than OAT or TAT, a
finding that could not be deduced from the weak-decoherence
simulations presented in Figure \ref{fig:benchmarking}.
Strong-decoherence computations of this sort were also used to put
lower bounds on theoretically achievable spin squeezing via TAT with
decoherence in Ref.~[\citenum{he2019engineering}], exemplifying a
concrete application of the truncated Taylor expansion and the
...
\section{Short-time linear response and two-time commutators}
\label{sec:linear_response}

As another example of physics that is numerically accessible via the
truncated Taylor expansion of Heisenberg operators, we consider the
calculation of two-time commutators, and their relation to the
short-time linear response of Heisenberg operators to perturbations of
a Hamiltonian.  Consider a target Hamiltonian $H$ perturbed by an
operator $V$ with $\norm{V}\ll\norm{H}$, where $\norm{\O}$ denotes the
...
\end{align}
Through quadratic order in the time $t$, we can say that
\begin{align}
  e^{t\widetilde T}
  \approx \f12\sp{e^{it\hat V}, e^{tT}}_+,
\end{align}
which at first order in the perturbation $\hat V$ implies
\begin{align}
  e^{t\widetilde T}
  \approx e^{tT} + \f12 it \sp{\hat V, e^{t T}}_+.
\end{align}
...
\begin{align}
  ...
  \label{eq:response}
\end{align}
Short-time linear response functions of this sort,
i.e.~$\widetilde\O\p{t}-\O\p{t}$, can thus be used to experimentally
probe two-time commutators $\sp{V,\O\p{t}}_-$ that contain information
about the spreading of information across many-body degrees of freedom
in a collective spin system.  The numerical methods presented in this
paper can in turn benchmark such experiments, albeit at short times,
even in the presence of strong decoherence.
...
% extension to spin-boson and cavity systems / dicke model / chaos?
% different initial conditions <--> pseudo-steady states in models with a large separation of scales
% OTOCs; theoretical interest; comparing "true" OTOCs against "practical" ones / time reversal of coherent dynamics
