...
def get_optimum_parameters():
    ...
    for depth in depths:
        ...
        for confinement in confinements:
            ...
            sqz_opt.at[depth, confinement] = params[2]
            t_opt.at[depth, confinement] = params[3]
...
def make_plot(U_int, phi_opt, values, label, vmin = None, vmax = None):
    ...
    plt.pcolormesh(depths, confinements, values.T, vmin = vmin, vmax = vmax,
    ...
    plt.contourf(depths, confinements, t_J_shade_vals.T, fill_regions,
                 colors = "none", hatches = [ t_J_hatch, None, None ], zorder = 1)
    labels.append(r"$t_{{\mathrm{{opt}}}} J_T/2\pi > {}$".format(t_J_T_cap))
    hatches.append(4*t_J_hatch)
    ...
    legend = plt.legend(handles = handles, ncol = 2, framealpha = 0,
