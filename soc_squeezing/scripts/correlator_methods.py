...
def factorial(nn, vals = {}):
    ...
    vals[nn] = scipy_factorial(nn, exact = True)
    return vals[nn]
...
def binom(nn, kk, vals = {}):
    ...
    vals[nn,kk] = scipy_binom(nn, kk, exact = True)
    return vals[nn,kk]
...
def zeta(mm, nn, pp, qq, vals = {}):
    ...
    sum_vals = [ stirling(pp,ss) * binom(ss,qq) * (mm+nn-2*pp)**(ss-qq)
                 for ss in range(qq,pp+1) ]
    val = (-1)**pp * 2**qq * np.sum(sum_vals, dtype = int)
...
def factors_X(SS, kk, ll, nn):
    num = factorial(SS - kk)
    den = factorial(SS + kk) * factorial(SS - kk - ll) * factorial(SS - kk - nn)
    return frac(num, den)
factors_X = np.vectorize(factors_X)

# correlator < X | S_+^ll S_\z^mm S_-^nn | X >
def op_val_X(op, SS, vals = {}):
    if not type(SS) is int:
        print("calculating <X|O|X> is only allowed for integer total spin")
        exit()
    ...
    prefactor = frac(factorial(int(2*SS)), 2**(int(2*SS)))
    k_vals = np.arange(-SS,SS-ll+0.5, dtype = frac)
    val = prefactor * ( k_vals**mm * factors_X(SS,k_vals,ll,nn) ).sum()
...
def op_val_Z_p(op, SS, vals = {}):
    ...
    num = factorial(int(2*SS)) * factorial(nn)
    den = factorial(int(2*SS)-nn)
    val = (SS-nn)**mm * frac(num, den)
    vals[ll,mm,SS] = val
    return val
...
def op_image_decoherence_Q_individual(op, SS, dec_vec, mu):
    ...
    image_K = {}
    if gg_zp + gg_mz != 0:
        image_K = binom_op(ll+1, mm, nn, 1, frac(mu,2) * (gg_zp + gg_mz))
    ...
    if gg_zp != 0 and nn != 0:
        ...
        coefficients = [ -2*SS+2*ll+frac(3,2)*(nn-1), 1 ]
...
def get_dec_vecs(dec_rates, dec_mat):
    ...
    dec_vecs = []
    for jj in range(3):
        dec_vec_g = dec_mat[:,jj] * frac(np.sqrt(dec_rates[0][jj]))
        dec_vec_G = dec_mat[:,jj] * frac(np.sqrt(dec_rates[1][jj]))
...
def compute_correlators(spin_num, order_cap, chi_times, initial_state, h_vec,
    ...
    if state_dir == "Z":
        ...
        if state_dir == "Y":
            init_val_sign = lambda op : (-1)**op[1] * (cfrac(0,1j)*mu*nu)**(op[0]-op[2])
