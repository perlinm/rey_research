...
$n$-level subsystems, e.g.~an array of $n$-level nuclear spins, we
identify the set $\Pi\equiv\set{\Pi_\sigma}$ of operators that permute
subsystems, labeled by corresponding elements $\sigma\in\SS_N$ of the
symmetric group $\SS_N$ of order $N$.  We refer to simultaneous
eigenstates of all $\Pi_\sigma\in\Pi$ as {\it collective states}.  The
subset of collective states that have eigenvalue $1$ with respect to
all permutation operators $\Pi_\sigma\in\Pi$ is the {\it maximally
  symmetric collective (MSC)} manifold $\M_\MSC$, which is precisely
the Dicke manifold in the case of two-level subsystems ($n=2$).  The
MSC manifold is spanned by a basis $\set{\ket{\v m}}$ of MSC states
that can be uniquely identified by a list of integers
$\v m=\p{m_0,m_1,\cdots,m_{n-1}}\in\ZZ_{N+1}^n$, where $m_k$ indicates
the total occupation number of subsystem state $k$, and
$\sum_k m_k=N$.  The Dicke manifold of $N$ qubits, for example, is
spanned by the Dicke states $\set{\ket{m_0,m_1}}$, where $m_0+m_1=N$
and the integer $m_0$ ($m_1$) indicates the occupation number of the
qubit state $\ket{0}$ ($\ket{1}$).  The dimension of the MSC manifold
is determined by the number of ways to assign $N$ sub-systems to $n$
states, which is ${N+n-1 \choose n-1}$.

The MSC manifold has two key properties.  First, the MSC manifold is
closed under the action of unitaries generated by collective drive
operators $\D_{LM}$, as well as linear combinations and products
thereof.  Closure of the MSC manifold under collective dynamics is a
straightforward consequence of the permutational symmetry obeyed by
collective operators.  Second, the MSC manifold is the ground-state
manifold of the collective SU($n$)-symmetric interaction Hamiltonian
$H_{\t{int}}$ in \eqref{eq:H_int_spin}.  To illuminate the spectral
properties of $H_{\t{int}}$, we can expand
\begin{align}
  H_{\t{int}}
  = -\f{u}{2} \sum_{p,q,\mu,\nu} S_{\mu\nu}^{(p)} S_{\nu\mu}^{(q)}
  = -u\sum_{\substack{p<q\\\mu,\nu}} S_{\mu\nu}^{(p)} S_{\nu\mu}^{(q)}
  - \f{u}{2} \sum_{p,\mu,\nu} S_{\mu\nu}^{(p)} S_{\nu\mu}^{(p)}
  = -u\sum_{p<q} \Pi_{pq} - \f12 N n u
  \simeq -u\sum_{p<q} \Pi_{pq},
  \label{eq:H_int_perm}
\end{align}
where $\Pi_{pq}\in\Pi$ permutes subsystems $p$ and $q$, and $\simeq$
denotes equality up to an overall constant with no physical
consequence.  The form of $H_{\t{int}}$ in \eqref{eq:H_int_perm}
reveals that its eigenstates can be classified by their symmetries
under the permutation of subsystems.  In particular, the ground-state
manifold of $H_{\t{int}}$ consists of states that have eigenvalue 1
with respect to all pair-wise permutation operators $\Pi_{pq}$, which
is precisely the MSC manifold $\M_\MSC$.  The interaction energy of
states $\ket{\psi_\MSC}\in\M_\MSC$ is (up to an overall constant)
simply $-{ N\choose 2 } u = -\p{1/4}N\p{N-1} u$.
