...
important feature of $\M_\FS$ is that it is closed under the action of
...
\begin{align}
  ...
  \label{eq:pert}
\end{align}
where $\P_0$ is a projector onto the fully symmetric manifold $\M_\FS$
and $\ket{E_\varepsilon}$ is an eigenstate of the interaction
...
\end{align}
where $A_L^{(\phi)}$ is defined in \eqref{eq:A_L_phi}, and scales as
...
\begin{align}
  ...
  &&
  \EE_q\sp{X\p{q}} \equiv \f1N \sum_q X\p{q}.
  \label{eq:H_eff_1}
\end{align}
Here $\EE_q\sp{X\p{q}}$ is an average of $X\p{q}$ over all occupied
...
by the SOC Hamiltonian $H_{\t{lat}}^{(\phi)}$ when $\abs{\phi}\ll1$,
we expand $w_L\p{q}$ in \eqref{eq:H_lat_SOC_A} as
\begin{align}
  w_L\p{q} = \f12 \sum_{s\in\set{\pm1}} \chi_{Ls} e^{isq},
  &&
  \chi_{Ls} \equiv \p{-i}^{L\,\t{mod}\,2} s^L,
\end{align}
which implies that
\begin{align}
  H_{\t{lat}}^{(\phi)}
...
\end{align}
where $\E$ is a sum over excited-state projectors with energetic
suppression factors, defined in \eqref{eq:pert}.

\subsubsection{Eliminating excited-state projectors in $\E$}

In order to simplify \eqref{eq:H_eff_2_start}, we first note that the
image of any fully symmetric state $\ket{\psi_{\FS}}\in\M_\FS$ under
the action of the ``spin-wave'' operator $\Z_{Ks}$ is an eigenvector
of the interaction Hamiltonian $H_{\t{int}}=-u\sum_{p<q}\Pi_{pq}$.  To
substantiate this claim, we expand
\begin{align}
  \sum_{p<q} \Pi_{pq} \Z_{Ks} \ket{\psi_\FS}
  = \sum_{\substack{p<q\\k\notin\set{p,q}}} e^{isk}
  \Pi_{pq} D_{K,0}^{(k)} \ket{\psi_\FS}
  + \sum_{\substack{p<q\\k\in\set{p,q}}} e^{isk}
  \Pi_{pq} D_{K,0}^{(k)} \ket{\psi_\FS}.
\end{align}
...
\begin{align}
  ...
  D_{K,0}^{(k)} \ket{\psi_\FS}
  = { N-1 \choose 2 } \Z_{Ks} \ket{\psi_\FS}.
\end{align}
...
\begin{align}
  \sum_{\substack{p<q\\k\in\set{p,q}}} e^{isk}
  \Pi_{pq} D_{K,0}^{(k)} \ket{\psi_\FS}
  = \f12 \sum_{\substack{p\ne q\\k\in\set{p,q}}} e^{isk}
  \Pi_{pq} D_{K,0}^{(k)} \Pi_{pq}^\dag \Pi_{pq} \ket{\psi_\FS}
  = \sum_{q\ne k} e^{isk} D_{K,0}^{(q)} \ket{\psi_\FS},
\end{align}
where for $s\in\set{\pm1}$ we can simplify the sum
$\sum_{k\ne q} e^{isk} = -e^{isq}$,\footnote{We assume for now that
  $\sum_k e^{\pm ik}=0$, which is only valid when all quasi-momenta
  are occupied, such that the sum $\sum_k$ runs over all angles
  $k\in\ZZ_N\times2\pi/N$.  We will consider the slightly more
  complicated case with $\sum_k e^{\pm ik}\ne0$ later in this
  section.}  so
\begin{align}
  \sum_{\substack{p<q\\k\in\set{p,q}}} e^{isk}
  \Pi_{pq} D_{K,0}^{(k)} \ket{\psi_\FS}
  = - \sum_q e^{isq} D_{K,0}^{(q)} \ket{\psi_\FS}
  = - \Z_{Ks} \ket{\psi_\FS}.
\end{align}
In total, we have that
\begin{align}
  \sum_{p<q} \Pi_{pq} \Z_{Ks} \ket{\psi_\FS}
  = \sp{{ N-1 \choose 2 } - 1} \Z_{Ks} \ket{\psi_\FS},
\end{align}
which implies that $\Z_{Ks}\ket{\psi_\FS}$ is proportional to an
eigenstate of $H_{\t{int}}$ with interaction energy
$-u\sp{{ N-1 \choose 2 } - 1}$, which is $Nu$ above the interaction
energy of $-u{N\choose2}$ of states in the fully symmetric manifold,
so
\begin{align}
  \E \Z_{Ks} \P_0
  =  \f1{Nu} \Z_{Ks} \P_0
  ...
  \label{eq:proj_elim}
\end{align}
where $\E$ is defined in \eqref{eq:pert} and $f\equiv N/L$ is the
filling fraction of quasi-momentum modes.  We have thus far simplified
the effective Hamiltonian in \eqref{eq:H_eff_2_start} to
\begin{align}
  H_{\t{eff}}^{(2)}
...
$\P_0 \Z_{Jr} \Z_{Ks} \P_0$, as well as the sum over $r,s$.


\subsubsection{The case of $r=s$}

To evaluate the product $\P_0 \Z_{Jr} \Z_{Ks} \P_0$ of spin-wave
operators projected onto the fully symmetric manifold $\M_\FS$, we
first consider the case of $r=s$, and use the permutational symmetry
enforced by the projectors $\P_0$ to expand
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\pm} \P_0
  &= \sum_{p,q} e^{\pm i\p{p+q}} \P_0 D_{J,0}^{(p)} D_{K,0}^{(q)} \P_0 \\
  &= \sum_p e^{\pm i2p} \P_0 D_{J,0}^{(0)} D_{K,0}^{(0)} \P_0
  + \sum_{p\ne q} e^{\pm i\p{p+q}} \P_0 D_{J,0}^{(0)} D_{K,0}^{(1)} \P_0,
\end{align}
where $\sum_p e^{\pm i2p}=0$, and
\begin{align}
  \sum_{p\ne q} e^{\pm i\p{p+q}}
  = \sum_p e^{\pm ip} \sum_{q\ne p} e^{\pm iq}
  = \sum_p e^{\pm ip} \p{-e^{\pm ip}}
  = -\sum_p e^{\pm i2p}
  = 0,
\end{align}
which implies that
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\pm} \P_0 = 0.
  \label{eq:ZZ_eq}
\end{align}

\subsubsection{The case of $r\ne s$}
\label{sec:H_eff_2_end}

For the case of $r\ne s$, we similarly expand
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\mp} \P_0
  &= \sum_{p,q} e^{\pm i\p{p-q}} \P_0 D_{J,0}^{(p)} D_{K,0}^{(q)} \P_0 \\
...
\begin{align}
  ...
  \label{eq:ZZ_neq_mid}
\end{align}
In order to express this product in terms of collective operators, we
expand
\begin{align}
...
\end{align}
then solve for the last term and substitute the result into
\eqref{eq:ZZ_neq_mid}, giving us
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\mp} \P_0
  = \f{N^2}{N-1} \P_0 D_{J,0}^{(0)} D_{K,0}^{(0)} \P_0
  - \f1{N-1} \P_0 \D_{J,0} \D_{K,0} \P_0.
\end{align}
Using the structure constants $f_{\ell_1m_1;\ell_2m_2}^{LM}$ of the
transition operator algebra (see Appendix \ref{sec:trans_prod}), we
then expand
\begin{align}
  N \P_0 D_{J,0}^{(0)} D_{K,0}^{(0)} \P_0
...
Having evaluated the product $\P_0 \Z_{Jr} \Z_{Ks} \P_0$ of spin-wave
operators in \eqref{eq:ZZ_eq} and \eqref{eq:ZZ_neq}, the last
ingredient needed to simplify the effective Hamiltonian
$H_{\t{eff}}^{(2)}$ in \eqref{eq:H_eff_2_mid} is an evaluation of the
sum over $r,s\in\set{\pm1}$.  The product of spin-wave operators
enforces $r\ne s$, but is otherwise independent of $r,s$, so we need
only evaluate
\begin{align}
  \sum_{s\in\set{\pm1}} \chi_{J,-s} \chi_{Ks}
  =
  \begin{cases}
    2 & J = K \, \t{mod}\, 2 \\
    0 & \t{otherwise}
...
\begin{align}
  H_{\t{eff}}^{(2)}
  = \f{t^2}{2\p{N-1}fU}
  \sum_{J=K\,\t{mod}\,2} A_J^{(\phi)} A_K^{(\phi)}
  \sp{\D_{J,0} \D_{K,0} - N\sum_L f_{J,0;K,0}^{L,0} \D_{L,0}},
  \label{eq:H_eff_2_unit}
\end{align}
where, as for $H_{\t{eff}}^{(1)}$ in \eqref{eq:H_eff_1}, we drop the
explicit projectors $\P_0$ in \eqref{eq:H_eff_2_unit} with the
understanding that we are implicitly restricting all effective
dynamics to the fully symmetric manifold $\M_\FS$.

Thus far, we have assumed that the sum over quasi-momenta
$\sum_ke^{\pm ik}=0$, which is only valid when all quasi-momenta are
occupied, such that the sum $\sum_k$ runs over all angles
$k\in\ZZ_N\times2\pi/N$.  If not all quasi-momenta are occupied, then
we can repeat all derivations above with mean-subtracted spin-wave
operators
\begin{align}
  \Z_{Ks} \equiv \sum_q e^{isq} D_{K,0}^{(q)}
  \to
  \tilde\Z_{Ks}
  \equiv \sum_q \p{e^{isq} - \EE_k\sp{e^{isk}}} D_{K,0}^{(q)},
\end{align}
where $\EE_k\sp{e^{isk}}$ is an average of $e^{isk}$ over all occupied
quasi-momenta $k$.  Note that the difference
$\tilde\Z_{Ks}-\Z_{Ks}=-\EE_k\sp{e^{isk}}\D_{K,0}$ is permutationally
symmetric, which implies that it only couples fully symmetric states
$\ket{\psi_\FS}\in\M_\FS$ to fully symmetric states.  In particular,
due to the fact that $\E$ is a linear combination of projectors onto
interaction-energy eigenstates strictly outside of $\M_\FS$,
\begin{align}
  \P_0 \tilde\Z_{Jr} \E \tilde\Z_{Ks} \P_0
  = \P_0 \Z_{Jr} \E \Z_{Ks} \P_0,
\end{align}
which implies that we can freely take
$\Z_{Jr},\Z_{Ks}\to\tilde\Z_{Jr},\tilde\Z_{Ks}$ with no consequence
for the effective Hamiltonian $H_{\t{eff}}^{(2)}$ in
\eqref{eq:H_eff_2_start}.

Repeating the previous calculations in this section with the
mean-subtracted spin-wave operators, we still find that, just as in
\eqref{eq:proj_elim},
\begin{align}
  \E \tilde\Z_{Ks} \P_0 = \f1{fU} \tilde\Z_{Ks} \P_0,
\end{align}
whereas, in contrast to \eqref{eq:ZZ_eq} and \eqref{eq:ZZ_neq},
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\pm} \P_0
  = -\f{\var_q\sp{e^{\pm iq}}}{N-1}
  \sp{\P_0 \D_{J,0} \D_{K,0} \P_0
    - N \sum_L f_{J,0;K,0}^{L,0} \P_0 \D_{L,0} \P_0},
\end{align}
where
$\var_q\sp{X_q} \equiv \EE_q\sp{\p{X_q-\EE_p\sp{X_p}}^2} =
\EE_q\sp{X_q^2}-\EE_q\sp{X_q}^2$ is the variance of $X_q$ over all
occupied quasi-momenta $q$, and
\begin{align}
  \P_0 \Z_{J\pm} \Z_{K\pm} \P_0
  = -\f{1-\EE_p\sp{e^{ip}}\EE_q\sp{e^{-iq}}}{N-1}
  \sp{\P_0 \D_{J,0} \D_{K,0} \P_0
    - N \sum_L f_{J,0;K,0}^{L,0} \P_0 \D_{L,0} \P_0}.
\end{align}
...
\red{To do: Work out SU($2$) explicitly as a special case in these
  notes.  Also, the current second-order derivation assumes unit
  filling, and needs to be adjusted for the possibility of $f<1$.  I
  think that in the case of $f<1$ the factor of $1/2$ in
  \eqref{eq:H_eff_2} turns into $\bk{w_J\p{q}w_K\p{q}}_q$, e.g.~when
  $J=K=1$, this factor becomes the variance of $\sin\p{q}$ over all
  occupied modes, as in the known case of SU($2$).  I should also work
  out explicitly the contributions through second order in $\phi$,
  which will simplify things somewhat as we will only need $J,K\le2$
  in $H_{\t{lat}}^{(1)}$, and $J=K=1$ in $H_{\t{lat}}^{(2)}$.  I can
  also do a third-order calculation of $H_{\t{eff}}^{(3)}$ to extract
  dependence on $\phi^3$, but that's probably unnecessary / not
