...
space of linear operators on the Hilbert space $\H_n$ of an $n$-level
...
subsystems, e.g.~an array of $n$-level nuclear spins, we identify the
set $\Pi\equiv\set{\Pi_\sigma}$ of operators that permute subsystems.
Permutation operators $\Pi_\sigma$ are labeled by elements
$\sigma\in\SS_N$ of the symmetric group $\SS_N$ of order $N$.  We
refer to simultaneous eigenstates of all $\Pi_\sigma\in\Pi$ as {\it
  collective states}.  The subset of collective states that have
eigenvalue 1 with respect to all permutation operators
$\Pi_\sigma\in\Pi$ is the {\it maximally symmetric collective (MSC)}
manifold, which is precisely the Dicke manifold in the case of
two-level subsystems ($n=2$).  The MSC manifold is spanned by a basis
$\set{\ket{\v m}}$ of MSC states that can be uniquely identified by a
list of integers $\v m=\p{m_0,m_1,\cdots,m_{n-1}}\in\ZZ_{N+1}^n$,
where the integer $m_j\in\ZZ_{N+1}$ ($0\le m_j\le N$) indicates the
total occupation number of subsystem state $j$, and $\sum_j m_j=N$.
The Dicke manifold of $N$ qubits, for example, is spanned by the Dicke
states $\set{\ket{m_0,m_1}}$, where $m_0+m_1=N$ and the integer $m_0$
($m_1$) indicates the occupation number of the qubit state $\ket{0}$
($\ket{1}$).

The MSC manifold has two important properties that concern us.  First,
the MSC manifold is closed under the action of unitaries generated by
linear combinations collective drive operators $\D_{LM}$.  Second, the
MSC manifold is precisely the ground-state manifold of the collective
SU($n$)-symmetric interaction Hamiltonian $H_{\t{int}}$ in
\eqref{eq:H_int_spin} or \eqref{eq:H_int_drive}.  We prove these
properties of the MSC manifold in Appendix \ref{sec:MSC_proofs}.
...
\begin{align}
  ...
  \sum_j X_j X_j^\dag.
\end{align}
Choosing a different basis $Y\equiv\set{Y_j}$ for $\B\p{\H_n}$ with
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Key properties of the maximally-symmetric collective
  manifold}
\label{sec:MSC_proofs}

Here we prove two important properties of the maximally-symmetric
collective (MSC) manifold defined in Section \ref{sec:classifying}:
\begin{enumerate*}
\item \label{pt:MSC_closure} that the MSC manifold is closed under the
  action of unitaries generated by linear combinations collective
  drive operators $\D_{LM}$; and
\item \label{pt:MSC_ground} that the MSC manifold is precisely the
  ground-state manifold of the collective SU($n$)-symmetric
  interaction Hamiltonian $H_{\t{int}}$ in \eqref{eq:H_int_spin} or
  \eqref{eq:H_int_drive}.
\end{enumerate*}
Our starting ingredients are an $n$-dimensional Hilbert space $\H_n$,
its $N$-fold tensor product $\H_n^{\otimes N}$, and the set
$\Pi\equiv\set{\Pi_\sigma}$ of operators that permute the tensor
factors of $\H_n^{\otimes N}$.  Permutation operators $\Pi_\sigma$ are
labeled by elements $\sigma\in\SS_N$ of the symmetric group $\SS_N$ of
order $N$.  The MSC sub-manifold of $\H_n^{\otimes N}$ is defined by
the set of states that have eigenvalue $1$ with respect to all
permutation operators $\Pi_\sigma\in\Pi$.

Given an orthonormal basis $X=\set{X_j}$ for the space $\B\p{\H_n}$ of
linear operators on $\H_n$, we define the collective operator
$\X_j\in\B\p{\H_n^{\otimes N}}$ by
\begin{align}
  \X_j \equiv \sum_{p\in\ZZ_N} X_j^{(p)},
\end{align}
where $\ZZ_N\equiv\set{0,1,\cdots,N-1}$ and $X_j^{(p)}$ acts with
$X_j$ on tensor factor $p$ of $\H_n^{\otimes N}$ and trivially on all
other tensor factors, i.e.
\begin{align}
  X_j^{(p)} \equiv
  \1^{\otimes p} \otimes X_j \otimes \1^{\otimes\p{N-p-1}}.
\end{align}
We denote a vector of all collective operators in $\X\equiv\set{\X_j}$
by $\v\X$, i.e.~$\v\X\equiv\p{\X_j:\X_j\in\X}$.

Given an arbitrary MSC state
$\ket{\psi_{\t{MSC}}}\in\H_n^{\otimes N}$, we can use the fact that
all permutation operators $\Pi_\sigma\in\Pi$ are unitary to expand
\begin{align}
  \Pi_\sigma e^{i\v\theta\c\v\X} \ket{\psi_{\t{MSC}}}
  = \Pi_\sigma e^{i\v\theta\c\v\X}
  \Pi_\sigma^\dag \Pi_\sigma \ket{\psi_{\t{MSC}}}
  = e^{i\v\theta\c\Pi_\sigma\v\X\Pi_\sigma^\dag}
  \Pi_\sigma \ket{\psi_{\t{MSC}}}
  = e^{i\v\theta\c\Pi_\sigma\v\X\Pi_\sigma^\dag} \ket{\psi_{\t{MSC}}},
\end{align}
where
$\Pi_\sigma\v\X\Pi_\sigma^\dag \equiv \p{\Pi_\sigma\X_j\Pi_\sigma^\dag
  : \X_j\in\X}$, and
\begin{align}
  \Pi_\sigma \X_j \Pi_\sigma^\dag
  = \sum_{p\in\ZZ_N} \Pi_\sigma X_j^{(p)} \Pi_\sigma^\dag
  = \sum_{p\in\ZZ_N} X_j^{(\sigma\c p)}
  = \sum_{p\in\ZZ_N} X_j^{(p)},
\end{align}
which implies that $\Pi_\sigma\v\X\Pi_\sigma^\dag=\v\X$, and in turn
\begin{align}
  \Pi_\sigma e^{i\v\theta\c\v\X} \ket{\psi_{\t{MSC}}}
  = e^{i\v\theta\c\v\X} \ket{\psi_{\t{MSC}}}.
\end{align}
We thus find that $e^{i\v\theta\c\v\X} \ket{\psi_{\t{MSC}}}$ is itself
an MSC state, as it has eigenvalue 1 with respect to arbitrary
permutation operators $\Pi_\sigma\in\Pi$, thereby proving claim
\ref{pt:MSC_closure} by choice of the orthonormal basis $\set{D_{LM}}$
for the space $\B\p{\H_n}$ of linear operators on $\H_n$.



[Proof of claim \ref{pt:MSC_ground} to come]
