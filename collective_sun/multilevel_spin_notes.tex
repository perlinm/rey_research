...
symmetric manifold $\M_\FS\subset\H_n^{\otimes N}$ is spanned by a
...
${N+n-1 \choose n-1}$, and energy of $\ket{\psi_\FS}\in\M_\FS$ with
respect to the interaction Hamiltonian $H_{\t{int}}$ in
\eqref{eq:H_int_perm} is $-u{N \choose 2}=-\p{1/4}N\p{N-1}u$.
...
\end{align}
Spin-orbit coupling with multilevel systems thus allows for the
