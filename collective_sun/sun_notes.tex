...
should identify $\p{D_{1,0},D_{1,1}}\sim\p{S_\z,S_\x}$, where
$S_\alpha$ is a collective SU(2) spin-$\alpha$
...
2-level spins via
\begin{align}
  \ket{\mu}_{\t{single}} \to \ket{\mu+I}_{\t{collective}}
  \propto S_+ \ket\dn^{\otimes\p{n-1}},
\end{align}
...
\begin{align}
  H_{\t{drive}}^{(\phi)}
  = -\f12 \Omega \sum_j \p{e^{i\phi_j} T_{1,1}^{(j)}
...
\begin{align}
  H_{\t{drive}}
  = -\f12 \Omega \sum_j\p{T_{1,1}^{(j)} + {T_{1,1}^{(j)}}^\dag}
