...
\begin{align}
  ...
  = \sum_p v_p \P_0 X_0 \P_0 \EQFS \bar v \X,
  \label{eq:H_1_1}
\end{align}
...
$\v Z$ is a vector of all $Z_k$.  We thus find that the vector
$\V_2\ket\psi$ is an eigenvector of the interaction Hamiltonian $H_0$
if the coefficient matrix $\m w$ of all $w_{pq}$ satisfies
\begin{align}
...
\begin{align}
  ...
  \EQFS -\f{\v v\c\v v}{N\p{N-1}} \p{\X^2 - \X^{(2)}}
  \label{eq:PXXP_neq}
\end{align}
Altogether, we have that
\begin{align}
  \V_1\p{\v v}^2 \EQFS -\f{\v v\c\v v}{N\p{N-1}} \p{\X^2 - N \X^{(2)}}.
\end{align}
...
and expand
\begin{align}
  \X\odot\Y \EQFS \Z + N\p{N-1} X_0 Y_1.
\end{align}
Substituting the product $X_0 Y_1$ back into \eqref{eq:PXYP_start}
gives us
\begin{align}
  \V_2 \EQFS \f12 \bar w \p{\X\odot\Y -  \Z}.
\end{align}
...
\begin{multline}
  \P_0 \V_2^2 \P_0
  \EQFS \p{\f12 \v{\m w}\c\v{\m w} - \v w\c\v w} X_0 X_1 Y_2 Y_3
