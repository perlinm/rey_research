...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Product of two single-body operators}
\label{sec:PXYP}
...
\begin{align}
  \P_0 \sp{\sum_{p,q} v_{Xp} v_{Yq} X_p Y_q} \P_0
  \EQFS \sum_p v_{Xp} v_{Yp} X_1 Y_1
  + \sum_{p\ne q} v_{Xp} v_{Yq} X_1 Y_2,
  \label{eq:PXYP_start}
\end{align}
where $X,Y$ are single-body operators; and the coefficients
$v_{Xp},v_{Yq}$ satisfy \eqref{eq:cond_1} with eigenvalue
$\Delta\ne0$, which implies that they are mean-zero.  The first sum on
the right of \eqref{eq:PXYP_start} is then
\begin{align}
  \sum_p v_{Xp} v_{Yp} X_1 Y_1
  \EQFS \f1N \v v_X \c \v v_Y\, \col{XY},
  &&
  \col{Z} \equiv \sum_p Z_p.
\end{align}
To simplify the second sum on the right of \eqref{eq:PXYP_start}, we
use the fact that $\v v$ is mean-zero to write
\begin{align}
  \sum_{p\ne q} v_{Xp} v_{Yq}
...
\begin{align}
  \sum_{p\ne q} v_{Xp} v_{Yq} X_1 Y_2
  \EQFS - \v v_X\c\v v_Y \p{\col{X}\,\col{Y} - \col{XY}}.
\end{align}
Altogether, we have that
\begin{align}
  \sum_{p,q} v_{Xp} v_{Yq} X_p Y_q
  \EQFS - \f{\v v_X \c\v v_Y}{N\p{N-1}}
  \p{\col{X}\,\col{Y} - N \col{XY}}.
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Product of two two-body operators}
\label{sec:POQP}
...
\begin{multline}
  \P_0 \sp{\sum_{\substack{k<l\\p<q}}
    w_{Ok\ell} w_{Qpq} O_{k\ell} Q_{pq}} \P_0
  \EQFS \sum_{p<q} w_{Opq} w_{Qpq} O_{1,2} Q_{1,2}
  + \sum_{\substack{k,p,q\\k\ne p\ne q\ne k}}
  w_{Okp} w_{Qkq} O_{1,2} Q_{1,3} \\
  + \sum_{\substack{k<\ell\\p<q\\\set{k,\ell}\ne\set{p,q}}}
  w_{Ok\ell} w_{Qpq} O_{1,2} Q_{3,4},
  \label{eq:POQP_start}
\end{multline}
where $O$ and $Q$ are permutationally-symmetric two-body operators;
and the coefficients $w_{Ok\ell},w_{Qpq}$ satisfy \eqref{eq:cond_2}
...
\end{align}
we can write the first sum on the right of \eqref{eq:POQP_start} as
\begin{align}
  \sum_{p<q} w_{Opq} w_{Qpq} = \v{\m w}_O \c\v{\m w}_Q,
\end{align}
and, using the fact that $w_{Opp}=0$, we can write the second sum on
the right of \eqref{eq:POQP_start} as
\begin{align}
  \sum_{\substack{k,p,q\\k\ne p\ne q\ne k}} w_{Okp} w_{Qkq}
  = \sum_{k,p} w_{Okp} \sum_{q\ne p} w_{Okq}
  = \sum_{k,p} w_{Okp} \p{w_{Qk} - w_{Qkp}}
  = \v w_O \c \v w_Q - 2 \v{\m w}_O \c \v{\m w}_Q.
\end{align}
Using additionally the fact that $w_{Opq},w_{Qpq}$ are mean-zero, we
can similarly simplify
\begin{align}
  \sum_{\substack{p<q\\k<\ell\\\set{k,\ell}\ne\set{p,q}}}
  w_{Ok\ell} w_{Qpq} = -\v w_O \c \v w_Q + \v{\m w}_O \c \v{\m w}_Q.
\end{align}
Altogether, we thus have that
\begin{multline}
  \sum_{\substack{k<l\\p<q}} w_{Ok\ell} w_{Qpq} O_{k\ell} Q_{pq}
  \EQFS \v{\m w}_O \c \v{\m w}_Q O_{1,2} Q_{1,2}
  + \p{\v w_O\c\v w_Q - 2 \v{\m w}_O \c \v{\m w}_Q} O_{1,2} Q_{1,3} \\
  - \p{\v w_O \c \v w_Q - \v{\m w}_O \c \v{\m w}_Q} O_{1,2} Q_{3,4}.
  \label{eq:POQP}
\end{multline}
