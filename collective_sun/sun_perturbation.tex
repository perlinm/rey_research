...
\begin{align}
  ...
  &&
  \v{\m w} \equiv \sum_{p<q} w_{pq} \ket{\set{p,q}}.
  \label{eq:V2_eig_mat}
\end{align}
...
\eqref{eq:V2_eig} nominally requires diagonalizing the
...
dimensions $\sim N\times N$, and isotropy can reduce this problem even
further.  We discuss these reductions in Section
\ref{sec:multi_body_symmetries}.
...
a choice of $M$ distinct spins that $O$ acts on; and $\C_N\p{M}$ is
the set of all distinct choices of $M$ elements from a set of size
$N$.
...
\begin{align}
  ...
  = \sum_{k\in\C_N\p{M}} \sum_{\substack{p\in\ZZ_N\\p\notin k}}
  \sum_{a\in\ZZ_M} h_{p k_a} w_k \Pi_{p k_a} O_k \ket\psi.
\end{align}
where $k\equiv\p{k_1,k_2,\cdots,k_M}$, with $k_a$ the $a$-th entry in
$k$.  Defining
$k_{a:p}\equiv\p{k_1,k_2,\cdots,k_{a-1},p,k_{a+1},\cdots,k_M}$,
i.e.~$k$ with the $a$-th number $k_a$ replaced by $p$, we can simplify
\begin{align}
...
\begin{align}
  ...
  h_{pq} w_k  \Pi_{pq} O_k \ket\psi
  = \sum_{k\in\C_N\p{M}} \sum_{\substack{p\in\ZZ_N\\p\notin k}}
  \sum_{a\in\ZZ_M} h_{pk_a} w_{k_{a:p}} O_k \ket\psi.
\end{align}
...
\begin{align}
  ...
  &&
  \m h\circ\m w \equiv \sum_{a\in\ZZ_M} \m h\c_a\m w,
\end{align}
...
where
\begin{align}
  \check{\m h} \equiv \sum_{k\in\C_N\p{M}}
  \sum_{\substack{p\in\ZZ_N\\p\notin k}} \sum_{a\in\ZZ_M}
  h_{pk_a} \op{k}{k_{a:p}}
...
\label{sec:multi_body_symmetries}
...
$\check{\m h}$ with dimensions
${N\choose M}\times{N\choose M}\sim N^M\times N^M$.  If the
interaction Hamiltonian $H_0$ and multi-body perturbation $\V_M$ have
any symmetries, such as translational invariance isotropy, then we can
use these symmetry to reduce the eigenvalue problem in
\eqref{eq:H_VM_psi_eig}.  Translational invariance, for example, can
reduce this problem to diagonalizing a matrix with dimensions
...
and $\V_M$ to simplify the eigenvalue problem in
\eqref{eq:H_VM_psi_eig}.

Symmetries of $H_0$ and $\V_M$ essentially break the set of choices
$\C_N\p{M}$ into equivalence classes $\S\p{M}$\footnote{For brevity,
  we will suppress the explicit dependence of $\S\p{M}$ on $N$, as
  well as the dependence of $\S\p{M}$ on any other factors such as
  lattice geometry.}, where elements $s\in\S\p{M}$ are subsets
...
in \eqref{eq:H_VM_psi_eig} in a basis that obeys the symmetries under
consideration.  To this end, for any equivalence class $s\in\S\p{M}$
we define a uniform superposition $\ket{s}$ over all choices $k\in s$,
as well as the projector $\P_\S$ onto the space of uniform
superpositions:
\begin{align}
  \ket{s} \equiv \f1{\sqrt{\abs{s}}} \sum_{k\in s} \ket{k},
  &&
  \P_{\S_M} \equiv \sum_{s\in\S\p{M}} \op{s},
\end{align}
...
projection of $\check{\m h}$ onto the symmetric subspace:
\begin{align}
  \check{\m h}_{\S_M}
  \equiv \P_{\S_M} \check{\m h} \P_{\S_M}
...
\begin{align}
  \check{\m h}_{\S_M} \c \v{\m w} = \Delta \v{\m w}.
\end{align}
In general, symmetries of the interaction Hamiltonian $H_0$ give the
matrix $\check{\m h}$ a block diagonal structure, with blocks of size
$\abs{\S_M}\times\abs{\S_M}$ corresponding to different orbits under
the symmetry group action.  These blocks can be classified, for
example, by a choice of wavenumber
$k_s\in\ZZ_{\abs{s}}\times 2\pi/\abs{s}$ for each equivalence class
$s\in\S_M$.  Even if the multi-body operator $\V_M$ does not obey the
same symmetries as $H_0$, this block diagonal structure can be used to
reduce the full multi-body eigenvalue problem to that of diagonalizing
each block.  We defer this reduction to future work.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Projecting products of operators onto the permutationally
  symmetric manifold}
\label{sec:sym_prod}
...
expansion for these products, as well as a set of rules to simplify
these expansions so as to minimize their computational cost.  These
diagrammatic methods allow us to algorithmically evaluate products
such as \eqref{eq:example_prod} from the bare multi-body operator data
$\p{w^{(j)},O},\p{v,Q},\p{w^{(k)},O}$.  We then work out some special
cases of products that are relevant to the perturbation theory in
Sections \ref{sec:single_body_pert} and \ref{sec:two_body_pert}.
