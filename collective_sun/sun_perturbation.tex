...
Given a system of $N$ spins, we wish to simplify a product of
multi-body operators restricted to the fully symmetric manifold.  For
$p$ multi-body operators, such a product takes the form
\begin{align}
  \P_0 \sp{\prod_{j\in\ZZ_p}
    \sum_{k\in\C_N\p{M_j}} w_j\p{k} O_j\p{k}} \P_0,
  \label{eq:sym_prod_choice}
\end{align}
where $\P_0$ is a projector onto the fully symmetric manifold;
$k\in\C_N\p{M_j}$ is a choice of $M_j$ distinct spins; $O_j\p{k}$
denotes the action of a permutationally-symmetric $M_j$-body operator
acting on spins $k$; and $w_j\p{k}$ is a scalar coefficient for
$O_j\p{k}$.

To simplify the product in \eqref{eq:sym_prod_choice}, we first write
\begin{align}
  \prod_{j\in\ZZ_p} \sum_{k\in\C_N\p{M_j}} w_j\p{k} O_j\p{k}
  = \prod_{j\in\ZZ_p} \f1{M_j!} \sum_{k\in\ZZ_N^{M_j}} w_j\p{k} O_j\p{k},
...
which $w_j\p{k}=O_j\p{k}=0$ if $k$ contains any repeated symbols.

In order to project \eqref{eq:sym_prod_vec} onto the fully symmetric
manifold, we need to collect terms whose operator content is
equivalent up to a permutation of spins.  To this end, we classify
terms in \eqref{eq:sym_prod_vec} by the numbers $g_S$ of indices
shared by all tensors $w_j,O_j$ with $j\in S\subset\ZZ_p$.  For
example, a term of the form $w_1\p{a,b,c,d} w_2\p{b,c,d} w_3\p{b}$
with distinct indices $\p{a,b,c,d}\in\C_N\p{4}$ would have
\begin{align}
  g_{\set{1}} = 1,
  &&
  g_{\set{1,2,3}} = 1,
...
\end{align}
where $O\p{g}$ is an operator acquired assigning indices to the
...
explicitly, we define the set of values that the distinct indices
assigned according to $g$ can take,
\begin{align}
  \C_N\p{g}
  \equiv \set{ k \in \bigotimes_{S\in\PP\p{\ZZ_p}} \C_N\p{g_S}
    : \t{all elements of $k$ are distinct} }
  \simeq \C_N\p{\sum_{S\in\PP\p{\ZZ_p}}g_S},
\end{align}
where each $k\in\C_N\p{g}$ has the natural decomposition
$k=\p{k_S:S\in\PP\p{\ZZ_p}}$ with $k_S\in\C_N\p{g_S}$, and we define
the restriction of $k\in\C_N\p{g}$ to values associated with the
tensors $w_j,O_j$,
\begin{align}
  k_j \equiv \bigcup_{S\in\PP\p{\ZZ_p} : j\in S} k_S
  = \bigcup_{R\in\PP\p{\ZZ_p\setminus\set{j}}} k_{\set{j}\cup R},
\end{align}
where the union of lists $k_S$ denotes a concatenation of those lists.  We then have that
\begin{align}
  w\p{g} \equiv \sum_{k\in\C_N\p{g}} \prod_{j\in\ZZ_p} w_j\p{k_j},
  &&
  O\p{g}
  \equiv \prod_{j\in\ZZ_p} O_j\p{k_j}~\t{for some}~k\in\C_N\p{g},
\end{align}
and
\begin{align}
...
\end{align}
where the first product in \eqref{eq:sym_prod_symmetry} contains multinomial coefficients
\begin{align}
...
on every tensor.  The choice of $k\in\C_N\p{g}$ in the definition of
$O\p{g}$ is irrelevant after projection onto the fully symmetric
manifold.  Altogether, we have that
\begin{align}
  \prod_{j\in\ZZ_p} \sum_{k\in\C_N\p{M_j}} w_j\p{k} O_j\p{k}
