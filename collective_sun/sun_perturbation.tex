...
states $\set{\ket{m}:m\in\A_n\p{N}}$, where each
$m=\p{m_1,m_2,\cdots,m_n}$ specifies the occupation number $m_\mu$ of
the single-spin state $\mu$.  For consistency with the expression of
matrix elements in \eqref{eq:matrix_element}, for each
$\ket{m}\in\M_0$ we define $\ket{\1,1,m}\equiv\ket{m}$, with $\1$ the
identity operator, and we define $\1_1\equiv\1$.  We now need to
determine the set of $r$-body operators $\set{O}$ and dimension-$r$
tensors $\set{\m u}$ that define our basis for $\M_r$ with
$r=1,2,\cdots,r_{\t{max}}$.  As an initial guess for $\set{\m u}$, we
...
$\col\B_{r-1}\equiv\bigcup_{s<r-1}\B_s$ for $\col\M_{r-1}$.  To
...
to check orthogonality can be computationally intensive.  When
