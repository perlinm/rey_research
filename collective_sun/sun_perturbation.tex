...
satisfies the condition in \eqref{eq:cond_1} with eigenvalue $0$.  All
other vectors $\v v_\Delta$ satisfying \eqref{eq:cond_1} with
$\Delta\ne0$ must be orthogonal to $\v\1$, and therefore mean-zero.
...
\end{align}
We can thus (numerically) diagonalize each of the blocks $\m h_\ell$,
and classify their eigenvectors $\v w_\ell$ by the corresponding
