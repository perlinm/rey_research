...
\begin{align}
  H_1^{(2)}
  \EQFS \f{N}{N-1} \sum_{X,Y} \sum_{\Delta\ne0}
...
and for any single-body operator $Z$, we define the spin-averaged
operator
\begin{align}
  \col{Z} \equiv \f1N \sum_p Z_p.
\end{align}
...
\begin{align}
  ...
  \EQFS {N \choose 2} \sum_O \bar w_O \col{O},
\end{align}
where
\begin{align}
  \bar w_O \equiv {N \choose 2}^{-1} \sum_{p<q} w_{Opq},
  &&
  \col{O} \equiv {N \choose 2}^{-1} \sum_{p<q} O_{pq},
\end{align}
are averages of $w_{Opq}$ and $O_{pq}$ over all pairs of spins $p,q$.
If the two-body operator $O=X\otimes Y$, then
\begin{align}
  \col{O} \EQFS X_0 Y_1
  \EQFS \f1{N-1} \p{N\, \col{X}\,\col{Y} - \col{XY}},
\end{align}
so
\begin{align}
  H_2^{(1)} \EQFS \f12 N \sum_{\p{X,Y}} \bar w_{\p{X,Y}}
  \p{N\, \col{X}\,\col{Y} - \col{XY}}.
\end{align}
In order to compute the second-order effective Hamiltonian
...
\end{align}
which implies that $\V_2^\Delta\ket\psi$ is an (unnormalized0 state of
...
\begin{align}
  ...
  \EQFS \sum_{X,Y} \v v_X \c\v v_Y \col{XY},
  &&
  \col{Z} \equiv \f1N \sum_p Z_p.
  \label{eq:PXYP_eq}
\end{align}
...
\begin{align}
  N^2 \col{X}\,\col{Y} = \sum_{p,q} X_p Y_q
  = \sum_{p=q} X_p Y_q + \sum_{p\ne q} X_p Y_q
  \EQFS N \col{XY} + N\p{N-1} X_0 Y_1,
...
\begin{align}
  ...
  \EQFS - \f1{N-1}\sum_{X,Y} \v v_X\c\v v_Y
  \p{N \col{X}\,\col{Y} - \col{XY}}.
  \label{eq:PXYP_neq}
\end{align}
...
\begin{align}
  \P_0 \p{\V_1^\Delta}^2 \P_0
  = - \f{N}{N-1} \sum_{X,Y} \v v_X \c\v v_Y
  \p{\col{X}\,\col{Y} - \col{XY}}.
\end{align}
...
spin-average, i.e.~$\col{Q}\equiv\sum_j Q^{(j)}/N$, where $Q^{(j)}$
acts with the single-body operator $Q$ on spin $j$ and trivially on
all other spins.  By construction, the set of collective operators
$\set{\O_P:\t{partitions}~P~\t{of}~S}$ is one-to-one to the set of
multi-local operators $\set{O_P:\t{partitions}~P~\t{of}~S}$, and it is
straightforward to work out that for any set $S$ of single-spin
operators,
\begin{align}
  N^{\abs{S}} \O_S
...
\begin{align}
  N!_{\abs{S}} O_S
  &\EQFS \sum_{\substack{\t{partitions}\\P~\t{of}~S}}
  \p{-1}^{\abs{S}-\abs{P}} N^{\abs{P}} \O_P
  \times \prod_{p\in P}\p{\abs{p}-1}!
  ...
  \p{-1}^{\abs{S}-\abs{\tilde P}} N^{\abs{\tilde P}} \O_{\tilde P}
...
the latter can involve less computational overhead due to the lack of
symmetrization in the construction of $\O_{\tilde P}$.  There are, in
principle, even more economical decompositions of a multi-local
operators into collective opreators, for example
\begin{align}
  N!_2\, X\otimes Y
  &\EQFS N^2\, \col{X}\,\col{Y} - N\, \col{XY}, \\
  N!_3\, X\otimes Y \otimes Z
  &\EQFS N^3\, \col{X}\,\col{Y}\,\col{Z}
  - N^2 \p{\col{X}\,\col{YZ} + \col{Y}\,\col{XZ} + \col{Z}\,\col{XY}}
  + N \p{\col{ZXY} + \col{YXZ}},
\end{align}
...
\begin{align}
  N!_2\, X\otimes X \EQFS N^2\, {\col{X}\,}^2 - N\, \col{X^2},
  &&
  N!_3\, X^{\otimes 3} \EQFS N^3\, {\col{X}\,}^3
  - 3 N^2\, \col{X} \odot \col{X^2} + 2 N\, \col{X^3},
\end{align}
\begin{align}
  N!_4\, X^{\otimes 4}
  \EQFS N^4\, {\col{X}\,}^4 - N^3 \p{4 {\col{X}\,}^2 \odot \col{X^2}
    + 2 \col{X} \, {\col{X}\,}^2 \, \col{X}}
  + N^3\p{3 {\col{X^2}\,}^2 + 8 \col{X} \odot \col{X^3}}
  - 6 N\, \col{X^4}.
\end{align}
If furthermore $X^2=1$, as with e.g.~the Pauli operators of SU(2),
then
\begin{align}
  N!_2\, X\otimes X \EQFS N^2\, {\col{X}\,}^2 - N,
  &&
  N!_3\, X^{\otimes 3}
  \EQFS N^3\, {\col{X}\,}^3 - \p{3N-2} N\, \col{X},
\end{align}
\begin{align}
  N!_4\, X^{\otimes 4} \EQFS N^4\, {\col{X}\,}^4
  - 2\p{3N-4} N^2\, {\col{X}\,}^2 + 3 N \p{N-2}.
\end{align}
