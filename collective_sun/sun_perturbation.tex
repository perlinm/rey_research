...
$\norm{\O\p{\m w,X}}<\Delta_{\t{gap}}/2$, performing such simulations
with an appropriate choice of excited states is equivalent to
high-order ground-state perturbation theory.  The corresponding order
of perturbation theory is determined by the choice of excited states
in a manner that will be clarified below.  Moreover, the first-class
treatment of excited states enables simulating the dynamics induced by
{\it non-perturbative} additions $\O\p{\m w,X}$ to the base
Hamiltonian $H_0$ in finite systems.
...
suitable basis.  In the case of SU(2), a good starting point is to
identify states with definite (non-negative) spin length
$S=\set{N/2,N/2-1,\cdots}$, and spin projection
...
Similarly, rather than the spin projection $m_\z$ we identify states
by labels $m=\p{m_\up,m_\dn}$ in which $m_\mu$ denotes the occupation
number of the single-spin state $\mu$, with $m_\up-m_\dn=m_\z$ and
$m_\up+m_\dn=N$.

The label $m$ suffices to identify the PS (Dicke) manifold
$\M_0=\oper{span}\set{\ket{m}}$, but we need additional labels to
uniquely identify states in the $r$-magnon manifold $\M_r$ with $r>0$.
Single-magnon ($r=1$) spin-wave states of SU(2) spin systems can be
written in the form $\ket{m,\m v}\equiv\O\p{\m v,\sigma_\z}\ket{m}$,
where $\m v=\p{v_1,v_2,\cdots,v_N}$ and $\sigma_\z$ is a Pauli-$z$
operator.  If an SU(2) spin system is translationally invariant, then
its spin-wave eigenstates have $v_j=e^{\i k_{\m v}\c j}/\sqrt{N}$ with
wavenumber $k_{\m v}\ne0$.  Similarly, $r$-magnon states for SU(2)
take the form
$\ket{m,\m v}\equiv\O\p{\m v,\sigma_\z^{\otimes r}}\ket{m}$, where now
$\m v$ is an $r$-index tensor that encodes spatial structure and
correlations in the excited state $\ket{m,\m v}$ (see Figure
\ref{fig:pair_structure}).

\begin{figure}
  \centering
  \includegraphics{pair_structure/pair_L16_A3_s1.pdf}
  \includegraphics{pair_structure/pair_L16_A3_s2.pdf}
  \includegraphics{pair_structure/pair_L16_A3_s3.pdf} \\[1em]
  \includegraphics{pair_structure/pair_L16_A3_s4.pdf}
  \includegraphics{pair_structure/pair_L16_A3_s5.pdf}
  \includegraphics{pair_structure/pair_L16_A3_s6.pdf}
  \includegraphics{pair_structure/pair_L16_A3_s7.pdf}
  \caption{Matrices $\m v_\Delta=\sum_{i,j}v^{(\Delta)}_{ij}\op{i}{j}$
    that generate two-magnon states
    $\ket{\m v_\Delta,m} \equiv \O\p{\m v_\Delta,
      \sigma_\z\otimes\sigma_\z} \ket{m}$ with definite excitation
    energy $\Delta$ in a periodic 1D chain of $N=16$ spins with
    power-law SU(2) couplings $h_{ij}=-1/\oper{dist}\p{i,j}^3$, where
    $\oper{dist}\p{i,j}\equiv\min\p{\abs{i-j},N-\abs{i-j}}$.  Darker
    colors correspond to larger values of $\abs*{v^{(\Delta)}_{ij}}$,
    and a blue (red) color corresponds to a positive (negative) sign
    of $v^{(\Delta)}_{ij}$.  In the case of SU($n$) with $n>2$, the
    ``two-magnon'' excitation energies $\Delta$ and matrices
    $\m v_\Delta$ remain identical, but now correspond to a degenerate
    set of states that generated by different choice of two-body
    operators in place of $\sigma_\z\otimes\sigma_\z$.}
  \label{fig:pair_structure}
\end{figure}
...
\red{[note: text below is old and unsimplified]}

We denote the manifold of all states that can be generated by the
action of some $M$-body operator, but not any $r$-body operator with
$r<M$, by $\M_M$; we will refer to $\M_M$ as the $M$-body excitation
manifold.  We then denote the a direct sum of all $\M_r$ for $r\le M$
by $\col\M_M$, i.e.~$\col\M_M\equiv\bigoplus_{r\le M}\M_r$.  The
manifold $\col\M_M$ consists of all states accessible by the action of
$r$-body operators with $r\le M$ on PS states in $\M_0$.  These
definitions are sketched out in Figure \ref{fig:manifold_sketch}.
