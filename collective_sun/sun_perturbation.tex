...
\begin{align}
  \abs{g} \equiv \sum_{S\in\PP\p{\ZZ_p}} g_S,
\end{align}
which is equivalent to the total number of dots in the Venn diagram
associated with $g$, and identify the set of values that the distinct
indices assigned according to $g$ can take:
\begin{align}
  \C_N\p{g}
...
on every tensor.  Altogether, the symmetry factors $\S\p{g}$ turn out
to be independent of $g$, and cancel out exactly with the $\sim M_j!$
prefactors in \eqref{eq:sym_prod_vec}, so
\begin{align}
  \prod_{j\in\ZZ_p} \sum_{k\in\C_N\p{M_j}} w_j\p{k} O_j\p{k}
