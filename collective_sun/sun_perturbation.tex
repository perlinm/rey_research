...
% \end{align}
% where $i,j,k$ index individual spins that the single-spin operators
% $X,Y,K$ act on; and $\m m\equiv\sum_i m_i\ket{i}$ for
% $\m m\in\set{\m u,\m v,\m w}$ are vectors of scalar coefficients.
% This product appears, for example, in the calculation of matrix
% elements of the operator $\O\p{\m v,Y}$ within the single-magnon
% manifold.  The diagrammatic expansion of this operator is
% \begin{multline}
%   \O\p{\m u,X} \O\p{\m v,Y} \O\p{\m w,Z}
