...
calculations in this section.  Here, we define diagrams as formal
objects that we can use in mathematical expressions.  Formalizing
diagrams will enable us to perform necessary calculations that are
...
\end{align}
Additionally, we will define diagrams in which dots may be replaced by
colored crosses (``$\bm\times$'') or colored pluses (``$\bm+$'').
Whereas a diagram with $\abs{g}$ dots defines a scalar that takes
$\O\p{N^\abs{g}}$ time to compute, diagrams with crosses and pluses
will be used to compute the same scalar with a reduced computational
cost.

In a diagram with dots, crosses, and pluses, the values of indices
associated with dots are summed as if the diagram had only dots,
treating all other indices as independent and fixed.  Crosses in a
diagram then correspond to indices that are summed over all of $\ZZ_N$
without additional constraints, e.g.
\begin{align}
  \diagram{example_triple_xx_2} \equiv
  \sum_{a,c\in\ZZ_N}
  \sum_{\substack{b\in\ZZ_N\\\p{d,e}\in\C_N\p{2}\\
      \abs{\set{b,d,e}}=3}}
  w_1\p{a,b,c} w_2\p{b,d,e} w_3\p{b,c,d,e}.
\end{align}
If two crosses have the same color, then the corresponding indices
must be equal:
\begin{align}
  \diagram{example_triple_xx_1} \equiv
  \sum_{c\in\ZZ_N} \sum_{\substack{a,b,d\in\ZZ_N\\\abs{\set{a,b,d}}=3}}
  w_1\p{a,b,c} w_2\p{b,d,c} w_3\p{b,c,d,c}.
\end{align}
Crosses thus correspond to indices that are entirely independent of
all other indices.  Pluses, meanwhile, correspond to indices that are
{\it maximally dependent} on the dots: ``plus indices'' are summed
{\it only} over the values of ``dot indices''.  As with crosses, the
same color on two pluses indicates that the corresponding indices are
equal, e.g.
\begin{align}
  \diagram{example_triple_pp_1}
  \equiv \sum_{\substack{a\in\ZZ_N\\\p{d,e}\in\C_N\p{2}\\
      \abs{\set{a,d,e}}=3}} \sum_{b\in\set{a,d,e}}
  w_1\p{a,b,b} w_2\p{b,d,e} w_3\p{b,b,d,e}.
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Decomposing diagrams to reduce computational costs}
\label{sec:costs}

The diagrams generated by a product of multi-body operators in
\eqref{eq:sym_prod_group} generally have only dots, without crosses or
pluses.  Such a diagram with $\abs{g}$ dots nominally takes
$\O\p{N^{\abs{g}}}$ time to compute.  In practice, this cost can be
prohibitive for performing numerical simulations of systems of any
appreciable size.  Here, we discuss the decomposition of the diagrams
that appear in \eqref{eq:sym_prod_group} into diagrams with reduced
computational cost.  As an example, we can decompose
\begin{align}
  \diagram{example_triple}
  = \diagram{example_triple_x_top} - \diagram{example_triple_p_top},
...
\end{align}
\begin{align}
  \diagram{example_triple_p_top}
  \equiv \sum_{\substack{b,c\in\ZZ_N\\\p{d,e}\in\C_N\p{2}\\
      \abs{\set{b,c,d,e}}=4\\a\in\set{b,c,d,e}}}
  w_1\p{a,b,c} w_2\p{b,d,e} w_3\p{b,c,d,e},
\end{align}
\begin{align}
  ...
  \equiv \sum_{\substack{b,c\in\ZZ_N\\\p{d,e}\in\C_N\p{2}\\
      \abs{\set{b,c,d,e}}=4}}
  w_1\p{\times,b,c} w_2\p{b,d,e} w_3\p{b,c,d,e},
  \label{eq:example_triple_x_top}
\end{align}
with
\begin{align}
  w_1\p{\times,b,c} \equiv \sum_{a\in\ZZ_N} w_1\p{a,b,c},
\end{align}
...
over $a\in\ZZ_N$ to compute $w_1\p{\times,b,c}$ has $\O\p{N}$ cost,
...
In general, in a diagram with only dots and crosses, any dot may be
decomposed similarly to \eqref{eq:example_decomposition} into a cross
and a plus (with a new color).  The only catch in making such a
replacement is that one must manually keep track of combinatorial
factors: if the replaced dot was in a region of the diagram with $D$
dots, then the new diagrams pick up a factor of $1/D$, e.g.
\begin{align}
  \diagram{example_double}
  = \f13 \diagram{example_double_x}
  - \f13 \diagram{example_double_p}.
\end{align}
This factor can be derived by first treating the values $\bar R$ of
dots {\it outside} the region of interest fixed, and replacing the sum
over $\C_N\p{D}\setminus\bar R$ for the dots {\it within} the region
by a sum over $\ZZ_N^D\setminus\bar R$.  This replacement picks up a
factor of $1/D!$ to account for the number of ways to permute $D$
values.  After setting aside one dot for replacement by a cross and a
plus, the sum over $\ZZ_N^{D-1}\setminus\bar R$ for the remaining dots
can be changed back to a sum over $\C_N\p{D-1}\setminus\bar R$,
picking up a factor of $\p{D-1}!$.  The overall factor picked up
throughout this procedure is therefore $\p{D-1}!/D!=1/D$.

While it is possible to derive rules for decomposing dots in a diagram
with pluses, it is easier to instead eliminate pluses from a diagram
altogether.
...
The results in \eqref{eq:triple_multi} and \eqref{eq:triple_col}
suffer from the fact that the coefficients $A_6$, $A_4$, and $A_2$
respectively take $\O\p{N^6}$, $\O\p{N^5}$, and $\O\p{N^4}$ time to
compute in general.  Here, we reduce the computational cost of these
coefficients.

We begin with the (only) diagram for $A_6$:
\begin{align}
  \diagram{triple_6}
  &= \sum_{\substack{\p{k,\ell},\p{p,q},\p{r,s}\in\C_N\p{2}\\
      \abs{\set{k,\ell,p,q,r,s}}=6}}
  u_{k\ell} v_{pq} w_{rs} \\
  &= \f12 \sum_{k,\ell\in\ZZ_N} u_{k\ell}
  \  \f12 \sum_{p,q\in\ZZ_N\setminus\set{k,\ell}} v_{pq}
  \  \f12 \sum_{r,s\in\ZZ_N\setminus\set{k,\ell,p,q}} w_{rs},
\end{align}
where we first simplify
\begin{align}
  \sum_{r,s\in\ZZ_N\setminus\set{k,\ell,p,q}} w_{rs}
  = \col{w}_{k\ell pq}
  \equiv \col{w} - 2 \sum_{r\in\set{k,\ell,p,q}} w_r
  + \sum_{r,s\in\set{k,\ell,p,q}} w_{rs},
\end{align}
with
\begin{align}
  \col{w} \equiv \sum_{r,s} w_{rs},
  &&
  w_r \equiv \sum_{s\in\ZZ_N} w_{rs}.
\end{align}
In turn,
\begin{align}
  \sum_{p,q\in\ZZ_N\setminus\set{k,\ell}} v_{pq} \, \col{w}_{k\ell pq}
  &=
\end{align}
