...
\end{align}
The eigenvalue problem in \eqref{eq:cond_2} can be simplified further
...
$\p{N-1}\times\p{N-1}$ matrix.  Finally, we discuss the reduction of
the two-body problem to that of diagonalizing a
$\sim\p{N/2^D}\times\p{N/2^D}$ matrix, and work out the case of $D=1$
...
\end{align}
where $k\in\ZZ_N\times2\pi/N$ (with
$\ZZ_N\equiv\set{0,1,\cdots,N-1}$).  The corresponding eigenvalues can
be determined by expanding
\begin{align}
  \m h\c\v v_k = \sum_{p,q} h_{pq} e^{iqk} \ket{p}
  = \sum_{p,d} h_{p,d+p} e^{i\p{d+p}k} \ket{p}
  = \sum_d h_{0,d} e^{idk} \v v_k
...
\end{align}
This solution also holds for a $D$-dimensional periodic lattice of
$N=L^D$ spins, with $d\in\ZZ_L^D$ and $k\in\ZZ_L^D\times 2\pi/L$ (and
$dk\to d\c k\equiv\sum_{j=1}^D d_j k_j$).
...
\end{align}
This eigenvalue problem takes an identical form a $D$ dimensional
periodic lattice of $N=L^D$ spins, in which case lattice sites and
shift matrices are indexed by vectors $d\in\ZZ_L^D$.

We can reduce the eigenvalue problem in \eqref{eq:cond_2_square} even
further by making use of reflection symmetries such as
$h_{d,0}=h_{-d,0}$.  In $D$ dimensions, there is one such symmetry for
each coordinate that indexes lattice sites, which implies that the
degrees of freedom in $\ut{\v{\m w}}$ gets reduced by a factor of
...
$D\ge1$ to the future, if and when it becomes necessary.
