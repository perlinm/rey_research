...
% TODO: consider more general perturbations, namely:
% (1) a sum over X in V_1,
% (2) more general two-body perturbations (O_pq) in V_2, and
% (3) a sum over different O_pq in V_2
...
\begin{align}
  ...
  = E_0 \V_1\p{\v v} \ket\psi
  + \p{\sum_{p,q} h_{pq} v_p X_q - \sum_q h_q v_q X_q} \ket\psi,
  &&
  h_q \equiv \sum_p h_{pq}.
  \label{eq:H_V1_psi}
\end{align}
We thus find that $\V_1\p{\v v_\Delta}\ket\psi$ is an eigenvector of
the interaction Hamiltonian $H_0$ if the coefficients $\v v_\Delta$
satisfy
\begin{align}
  \p{\m h - \diag\v h}\c\v v_\Delta = \Delta \v v_\Delta,
  \label{eq:cond_1}
\end{align}
where $\v h,\m h$ are respectively a vector and matrix of all
$h_p,h_{pq}$; $\diag\v h$ is a matrix with $\v h$ on the diagonal and
zeros everywhere else; and $\Delta$ is a scalar.  Solving the general
eigenvalue problem in \eqref{eq:cond_1} requires diagonalizing an
$N\times N$ matrix.  When the interaction Hamiltonian $H_0$ is
...
the choice of a single-spin trace-zero operator $X$ used to build
$\V_1$ in \eqref{eq:perturbations}.
% TODO: consider more general single-body perturbations V_1, which sum
% over different operators X
Finding operators $\V_1\p{\v v_\Delta}$ that generate eigenvectors of
the interaction Hamiltonian $H_0$ when they act on fully symmetric
states $\ket\psi$ thus reduces to finding eigenvectors $\v v_\Delta$
...
\begin{align}
  ...
  = - \sum_{\Delta\ne0} \f1{\Delta} \P_0 \V_1\p{\v v_\Delta}^2 \P_0,
\end{align}
...
\begin{align}
  H_1^{(2)}
  \EQFS \f1{N\p{N-1}}
  \sum_{\Delta\ne0} \f{\v v_\Delta\c\v v_\Delta}{\Delta}
  \sp{{\col{X}\,}^2 - N \col{X^2}},
  \label{eq:H_1_2}
\end{align}
where $\EQFS$ denotes equality under a restriction to the fully
...
\begin{align}
  H_0 \V_2\p{\m w} \ket\psi
  = E_0 \V_2\p{\m w} \ket\psi
...
\end{align}
We thus find that the vector $\V_2\p{\m w_\Delta}\ket\psi$ is an
eigenvector of the interaction Hamiltonian $H_0$ if the coefficients
$\m w_\Delta$ satisfy the eigenvalue equation
\begin{align}
  \check{\m h} \c \v{\m w}_\Delta = \Delta \v{\m w}_\Delta,
  \label{eq:cond_2}
\end{align}
where for any symmetric $N\times N$ matrix $\m m$ with zeroes on the
diagonal (as is the case with the coefficient matrix $\m w_\Delta$),
we define the ${N \choose 2}$-component vector
\begin{align}
  \v{\m m} \equiv \sum_{p<q} m_{pq} \ket{\set{p,q}},
  \label{eq:flat_vec}
\end{align}
and we define the ${N \choose 2} \times {N \choose 2}$ matrix
\begin{align}
...
\end{align}
The basis vectors $\ket{\set{p,q}}$ are labeled by a choice
$\set{p,q}$ of two distinct spins $p,q$.  The eigenvalue problem in
\eqref{eq:cond_2} can be simplified when the interaction Hamiltonian
$H_0$ and the perturbation $\V_2\p{\m w}$ are translationally
invariant; we discuss this simplification in Appendix
\ref{sec:trans_inv}.

If we {\it construct} a perturbation $\V_2\p{\m w_\Delta}$ with
coefficients $\m w_\Delta$ that satisfy \eqref{eq:cond_2} with
eigenvalue $\Delta$, then
\begin{align}
...
of matrices satisfying \eqref{eq:cond_2} with eigenvalue $\Delta$,
...
\begin{align}
  ...
  = -\sum_{\Delta\ne0} \f1\Delta \P_0 \V_2\p{\m w_\Delta}^2 \P_0,
\end{align}
...
symmetric matrix $\m m$ with zeros on the diagonal, similarly to the
special definition $\v{\m m}$ in \eqref{eq:flat_vec} we define
\begin{align}
  \v m \equiv \sum_{p,q} m_{pq} \ket{p},
\end{align}
which is essentially the sum over all columns (or rows) of $\m m$, in
terms of which
\begin{align}
  W_4\p{\m h,\m w}
...
\end{align}
The dependence of the coefficients of $W_k\p{\m h,\m w}$ on the
interaction matrix $\m h$ is implicit in the decomposition of the
coefficient matrix $\m w$ into components $\m w_\Delta$ as
$\m w=\sum_\Delta\m w_\Delta$, where $\m w_\Delta$ is the projection
of $\m w$ onto the $\Delta$-eigenspace of the matrix $\check{\m h}$
