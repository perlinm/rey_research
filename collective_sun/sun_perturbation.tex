...
$\sim\p{n-1}^r$ distinct choices that generate degenerate, orthogonal
states: one of $n-1$ diagonal operators (that are linearly independent
of each other and the identity) for each of $r$ tensor factors.  We
discuss the exact choice of operators in more detail in Appendix
\ref{sec:operator_content}.  To summarize, for each PS state
$\ket{m}$, we identify a basis $\set{Z_{m\nu}:\nu\in\ZZ_{n-1}}$ of
single-spin operators that are diagonal and linearly independent of
the identity, and choose a list of integers
$z\equiv\p{z_1,z_2,\cdots,z_{n-1}}\in\A_{n-1}\p{r}$ to define the
$r$-spin operator
\begin{align}
  X_{mz} \equiv \bigotimes_{\nu=1}^{n-1} Z_{m\nu}^{\otimes z_\nu}
  = \underbrace{Z_{m,1} \otimes \cdots \otimes Z_{m,1}}_{z_1~\t{times}}
  \otimes
  \underbrace{Z_{m,2} \otimes \cdots \otimes Z_{m,2}}_{z_2~\t{times}}
  \otimes \cdots \otimes
  \underbrace{Z_{m,n-1} \otimes \cdots
    \otimes Z_{m,n-1}}_{z_{n-1}~\t{times}}.
\end{align}
Our basis for $\M_r$ then consists of the states
$\ket{m,\m w,z}\equiv\O\p{\m w,X_{mz}}\ket{m}$.
