...
occupation number of the single-spin state $\mu$, with $m_\up+m_\dn=N$
and $m_\up-m_\dn=m_\z$.
...
% focus on energy -- how different from non-interacting magnons?
...
$\ket{m,\m w,\sigma_\z^{\otimes r}}$.  Considering states of this
form, built using tensor products of Pauli-$z$ ($\sigma_\z$)
operators, suffices to construct a basis for all excited states of an
SU(2) spin system (see Appendix \ref{sec:operator_content}).  Note
that $r$-body operators with arbitrary coefficients $\m w$ will
generally couple the PS manifold $\M_0$ to all $q$-magnon manifolds
$\M_q$ with $q\le r$ (see Figure \ref{fig:manifold_sketch}).  In order
to generalize the notion of $r$-magnon manifolds $\M_r$ to the case of
SU($n$) with $n>2$, where the analogous notion of a spin length $S$ is
less intuitive, we can define $\M_r$ for $r>0$ as the manifold of
states that (i) can be generated by the action of $r$-body operators
on $\M_0$, and (ii) are orthogonal to all states in
$\bigoplus_{q<r}\M_q$.
...
$\col\M_{r_{\t{max}}}\equiv\bigotimes_{r\le r_{\t{max}}}\M_r$.  The
purpose of constructing the basis $\col\B_{r_{\t{max}}}$ is to
simulate the dynamics induced by the addition of a spin operator
$\O\p{\m v,Y}$ to the base Hamilonian $H_0$, such that the total
Hamiltonian is $H = H_0 + \O\p{\m v,Y}$.  Throughout the construction
of $\col\B_s$, an appropriate choice of tensors $\m w$ ensures that
all states $\ket{m,\m w,X}\in\col\B_s$ are eigenstates of the base
Hamiltonian $H_0$ (see Appendix \ref{sec:eigenstates}), which is to
say that $H_0$ is diagonal with respect to $\col\B_s$.  Matrix
elements of $\O\p{\m v,Y}$, meanwhile, take the form
\begin{align}
...
\ref{sec:operator_product}.  For brevity, we will refer to numerical
simulations restricted to $\col\M_{r_{\t{max}}}$ as {\it
  order-$r_{\t{max}}$ truncated shell} (TS$_{r_{\t{max}}}$)
simulations.

An $M$-spin operator $\O\p{\m v,Y}$ can only couple $r$-magnon states
to $q$-magnon states with $\abs{q-r}\le M$.  In the perturbative
regime, $\norm{\O\p{\m v,Y}}<\Delta_{\t{gap}}/2$, TS$_{r_{\t{max}}}$
simulations of initially PS (zero-magnon) states therefore capture all
virtual processes through order $2\floor{r_{\t{max}}/M}+1$ in
perturbation theory.  When $\norm{\O\p{\m v,Y}}\ge\Delta_{\t{gap}}/2$,
...
within large-$r$ manifolds $\M_r$ are small, one should expect these
