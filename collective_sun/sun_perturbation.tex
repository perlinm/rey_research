...
\begin{align}
  ...
  = \f1{M!} \sum_{\sigma\in\pi\p{M}} \sum_{k\in\C_N\p{M}}
  w_{\sigma\p{k}} X_{\sigma\p{k}},
\end{align}
...
\eqref{eq:off_diags}; $\pi\p{M}$ is the permutation group of $\ZZ_M$;
and $\C_N\p{M}$ is the set of subsets (``choices'') of $M$ elements
from $\ZZ_N$, defined in \eqref{eq:choices}.  For shorthand, we will
define $\tilde X_{\m w}\equiv M! X_{\m w}$.
...
$X_k$ and annihilates on $\ket\psi$, so
\begin{align}
...
energy of PS states $\ket\psi\in\M_0$.  Simplifying
\begin{align}
...
\end{align}
The sum in \eqref{eq:diagnosis_start} has terms with $p,q\notin k$,
terms with $p,q\in k$, and terms with only one of $p$ or $q\in k$.
The permutation operator $\Pi_{pq}$ acts trivially on $X_k\ket\psi$
when $p,q\notin k$, so
\begin{align}
...
$M=2$, then $h_{pq}^{\t{D}}=h_p+h_q-h_{pq}$, so
\begin{align}
...
$N!_M\times N!_M\sim N^M\times N^M$, where
$N!_M\equiv\prod_{j=0}^{M-1}\p{N-j}$ denotes a falling factorial.

\vspace{3cm}

If the interaction Hamiltonian $H_0$ has any spatial symmetries,
however, then we can use these symmetries to reduce the complexity of
this eigenvalue problem.  Translational invariance, for example, can
reduce this problem to that of diagonalizing a matrix with dimensions
...
$\set{p,q}\in\C_N\p{2}$ that are a fixed distance $\abs{p-q}=d_s$
apart, i.e.~$s=\set{\set{p,q}\in\C_N\p{2}:\abs{p-q}=d_s}$.
