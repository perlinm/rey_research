...
$O\in S$.  To this end, for any partition\footnote{A partition
  $P\equiv\set{p}$ of $S$ consists of mutually disjoint, non-empty
...
\end{enumerate*}
For any set $S$ of single-spin operators or partition $P$ of $S$, we
can similarly define the collective operators
\begin{align}
...
$\abs{S}\in\set{1,2,3,4}$ (and, by conjecture, all $S$), by use of a
computer algebra system (namely, Mathematica) we find that
\begin{align}
  N!_{\abs{S}}\, O_S
  &\EQFS \sum_{\substack{\t{partitions}\\P~\t{of}~S}}
  ...
  \times \prod_{p\in\tilde P} \p{\abs{p}-1}!
...
$\Pi$, as well as parts $p\in\tilde P$, are lexicographically ordered
in accordance with the ordering of operators in $\Pi$.  If
...
\begin{align}
  ...
  &&
  O_{\tilde P} \equiv \bigotimes_{p\in\tilde P} \prod_{O\in p} O,
  &&
  \O_{\tilde S} \equiv \prod_{O\in\tilde S} \col{O},
  &&
  \O_{\tilde P}
  \equiv \prod_{p\in\tilde P} \col{\textstyle\prod_{O\in p} O},
\end{align}
in which case we can also expand
\begin{align}
  \O_{\tilde S} \EQFS
