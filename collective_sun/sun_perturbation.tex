...
\begin{align}
  ...
  = \sum_{r,s\in\S_M} h_{rs} \op{r}{s},
\end{align}
where
\begin{align}
  h_{rs} \equiv \sum_{k,\ell\in\C_N} h_{k\ell} \bk{r|k} \bk{\ell|s}
  = \f1{\sqrt{\abs{r}\abs{s}}}
  \sum_{\substack{k\in r\\\ell\in s}} h_{k\ell}.
\end{align}
In practice, it can be computationally cheaper to construct
\begin{align}
  \check{\m h}_{\S_M} = \sum_{s\in\S_M} \ket{s} \f1{\sqrt{\abs{s}}}
  \sum_{k\in s} \sum_{\substack{p\in\ZZ_N\\p\notin k}} \sum_{a\in\ZZ_M}
  \f{h_{p k_a}}{\sqrt{\abs{\sp{k_{a:p}}}}} \bra{\sp{k_{a:p}}},
\end{align}
where $k_a$ is the $a$-th element of $k$;
