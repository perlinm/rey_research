...
\begin{figure}
  \centering \includegraphics[valign=m]{sketch_spins.pdf} $\stackrel{\displaystyle\Pi_{ij}} {\displaystyle\longleftrightarrow}$ \includegraphics[valign=m]{sketch_spins_swap.pdf}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Schematic of relationships between $q$-magnon manifolds $\M_q$ with total spin $S=N/2-q$ in an SU(2) spin system.
    Single-body operators $\O\p{\m v,\sigma_\z}$ generally couple the ground-state manifold $\M_0$ to the single-magnon manifold $\M_1$ as well as $\M_0$ itself, while $r$-body operators $\O\p{\m w,\sigma_\z^{\otimes r}}$ generally couple $\M_0$ to all $q$-magnon manifolds $\M_q$ with $q\le r$.}
...
The spectral gap of the interaction Hamiltonian is then determined by well-known spin-wave theory (see Ref.~[\citenum{ashcroft1978solid}] or Appendix \ref{sec:single_body_eigenstates})
\begin{align}
  \Delta
...
\begin{align}
  ...
  \EQPS \diagram{prod_b11_c0} X_1 Y_2
  + \diagram{prod_b11_c1} X_1 Y_1,
\end{align}
where $i,j$ index individual spins that the single-spin operators $X,Y$ act on; and $\m m\equiv\sum_i m_i\ket{i}$ for $\m m\in\set{\m v,\m w}$ are vectors of scalar coefficients.
This product appears, for example, in the calculation of the second-order effective Hamiltonian $H_1^{(2)}$ induced on the permutationally symmetric manifold $\M_0$ by a single-body perturbation.
Using the rules that we worked out in Appendix \ref{sec:diagrams} for reducing diagrams, we can simplify
\begin{align}
  \diagram{prod_b11_c0}
  = \diagram{prod_b11_c0_o} - \diagram{prod_b11_c0_x}
  = \diagram{prod_b11_c0_oo} - \diagram{prod_b11_c1}
  = \col{v}\,\col{w} - \m v \c \m w,
\end{align}
where $\col{m} \equiv \sum_i m_i$ for both $\m m\in\set{\m v,\m w}$, so
\begin{align}
  \O\p{\m v,X} \O\p{\m w,Y}
  \EQPS \p{\col{v}\,\col{w} - \m v\c\m w} X_1 Y_2
  + \m v\c\m w X_1 Y_1.
\end{align}
In order to write this result in terms of collective operators $\col{Z} \equiv \sum_i Z_i$, we expand
\begin{align}
...
\end{align}
where $i,j,k,\ell$ index individual spins that the two-spin operators $X,Y$ act on; and $\m m\equiv\sum_{i,j}m_{ij}\op{i}{j}$ for $\m m\in\set{\m v,\m w}$ are matrices of scalar coefficients, with $m_{ii}=0$.
This product appears, for example, in the calculation of the second-order effective Hamiltonian $H_2^{(2)}$ induced on the permutationally symmetric manifold $\M_0$ by a two-body perturbation.
The diagrammatic expansion of this operator is
\begin{align}
  \O\p{\m v,X} \O\p{\m w,Y}
  \EQPS \diagram{prod_b22_c0} \spp{X,Y}_4 + \diagram{prod_b22_c1}
  \spp{X,Y}_3 + \diagram{prod_b22_c2} \spp{X,Y}_2,
  \label{eq:two_prod_pair}
\end{align}
where $\spp{X,Y}_q$ denotes a sum over all $q$-spin operators that can be constructed from $X$ and $Y$, i.e.
\begin{align}
...
\end{align}
To minimize the computational cost of the diagrams in \eqref{eq:two_prod_pair}, we reduce
\begin{align}
  \diagram{prod_b22_c1}
  &= \sp{\diagram{prod_b22_c1_o}} - \sp{\diagram{prod_b22_c1_x}} \\
  &= \sp{\diagram{prod_b22_c1_oo} - \diagram{prod_b22_c1_ox}}
  - \sp{\diagram{prod_b22_c2}} \label{eq:mid_zero} \\
  &= \diagram{prod_b22_c1_ooo} - \diagram{prod_b22_c2_oo}
\end{align}
where the middle diagram in \eqref{eq:mid_zero} is zero from the fact that $w_{ii}=0$, and
\begin{align}
  \diagram{prod_b22_c0}
  &= \sp{\diagram{prod_b22_c0_o}} - \sp{\diagram{prod_b22_c0_x}} \\
  &= \sp{\diagram{prod_b22_c0_oo} - \diagram{prod_b22_c0_ox}}
  - \sp{2 \diagram{prod_b22_c1}} \\
  &= \sp{\diagram{prod_b22_c0_oooo} - 2 \diagram{prod_b22_c1_o}}
  - \sp{2 \diagram{prod_b22_c1_ooo} - 2 \diagram{prod_b22_c2_oo}} \\
  &= \diagram{prod_b22_c0_oooo} - 4 \diagram{prod_b22_c1_ooo}
  + 2 \diagram{prod_b22_c2_oo}.
\end{align}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \subsection{Three single-body operators}
% \label{sec:single_trip_prod}

% Here we simplify the single-body product
% \begin{align}
%   \O\p{\m u,X} \O\p{\m v,Y} \O\p{\m w,Z}
%   = \sum_{i,j,k} u_i v_j w_k X_i Y_j Z_k,
% \end{align}
% where $i,j,k$ index individual spins that the single-spin operators $X,Y,K$ act on; and $\m m\equiv\sum_i m_i\ket{i}$ for $\m m\in\set{\m u,\m v,\m w}$ are vectors of scalar coefficients.
% This product appears, for example, in the calculation of matrix elements of the operator $\O\p{\m v,Y}$ within the single-magnon manifold.
% The diagrammatic expansion of this operator is
% \begin{multline}
%   \O\p{\m u,X} \O\p{\m v,Y} \O\p{\m w,Z}
%   \EQPS \diagram{prod_b111_c0} X_1 Y_2 Z_3
%   + \diagram{prod_b111_c10} X_1 Y_1 Z_1 \\
%   + \diagram{prod_b111_c11_u} X_1 Y_2 Z_2
%   + \diagram{prod_b111_c11_v} X_2 Y_1 Z_2
%   + \diagram{prod_b111_c11_w} X_2 Y_2 Z_1,
% \end{multline}
% where we can reduce (by hand or with a computer)
...
\end{align}
projected onto the PS manifold $\M_0$, where $i,j,k,\ell,p,q$ index individual spins; $Z$ is a single-spin Ising-like operator, with $Z^2=1$; and $\m m\equiv\sum_{i,j} m_{ij}\op{i}{j}$ for $\m m\in\set{\m u,\m v,\m w}$ are matrices of scalar coefficients, with $m_{ii}=0$.
This product appears, for example, in the projection of Ising interactions onto the two-magnon manifold $\M_2$.
Collecting terms according to the number of $Z$ operators that remain after accounting for $Z^2=1$, we find that
\begin{align}
  \O\p{\m u,\m v,\m w,Z}
...
with
\begin{align}
  A_6 \equiv \diagram{prod_b222_c0},
  &&
  A_4 \equiv 4 \diagram{prod_b222_c01} + 8 \diagram{prod_b222_c1},
  &&
  A_0 \equiv 8 \diagram{prod_b222_c0111},
  \label{eq:triple_64}
\end{align}
\begin{align}
  A_2 \equiv 8 \diagram{prod_b222_c011} + 2 \diagram{prod_b222_c02}
  + 8 \diagram{prod_b222_c11} + 4 \diagram{prod_b222_c2},
  \label{eq:triple_2}
\end{align}
where a diagram with unlabeled circles denotes a sum over all distinct label assignments, e.g.
\begin{align}
  \diagram{prod_b222_c1} \equiv \diagram{prod_b222_c1_uvw},
  \label{eq:triple_uvw}
\end{align}
\begin{align}
  \diagram{prod_b222_c011}
  \equiv \diagram{prod_b222_c011_uvw}
  + \diagram{prod_b222_c011_vwu} + \diagram{prod_b222_c011_wuv}.
  \label{eq:triple_uvw_3}
\end{align}
The last diagram in each of $A_6,A_4,A_2,A_0$ as written in \eqref{eq:triple_64}, \eqref{eq:triple_2} thus has only one assignment of labels, as in \eqref{eq:triple_uvw}, while all other unlabeled diagrams have three assignments, as in \eqref{eq:triple_uvw_3}.

Having identified diagrammatic representations of the coefficients $A_6,A_4,A_2,A_0$, we now need to simplify all diagrams to reduce their computational complexity, as discussed in Appendix \ref{sec:diagrams}.
The process of eliminating filled dots and crosses from a diagram is algorithmic enough to execute on a computer, doing which we find
\begin{align}
  A_6 = D_6 - D_4 + D_2 - D_0,
  &&
  A_4 = D_4 - 2 D_2 + 3 D_0,
...
where
\begin{align}
  D_6 \equiv \diagram{prod_b222_c0_o},
  &&
  D_4 \equiv 4 \diagram{prod_b222_c01_o} - 16 \diagram{prod_b222_c1_o},
  &&
  D_0 \equiv 8 \diagram{prod_b222_c0111_o},
\end{align}
\begin{align}
  D_2 \equiv 8 \diagram{prod_b222_c011_o}
  + 2 \diagram{prod_b222_c02_o}
  - 16 \diagram{prod_b222_c11_o}
  + 16 \diagram{prod_b222_c2_o}.
\end{align}
If $m_{ij}=m_{ji}$ for all $\m m\in\set{\m u,\m v,\m w}$, then we can define
\begin{align}
  \v{\m m} \equiv \sum_{i\ne j} m_{ij} \ket{ij},
  &&
...
\end{multline}
Finally, we can also write the product $\O\p{\m u,\m v,\m w,Z}$ in terms of the collective operator $\col{Z} \equiv \sum_i Z_i$:
\begin{align}
  \O\p{\m u,\m v,\m w,Z}
