...
\end{align}
where $X_p,Y_p$ are trace-zero operators on spin $p$, with e.g.~$X_p$
related to $X_q$ by a permutation of spins $p$ and $q$; $\v X,\v Y$
...
\begin{align}
  ...
  = \sum_p v_p \P_0 X_0 \P_0 \FSEQ \bar v \X,
  \label{eq:H_1_1}
\end{align}
...
choice of single-spin trace-zero operator $X_p$ used to build $\V_1$
...
\begin{align}
  H_1^{(2)} \FSEQ \f{\tilde v}{N-1} \p{\X^2 - N \X^{(2)}},
  &&
...
\begin{align}
  H_2^{(1)}
  = \P_0 \V_2 \P_0
  \FSEQ \f12 \bar w \p{\X\odot\Y - \O_{X,Y}},
\end{align}
...
\begin{align}
  ...
  &&
  \O_{X,Y} \equiv \sum_p X_p \odot Y_p.
\end{align}
In order to compute the second-order effective Hamiltonian
...
\begin{align}
  ...
  - \v\1 \c \p{\m h\oc\m w} \c \v O_{X,Y} \ket\psi,
  \label{eq:H_V2_psi}
\end{align}
where $\m w,\m h,\bar{\m h}$ are matrices of all
$w_{k\ell},h_{k\ell},\bar h_{k\ell}$, with
\begin{align}
  \bar h_{k\ell} \equiv h_{k\ell} - \f12\p{h_k + h_\ell},
  &&
  h_k \equiv \sum_\ell h_{k\ell};
\end{align}
$\m x\oc\m y$ is an element-wise (Hadamard) product of $\m x$ and
$\m y$, i.e.~with matrix elements $x_{k\ell} y_{k\ell}$;
$\v\1\equiv\p{1,1,1,\cdots}$ is an $N$-component vector of ones; and
$\v O_{X,Y}$ is a vector with components $\p{X_kY_k+Y_kX_k}/2$, i.e.
\begin{align}
  \v O_{X,Y} \equiv \f12\p{\v X\oc\v Y + \v Y\oc\v X}.
\end{align}
We thus find that the vector $\V_2\ket\psi$ is an eigenvector of the
interaction Hamiltonian $H_0$ if the coefficient matrix $\m w$ of all
$w_{pq}$ satisfies
\begin{align}
...
that the last term ($\sim\v O_{X,Y}$) in \eqref{eq:H_V2_psi} vanishes.
...
$\tilde{\m h}$ are then built from eigenvectors $\v w_\ell$ within
...
\begin{align}
  ...
  - \sum_{k,\ell} w_{k\ell} h_{k\ell} X_k \odot Y_k \ket\psi
\end{align}
where $A\odot B \equiv \p{AB+BA}/2$ is a symmetrized product.  In
total,
\begin{align}
  H_0 \V_2 \ket\psi
  = E_0 \V_2 \ket\psi
...
\begin{align}
  ...
  = \sum_p X_p^2
  + \sum_{p\ne q} X_p X_q
  \FSEQ \X^{(2)} + N \p{N-1} X_0 X_1,
  \label{eq:PXXP_neq_ops}
\end{align}
which implies that
\begin{align}
  \sum_{p\ne q} v_p v_q X_p X_q
  \FSEQ - N \var\p{\v v} X_0 X_1
  \FSEQ -\f{\var\p{\v v}}{N-1} \p{\X^2 - \X^{(2)}}
  \label{eq:PXXP_neq}
\end{align}
Altogether, we have that
\begin{align}
  \V_1\p{\v v}^2
  \FSEQ -\f{\var\p{\v v}}{N-1} \p{\X^2 - N \X^{(2)}}.
\end{align}
...
and expand
\begin{align}
  \X\odot\Y \FSEQ \O_{X,Y} + N\p{N-1} X_0 Y_1.
\end{align}
Substituting the product $X_0 Y_1$ back into \eqref{eq:PXYP_start}
gives us
\begin{align}
  \V_2 \FSEQ \f12 \bar w \p{\X\odot\Y -  \O_{X,Y}}.
\end{align}
