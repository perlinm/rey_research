...
\begin{align}
  H_0 \V_2 \ket\psi
  = \sum_{\substack{k<\ell\\p<q}} w_{k\ell} h_{pq}
  \Pi_{pq} O_{k\ell} \ket\psi,
  \label{eq:H_V2_psi_start}
\end{align}
where without loss of generality we consider a two-body perturbation
$\V_2$ built out of only one two-body operator $O$.  The sum in
\eqref{eq:H_V2_psi_start} has terms with $\set{p,q}=\set{k,\ell}$,
terms with $p,q\notin\set{k,\ell}$, and terms with only one of $p$ or
$q\in\set{k,\ell}$.  The permutation operator $\Pi_{pq}$ acts
trivially on $O_{k\ell}\ket\psi$ when $\set{p,q}=\set{k,\ell}$ or
...
\begin{align}
  E_0 \V_2 \ket\psi
  + \sum_{k<\ell} 2 \bar h_{k\ell} w_{k\ell} O_{k\ell} \ket\psi.
\end{align}
...
\begin{align}
  ...
  + \f12 \sum_{k\ne\ell} \sp{2\bar h_{k\ell} w_{k\ell}
    + \sum_q \p{h_{kq} w_{q\ell} + w_{Okq} h_{q\ell}}}
  O_{k\ell} \ket\psi.
\end{align}
