...
interdependent (such as $\abs{\set{a,b,c,d,e}}=5$ above) by
unconstrained sums that can be carried out in sequence.  In order to
represent different constraints, we need to define diagrams in which
filled dots (``$\bullet$'') may be replaced by empty dots
...
eliminating filled dots, and eliminating crosses.
...
\end{align}
The decomposition in \eqref{eq:example_elim} essentially breaks
up a sum over $a\in\ZZ_N\setminus\set{b,c}$ into a difference of sums
over $a\in\ZZ_N$ and $a\in\set{b,c}$.  The sum over $a\in\ZZ_N$ to
compute $v\p{\circ,b}$ has $\O\p{N^2}$ cost, and can be performed
prior to evaluating the diagram in \eqref{eq:example_elim_o}.
The decomposition in \eqref{eq:example_elim} therefore splits
one $\O\p{N^3}$ into two $\O\p{N^2}$ diagrams.

After splitting a filled dot into an empty dot and a cross, the next
...
\begin{align}
  ...
  = 2 \sum_{b,c\in\C_N\p{2}} v\p{c,b} w\p{b,c}.
...
$F_{\t{new}}$ filled dots picks up an overall factor of $F_{\t{new}}$,
so
\begin{align}
  \diagram{example_sym_x}
  = 2 \diagram{example_sym_x_elim},
\end{align}
where the factor of $1/F_{\t{old}}$ that would be acquired from
removing a dot from the old region is canceled out by a factor of
$F_{\t{old}}$ that accounts for the number of ways to move a dot from
the old region to the new one.
...
\begin{align}
  \diagram{two_body_2} \equiv \sum_{p,q\in\C_N\p{2}} v_{pq} w_{pq}
  = \v{\m v} \c\v{\m w},
\end{align}
and simplify
\begin{align}
