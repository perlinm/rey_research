...
In order to project operators onto $\ul{\M}_{r_{\t{max}}}$, we need to
construct a basis for each $\M_r$ with $r\le r_{\t{max}}$.
...
simplify the eigenvalue problem in \eqref{eq:multi_body_eig}.

Our task is essentially to express the multi-body eigenvalue problem
in \eqref{eq:multi_body_eig} in a basis that respects the spatial
...
obey the same spatial symmetries as $H_0$, these symmetries generally
endow $\check{\m h}$ with a block-diagonal structure. \red{[TODO:
  finish discussion/derivation]}
...
$N$ spins to $n$ states by $\A_n\p{N}$, such that the basis
$\set{\ket{a}:a\in\A_n\p{N}}$ spans permutationally symmetric manifold
of $N$ spins.  Identifying this basis allows us to expand
\begin{align}
  \bk{a|X|b}
