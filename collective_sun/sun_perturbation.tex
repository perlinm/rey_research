...
eigenvector/eigenvalue problem.  We begin by writing the matrix $\m v$ as a vector,
\begin{align}
  \m v = \sum_{p,q} v_{pq} \op{k}{q}
  \to \check{\m v} \equiv \sum_{p,q} v_{pq} \ket{pq},
\end{align}
and then defining the matrix
\begin{align}
  \check{\m h}
  \equiv \sum_{k,\ell} \bar h_{k\ell} \op{k\ell}
  + \sum_{k,q,\ell} h_{kq} \op{k}{q} \otimes \op{\ell},
  \label{eq:h_super_mat}
\end{align}
in terms of which the condition in \eqref{eq:v_cond_2} becomes
\begin{align}
  \check{\m h} \c \check{\m v} = \e\p{\m v} \check{\m v}.
\end{align}
Note that the matrix $\check{\m h}$ in \eqref{eq:h_super_mat} is
block-diagonal, with $N$ blocks of size $N\times N$.  The task of
constructing eigenvectors $\V_2\p{\m v}\ket\psi$ of $H_0$ and
determining their energy thus reduces to finding the eigenvectors and
eigenvalues of $N$ matrices of size $N\times N$.
