...
Pauli operator.  This permutation operator is equivalently the
multilevel SWAP operation, which merely swaps the state of two spins.
...
manifold of $N$ qubits, spanned by ${N+2-1\choose2-1}=N+1$ Dicke
states $\ket{m_0,m_1}$ for which $m_0+m_1=N$ and the integer $m_0$
($m_1$) indicates the total occupation number of qubit state $\ket{0}$
($\ket{1}$).  The energy of any PS state $\ket\psi\in\M_0$ with
respect to the interaction Hamiltonian $H_0$ in \eqref{eq:H_0} is
\begin{align}
